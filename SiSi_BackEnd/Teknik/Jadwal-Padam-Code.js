...
function simpanJadwalPadam(payload){
  try{
    ...
    var peny=_jpText_(payload.penyulang),section=_jpText_(payload.section),jenis=_jpText_(payload.jenis),tanggal=_jpText_(payload.tanggal),jamPadam=_jpText_(payload.jamPadam),jamNyala=_jpText_(payload.jamNyala);if(!peny||!section||!jenis||!tanggal||!jamPadam||!jamNyala)return {ok:false,message:'Penyulang, Section, jenis pekerjaan, tanggal, dan jam wajib diisi.'};
    ...
    var mulai=_jpParseTime_(jamPadam),selesai=_jpParseTime_(jamNyala);if(mulai==null||selesai==null)return {ok:false,message:'Format jam tidak valid.'};if(selesai<mulai)selesai+=1440;var durasi=(selesai-mulai)/60,beban=Number(master.bebanMw)||0,ens=beban*durasi;
    ...
    _jpPut_(row,map,sh.getLastRow(),'No');_jpPut_(row,map,kode,'Kode Jadwal Padam');_jpPut_(row,map,master.ulp,'ULP');_jpPut_(row,map,master.penyulang,'Penyulang');_jpPut_(row,map,master.section,'Section');_jpPut_(row,map,jenis,'Jenis Pekerjaan','Jenis Pekejaan');_jpPut_(row,map,new Date(tanggal+'T00:00:00'),'Tanggal');_jpPut_(row,map,_jpTimeDate_(jamPadam),'Jam Padam');_jpPut_(row,map,_jpTimeDate_(jamNyala),'Jam Nyala');_jpPut_(row,map,durasi,'Durasi');_jpPut_(row,map,master.jumlahGardu,'Jumlah Gardu');_jpPut_(row,map,master.jumlahPelanggan,'Jumlah Pelanggan');_jpPut_(row,map,master.daerahSection,'Daerah Section','Daerah Padam','Dearah Padam');_jpPut_(row,map,master.arus,'Beban (A)','Beban','Arus (A)');_jpPut_(row,map,ens,'ENS');_jpPut_(row,map,master.pelangganVip,'Pelanggan VIP','Pelanggan VIP Padam');_jpPut_(row,map,_jpText_(payload.lokasi),'Lokasi Pekerjaan');_jpPut_(row,map,'Terjadwal','Status Jadwal Pekerjaan','Status');
