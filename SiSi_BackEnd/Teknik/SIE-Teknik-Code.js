...
var SHEET_SIE_DB = 'DB';
function _sieSS(){ return SpreadsheetApp.openById(SIE_SPREADSHEET_ID); }

/* === PETA KOLOM sheet DB (master switching/penyulang) — 0-based ===
   No | JENIS SWITCHING | SECTION | ULP | PENYULANG | KODE ULP */
var COL_SIE_DB = { no:0, jenisSwitching:1, section:2, ulp:3, penyulang:4, kodeUlp:5 };
var COL_SIE_DB_N = 6;
...
var COL_GASPOL = {
  no:0, tanggal:1, section:2, ulp:3,
  insSutmKms:4, insSutmTier1:5, insSutmTier2:6,            // E,F,G  Inspeksi Jaringan
  rowRabas:7, rowSedang:8, rowBesar:9,                     // H,I,J  ROW
  harkomIsolator:10, harkomArrester:11, harkomFco:12,      // K,L,M  HARKOM
  harkonsTiang:13, harkonsSutm:14, harkonsGw:15, harkonsGsw:16,
  groundingTm:17, harkeypoint:18, penghalangHewan:19,
  rekonektorSutm:20, rehabSutm:21, penyulangBaruSutm:22,   // N..W   HARKONS
  noGardu:23, insGarduTier1:24, insGarduTier2:25,          // X,Y,Z  Inspeksi Gardu
  manajemenGardu:26, tapChanger:27, penyeimbanganBeban:28,
  groundingGardu:29, coverGardu:30, lostContactPhbTr:31,   // ..AF   Gardu
  jurusanBaruPecahBeban:32, rekonektorSutr:33, srSeri:34, rekonektorSr:35, // AG..AJ SUTR/SR
  cfa:36, pemasanganAvr:37                                 // AK,AL  Lainnya
};
var COL_GASPOL_N = 38;
...
function _sieReadDbMaster(){
  ...
  for(var i=0;i<data.length;i++){
    ...
    if(!byPeny[k]){ byPeny[k] = rec; }
    else if(isPangkal && !/pangkal/i.test(byPeny[k].jenisSwitching)){ byPeny[k] = rec; }
  }
...
function _sieKey(peny, tgl){ return String(peny||'').trim() + '||' + String(tgl||''); }
...
function _gaspolFromROW(f){
  ...
  }catch(e){ Logger.log('[_gaspolFromROW] ' + e.message); }
  return map;
}
...
function _gaspolFromInspeksiJar(f){
  ...
  }catch(e){ Logger.log('[_gaspolFromInspeksiJar] ' + e.message); }
  return map;
}
...
function _gaspolFromInspeksiGardu(f){
  ...
  try{
    ...
    for(var i=0;i<data.length;i++){
      ...
      if(nomor && map[k].noGardu.indexOf(nomor) < 0) map[k].noGardu.push(nomor);
      if(tier.indexOf('1') >= 0) map[k].tier1 += 1;
      if(tier.indexOf('2') >= 0) map[k].tier2 += 1;
    }
  }catch(e){ Logger.log('[_gaspolFromInspeksiGardu] ' + e.message); }
  return map;
}
...
function _htkNorm(s){ return String(s==null?'':s).toLowerCase().replace(/\s+/g,' ').trim(); }
...
function _gaspolReadHartekMap(){
  var map = {};
  try{
    var sh = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(SHEET_HTK_LIST);
    if(!sh || sh.getLastRow() < 2) return map;
...
function _gaspolClassifyHartek(pekerjaan, jenis){
  ...
  if(/rekonektor/.test(p)){                                      // Rekonektorisasi (verb sendiri)
  ...
  if(/grounding|arding|galvanis/.test(p)){                       // Grounding: pisah per Jenis
    return (j.indexOf('gardu') >= 0) ? 'groundingGardu' : 'groundingTm';
  }
  if(isGanti){                                                   // HARKOM + Manajemen Gardu: Penggantian
  ...
  if(isPerbaikan){                                               // HARKONS umum: Perbaikan
...
function _gaspolFromHartek(f){
  ...
  try{
    ...
    var C = { tanggal:6, jenis:7, penyulang:8, pekerjaan:11, jumlahPekerjaan:12 };
  ...
  }catch(e){ Logger.log('[_gaspolFromHartek] ' + e.message); }
  return map;
}
...
function _gaspolFromYandal(f){
  ...
  try{
    ...
    var C = { kodeShift:2, kodeP0:3, tanggal:6, penyulang:9, namaPekerjaan:7, section:10 };
  ...
  }catch(e){ Logger.log('[_gaspolFromYandal] ' + e.message); }
  return map;
}
...
function getGaspolRekap(opts){
  try{
    ...
    var keys = {};
    Object.keys(src).forEach(function(s){ Object.keys(src[s]).forEach(function(k){ keys[k] = true; }); });

    var out = [];
    Object.keys(keys).forEach(function(k){
      var parts = k.split('||');
      var peny = parts[0], tgl = parts[1];
      ...
      var _secSrc = (src.insJar[k] && src.insJar[k].section)
      ...
      // ROW → H,I,J
      var r = src.row[k];
      if(r){ row[COL_GASPOL.rowRabas]=r.rabas; row[COL_GASPOL.rowSedang]=r.sedang; row[COL_GASPOL.rowBesar]=r.besar; }

      // Inspeksi Jaringan → E,F,G
      var ij = src.insJar[k];
      if(ij){ row[COL_GASPOL.insSutmKms]=ij.kms?Math.round(ij.kms*100)/100:0; row[COL_GASPOL.insSutmTier1]=ij.tier1||0; row[COL_GASPOL.insSutmTier2]=ij.tier2||0; }  // kmS dibulatkan 2 desimal

      // Inspeksi Gardu → X,Y,Z. X = daftar nomor gardu (teks), Y/Z = jumlah Tier.
      var g = src.gardu[k];
      if(g){ row[COL_GASPOL.noGardu]=(g.noGardu||[]).join(', '); row[COL_GASPOL.insGarduTier1]=g.tier1||0; row[COL_GASPOL.insGarduTier2]=g.tier2||0; }
      ...
      // Yandal → AK
      var y = src.yandal[k];
      if(y){ row[COL_GASPOL.cfa]=y.nilai||0; }

      out.push(row);
    });

    out.sort(function(a,b){
      return String(a[COL_GASPOL.tanggal]).localeCompare(String(b[COL_GASPOL.tanggal]))
          || String(a._penyulang).localeCompare(String(b._penyulang));
    });
    out.forEach(function(row,i){ row[COL_GASPOL.no]=i+1; });  // _penyulang dipertahankan utk submit (otomatis di-drop saat serialisasi ke frontend)
...
function _inputTblKey(peny, tgl, ulp){
  peny = String(peny||'').trim().toLowerCase();
  tgl  = _sieNormTgl(tgl);
  ulp  = String(ulp||'').trim().toLowerCase();
  if(!peny && !tgl) return '';
  return peny + '||' + tgl + '||' + ulp;
}
...
function _inputTblLastRowB(sh){
  ...
  for(var i=colB.length-1;i>=0;i--){
    if(String(colB[i][0]==null?'':colB[i][0]).trim() !== '') return i + 1;
  }
  return 0;
}
...
function submitGaspolToInputTbl(opts){
  try{
    ...
    var rows = rekap.rows || [];
    if(!rows.length) return { ok:false, error:'Tidak ada data pada periode/filter ini.' };

    var sh = _sieSS().getSheetByName(SHEET_SIE_INPUT_TBL);
    if(!sh) return { ok:false, error:"Sheet '" + SHEET_SIE_INPUT_TBL + "' tidak ditemukan." };
    ...
    if(lastDataRow >= dataStart){
      var exist = sh.getRange(dataStart, 1, lastDataRow-dataStart+1, nCols).getValues();
      for(var r=0;r<exist.length;r++){
        var ek = _inputTblKey(exist[r][COL_INPUT_TBL_PENYULANG], exist[r][COL_GASPOL.tanggal], '');
        if(ek) idx[ek] = dataStart + r;              // nomor baris (1-based)
      }
    }

    var appended = 0, replaced = 0;
    for(var i=0;i<rows.length;i++){
      ...
      var key = _inputTblKey(line[COL_INPUT_TBL_PENYULANG], line[COL_GASPOL.tanggal], '');
      var targetRow;
      if(key && idx[key]){ targetRow = idx[key]; replaced++; }
      else { targetRow = appendRow++; appended++; if(key) idx[key] = targetRow; }  // append SETELAH last row kolom B
      ...
      var rowVals = line.slice(5, COL_GASPOL_N).map(function(v){ return (v === 0 || v == null || v === '') ? '' : v; });
    ...
    if(lastNow >= dataStart){
      var nRows = lastNow - dataStart + 1, nums = [];
      for(var n=0;n<nRows;n++) nums.push([n+1]);
      sh.getRange(dataStart, 1, nRows, 1).setValues(nums);
    }

    return { ok:true, appended: appended, replaced: replaced, total: rows.length };
  }catch(e){
...
function _verifikasiUserDb_(username){
  ...
  if(!uname) return { ok:false, error:'Username kosong — verifikasi db_Users gagal.' };
  ...
  for(var i=1;i<d.length;i++){
    if(String(d[i][COL_USERS.userName] || '').trim().toLowerCase() === uname){
      return { ok:true, role:String(d[i][COL_USERS.role]||'').trim(), ulp:String(d[i][COL_USERS.ulp]||'').trim() };
    }
  }
  return { ok:false, error:'Username tidak terdaftar di db_Users: ' + username };
}
...
function getMonitoringTlTemuan(params){
  try{
    ...
    var dari = _normTgl(params.tglDari), sampai = _normTgl(params.tglSampai);
    var statusList = (params.statusList instanceof Array) ? params.statusList : [];
    var statusOk = {};
    statusList.forEach(function(s){ statusOk[String(s||'').trim()] = true; });
    ...
    if(!sh || sh.getLastRow() < 2) return { ok:true, ulp:ulpScope, role:ver.role, list:[] };
    ...
    out.sort(function(a,b){
      return String(b.tanggal).localeCompare(String(a.tanggal)) || String(b.kodePekerjaan).localeCompare(String(a.kodePekerjaan));
    });
