...
function _lastRowKodeHeader_(sh) {
  ...
  if (last < 2) return 1;
  var col = sh
    .getRange(2, COL_INS.HEADER.kodeHeader + 1, last - 1, 1)
    .getValues();
...
function migrasiGlobalHeaderBatch() {
  if (MIGRASI_DRY_RUN)
    Logger.log(
      "⚠️ MODE SIMULASI (DRY RUN) AKTIF — tidak ada data yang disalin/dihapus. Set MIGRASI_DRY_RUN = false utk menjalankan sungguhan.",
    );
  var H = COL_INS.HEADER;
  var LEBAR = H.statusTextWa + 1; // 17 kolom (A..Q) — kolom A = formula No, TIDAK disalin
  var shAktif = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(
    SHEET_INS.HEADER,
  );
  var shArsip = SpreadsheetApp.openById(SPREADSHEET_ID_ARSIP).getSheetByName(
    SHEET_INS.HEADER,
  );
  if (!shAktif || !shArsip)
    throw new Error("db_Global_Header tidak ditemukan di salah satu file.");
  ...
  if (shArsip.getLastRow() > 1) {
    var colArsip = shArsip
      .getRange(2, H.kodeHeader + 1, shArsip.getLastRow() - 1, 1)
      .getValues();
  ...
  if (!MIGRASI_DRY_RUN && akanDisalin.length) {
    ...
    shArsip
      .getRange(rowStart, 2, akanDisalin.length, LEBAR - 1)
      .setValues(akanDisalin);
    SpreadsheetApp.flush();
    // Verifikasi: baca balik Kode Header baris pertama yang ditulis
    var cekPertama = String(
      shArsip.getRange(rowStart, H.kodeHeader + 1).getValue() || "",
    ).trim();
    if (cekPertama !== String(akanDisalin[0][H.kodeHeader - 1] || "").trim()) {
      // idx -1 krn kolom A tdk ikut (slice(1))
      throw new Error(
        "Verifikasi append GAGAL — penghapusan dibatalkan untuk batch ini.",
      );
    }
  }
  ...
  if (!MIGRASI_DRY_RUN) {
    akanDihapus.sort(function (a, b) {
      return b - a;
    });
    for (var d = 0; d < akanDihapus.length; d++)
      shAktif.deleteRow(akanDihapus[d]);
    SpreadsheetApp.flush();
  }
  ...
  Logger.log(
    "[migrasiGlobalHeader] batch " +
      ...
      akanDihapus.length +
      " | next=" +
      (done ? "SELESAI" : nextStart) +
      (MIGRASI_DRY_RUN ? " [DRY RUN]" : ""),
  );
  return {
    done: done,
    disalin: akanDisalin.length,
    dihapus: akanDihapus.length,
  };
}
...
function mulaiMigrasiGlobalHeader() {
  ...
  for (var i = 0; i < trs.length; i++)
    if (trs[i].getHandlerFunction() === "migrasiGlobalHeaderTick")
      ScriptApp.deleteTrigger(trs[i]);
  ScriptApp.newTrigger("migrasiGlobalHeaderTick")
    .timeBased()
    .everyMinutes(1)
    .create();
  Logger.log(
    "Migrasi db_Global_Header DIMULAI (tiap 1 menit, batch " +
      MIGRASI_BATCH +
      ").",
  );
}
...
function previewMigrasiGlobalHeader() {
  ...
  if (!shAktif || !shArsip) {
    Logger.log("Sheet db_Global_Header tidak ditemukan di salah satu file.");
    return;
  }
  ...
  Logger.log(
    "================ PREVIEW MIGRASI db_Global_Header ================",
  );
  Logger.log(
    "Batas tanggal (H-" +
      MIGRASI_H_MINUS +
  ...
  Logger.log(
    "  > TETAP di aktif (belum H-" + MIGRASI_H_MINUS + ")         : " + tetap,
  );
  Logger.log("      contoh: " + (sampelTetap.join(", ") || "-"));
  Logger.log("Baris tanpa Kode Header (dilewati)   : " + tanpaKode);
  Logger.log(
    "Baris yang sudah ada di ARSIP        : " +
      Math.max(0, shArsip.getLastRow() - 1),
  );
  Logger.log(
    "==================================================================",
  );
...
function migrasiSatuBarisGlobalHeader(kodeHeader) {
  ...
  if (!shAktif || !shArsip)
    return {
      ok: false,
      message: "Sheet db_Global_Header tidak ditemukan di salah satu file.",
    };
  ...
  if (shArsip.getLastRow() > 1) {
    var colArsip = shArsip
      ...
      .getValues();
    for (var a = 0; a < colArsip.length; a++) {
      if (String(colArsip[a][0] || "").trim() === key) {
        adaDiArsip = true;
        break;
      }
    }
  ...
  if (shAktif.getLastRow() > 1) {
    var colAktif = shAktif
      .getRange(2, H.kodeHeader + 1, shAktif.getLastRow() - 1, 1)
      .getValues();
    for (var i = 0; i < colAktif.length; i++) {
      if (String(colAktif[i][0] || "").trim() === key) {
        rowNum = i + 2;
        break;
      }
    }
  ...
  if (rowNum < 0) {
    Logger.log(
      '[migrasiSatuBarisGlobalHeader] "' +
        key +
        '" TIDAK ditemukan di file AKTIF — tidak ada yang diproses. Pastikan Kode Header persis sama dgn kolom B sheet aktif (contoh format asli: R02-1613021202001).',
    );
    return {
      ok: true,
      mode: "tidak-ada-di-aktif",
      kodeHeader: key,
      sudahDiArsip: adaDiArsip,
    };
  }
  ...
  if (adaDiArsip) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowBaru, H.kodeHeader + 1).getValue() || "",
    ).trim();
    if (cek !== key) {
      throw new Error(
        "Verifikasi append GAGAL utk " +
          key +
          " — baris di aktif TIDAK dihapus.",
      );
    }
    mode = "disalin";
  }
...
function diagnosaMigrasi() {
  ...
  Logger.log(
    "MIGRASI_DRY_RUN = " +
      MIGRASI_DRY_RUN +
      (MIGRASI_DRY_RUN
        ? "   <<<< MASIH MODE SIMULASI! Ubah ke false utk eksekusi sungguhan."
        : "   (mode sungguhan)"),
  );
  Logger.log(
    "Batas tanggal H-" + MIGRASI_H_MINUS + " = " + _migrasiBatasTanggal(),
  );

  // 1) Koneksi file AKTIF
  var shAktif = null;
  try {
    shAktif = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(
      SHEET_INS.HEADER,
    );
    Logger.log(
      "File AKTIF : dibuka OK — sheet db_Global_Header " +
        (shAktif
          ? "ditemukan, baris data = " + (shAktif.getLastRow() - 1)
          : "TIDAK DITEMUKAN (cek nama sheet!)"),
    );
  } catch (e) {
    Logger.log("File AKTIF : GAGAL dibuka — " + e.message);
  }

  // 2) Koneksi file ARSIP
  var shArsip = null;
  try {
    shArsip = SpreadsheetApp.openById(SPREADSHEET_ID_ARSIP).getSheetByName(
      SHEET_INS.HEADER,
    );
    Logger.log(
      "File ARSIP : dibuka OK — sheet db_Global_Header " +
        (shArsip
          ? "ditemukan, baris data = " + (shArsip.getLastRow() - 1)
          : "TIDAK DITEMUKAN (cek nama sheet!)"),
    );
  } catch (e) {
    Logger.log(
      "File ARSIP : GAGAL dibuka — " + e.message + " (cek izin akses file)",
    );
  }

  if (!shAktif || !shArsip) {
    Logger.log("Diagnosa berhenti: perbaiki dulu koneksi/nama sheet di atas.");
    return;
  }

  // 2b) Deteksi baris "HANTU" (formula tanpa isi): getLastRow vs baris terakhir berisi Kode Header
  Logger.log(
    "File AKTIF : getLastRow=" +
      shAktif.getLastRow() +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeHeader_(shAktif),
  );
  Logger.log(
    "File ARSIP : getLastRow=" +
      shArsip.getLastRow() +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeHeader_(shArsip) +
      "   <<< bila > 1 padahal tampak kosong = ada baris hantu; HAPUS BARISNYA (bukan cuma clear isi)",
  );
  ...
  if (n < 1) {
    Logger.log("Sheet aktif KOSONG — tidak ada yang bisa dipindah.");
    return;
  }
...
function _lastRowKodeRowRlz_(sh) {
  ...
  if (last < 2) return 1;
  var col = sh
    .getRange(2, COL_ROW_RLZ.kodePekerjaan + 1, last - 1, 1)
    .getValues();
  for (var i = col.length - 1; i >= 0; i--) {
    if (String(col[i][0] || "").trim() !== "") return i + 2;
  }
...
function migrasiRowRealisasiBatch() {
  ...
  var R = COL_ROW_RLZ,
    LEBAR = COL_ROW_RLZ_N; // 13 kolom (A..M); kolom A = formula No, TIDAK disalin
  var NAMA = "db_ROW_Realisasi";
  var shAktif = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(NAMA);
  var shArsip =
    SpreadsheetApp.openById(SPREADSHEET_ID_ARSIP).getSheetByName(NAMA);
  if (!shAktif || !shArsip)
    throw new Error(NAMA + " tidak ditemukan di salah satu file.");

  // 1) INDEX Kode Pekerjaan yang SUDAH ada di arsip
  var adaDiArsip = {};
  if (shArsip.getLastRow() > 1) {
    var colArsip = shArsip
      .getRange(2, R.kodePekerjaan + 1, shArsip.getLastRow() - 1, 1)
      .getValues();
    for (var a = 0; a < colArsip.length; a++) {
      var ka = String(colArsip[a][0] || "").trim();
      if (ka) adaDiArsip[ka] = true;
  ...
  var lastRow = shAktif.getLastRow();
  if (start > lastRow) {
    props.deleteProperty(MIGRASI_CURSOR_PROP_RLZ);
    return { done: true, disalin: 0, dihapus: 0 };
  }
  var end = Math.min(start + MIGRASI_BATCH - 1, lastRow);
  var data = shAktif.getRange(start, 1, end - start + 1, LEBAR).getValues();
  ...
  if (!MIGRASI_DRY_RUN && akanDisalin.length) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowStart, R.kodePekerjaan + 1).getValue() || "",
    ).trim();
    ...
    if (cek !== String(akanDisalin[0][R.kodePekerjaan - 1] || "").trim()) {
      throw new Error(
        "Verifikasi append GAGAL — penghapusan dibatalkan utk batch ini.",
      );
    }
  }
...
function mulaiMigrasiRowRealisasi() {
  PropertiesService.getScriptProperties().deleteProperty(
    MIGRASI_CURSOR_PROP_RLZ,
  );
  var trs = ScriptApp.getProjectTriggers();
  for (var i = 0; i < trs.length; i++)
    if (trs[i].getHandlerFunction() === "migrasiRowRealisasiTick")
      ScriptApp.deleteTrigger(trs[i]);
  ScriptApp.newTrigger("migrasiRowRealisasiTick")
    .timeBased()
    .everyMinutes(1)
    .create();
  Logger.log(
    "Migrasi db_ROW_Realisasi DIMULAI (tiap 1 menit, batch " +
      MIGRASI_BATCH +
      ").",
  );
}
...
function hentikanMigrasiRowRealisasi() {
  ...
  Logger.log(
    "Migrasi ROW Realisasi dihentikan, " +
      n +
      " trigger dilepas, cursor direset.",
  );
}
...
function migrasiSatuBarisRowRealisasi(kodePekerjaan) {
  ...
  if (!shAktif || !shArsip)
    return {
      ok: false,
      message: "Sheet " + NAMA + " tidak ditemukan di salah satu file.",
    };
  ...
  if (shAktif.getLastRow() > 1) {
    var colAktif = shAktif
      .getRange(2, R.kodePekerjaan + 1, shAktif.getLastRow() - 1, 1)
      .getValues();
    for (var i = 0; i < colAktif.length; i++) {
      if (String(colAktif[i][0] || "").trim() === key) {
        rowNum = i + 2;
  ...
  if (rowNum < 0) {
    Logger.log(
      '[migrasiSatuBarisRowRealisasi] "' +
        key +
        '" TIDAK ditemukan di file AKTIF — tidak ada yang diproses. Ambil nilai persis dari kolom C db_ROW_Realisasi.',
    );
    return {
      ok: true,
      mode: "tidak-ada-di-aktif",
  ...
  if (adaDiArsip) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowBaru, R.kodePekerjaan + 1).getValue() || "",
    ).trim();
    if (cek !== key) {
      throw new Error(
        "Verifikasi append GAGAL utk " +
...
function previewMigrasiRowRealisasi() {
  ...
  var shArsip =
    SpreadsheetApp.openById(SPREADSHEET_ID_ARSIP).getSheetByName(NAMA);
  if (!shAktif || !shArsip) {
    Logger.log("Sheet " + NAMA + " tidak ditemukan di salah satu file.");
    return;
  }
  ...
  Logger.log(
    "================ PREVIEW MIGRASI db_ROW_Realisasi ================",
  );
  Logger.log(
    "Batas tanggal (H-" +
      MIGRASI_H_MINUS +
  ...
  Logger.log(
    "Baris yang sudah ada di ARSIP         : " + Object.keys(adaDiArsip).length,
  );
  Logger.log(
    "Deteksi hantu AKTIF : getLastRow=" +
      lastRow +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeRowRlz_(shAktif),
  );
  Logger.log(
    "Deteksi hantu ARSIP : getLastRow=" +
      shArsip.getLastRow() +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeRowRlz_(shArsip),
  );
  Logger.log(
    "==================================================================",
  );
...
function migrasiRowEksekusiBatch() {
  ...
  var C = COL_ROW,
    LEBAR = COL_ROW_N; // 30 kolom (A..AD); kolom A = formula No, TIDAK disalin
  ...
  if (shArsip.getLastRow() > 1) {
    var colArsip = shArsip
      .getRange(2, C.kodeEksekusi + 1, shArsip.getLastRow() - 1, 1)
      .getValues();
    for (var a = 0; a < colArsip.length; a++) {
      var ka = String(colArsip[a][0] || "").trim();
      if (ka) adaDiArsip[ka] = true;
  ...
  var lastRow = shAktif.getLastRow();
  if (start > lastRow) {
    props.deleteProperty(MIGRASI_CURSOR_PROP_EKS);
    return { done: true, disalin: 0, dihapus: 0 };
  }
  var end = Math.min(start + MIGRASI_BATCH - 1, lastRow);
  var data = shAktif.getRange(start, 1, end - start + 1, LEBAR).getValues();
  ...
  if (!MIGRASI_DRY_RUN && akanDisalin.length) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowStart, C.kodeEksekusi + 1).getValue() || "",
    ).trim();
...
function mulaiMigrasiRowEksekusi() {
  PropertiesService.getScriptProperties().deleteProperty(
    MIGRASI_CURSOR_PROP_EKS,
  );
  var trs = ScriptApp.getProjectTriggers();
  for (var i = 0; i < trs.length; i++)
    if (trs[i].getHandlerFunction() === "migrasiRowEksekusiTick")
      ScriptApp.deleteTrigger(trs[i]);
  ScriptApp.newTrigger("migrasiRowEksekusiTick")
    .timeBased()
    .everyMinutes(1)
    .create();
  Logger.log(
    "Migrasi db_ROW_Eksekusi DIMULAI (tiap 1 menit, batch " +
      MIGRASI_BATCH +
      ").",
  );
}
...
function hentikanMigrasiRowEksekusi() {
  ...
  Logger.log(
    "Migrasi ROW Eksekusi dihentikan, " +
      n +
      " trigger dilepas, cursor direset.",
  );
}
...
function migrasiSatuBarisRowEksekusi(kodeEksekusi) {
  ...
  if (shAktif.getLastRow() > 1) {
    var colAktif = shAktif
      .getRange(2, C.kodeEksekusi + 1, shAktif.getLastRow() - 1, 1)
      .getValues();
    for (var i = 0; i < colAktif.length; i++) {
      if (String(colAktif[i][0] || "").trim() === key) {
        rowNum = i + 2;
  ...
  if (rowNum < 0) {
    Logger.log(
      '[migrasiSatuBarisRowEksekusi] "' +
        key +
        '" TIDAK ditemukan di file AKTIF — tidak ada yang diproses. Ambil nilai persis dari kolom D db_ROW_Eksekusi.',
    );
    return {
      ok: true,
      mode: "tidak-ada-di-aktif",
  ...
  if (adaDiArsip) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowBaru, C.kodeEksekusi + 1).getValue() || "",
    ).trim();
    if (cek !== key) {
      throw new Error(
        "Verifikasi append GAGAL utk " +
...
function previewMigrasiRowEksekusi() {
  ...
  Logger.log(
    "================ PREVIEW MIGRASI db_ROW_Eksekusi ================",
  );
  Logger.log(
    "Batas tanggal (H-" +
      MIGRASI_H_MINUS +
  ...
  Logger.log(
    "Deteksi hantu AKTIF : getLastRow=" +
      lastRow +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeRowEks_(shAktif),
  );
  Logger.log(
    "Deteksi hantu ARSIP : getLastRow=" +
      shArsip.getLastRow() +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeRowEks_(shArsip),
  );
  Logger.log(
    "==================================================================",
  );
...
function _lastRowKodeInsTemuan_(sh) {
  ...
  if (last < 2) return 1;
  var col = sh
    .getRange(2, COL_INS.TEMUAN.kodePekerjaan + 1, last - 1, 1)
    .getValues();
  for (var i = col.length - 1; i >= 0; i--) {
    if (String(col[i][0] || "").trim() !== "") return i + 2;
  }
...
function migrasiInsTemuanBatch() {
  ...
  var T = COL_INS.TEMUAN,
    LEBAR = T.folderPath + 1; // 45 kolom (A..AS); kolom A = formula No, TIDAK disalin
  ...
  if (shArsip.getLastRow() > 1) {
    var colArsip = shArsip
      .getRange(2, T.kodePekerjaan + 1, shArsip.getLastRow() - 1, 1)
      .getValues();
    for (var a = 0; a < colArsip.length; a++) {
      var ka = String(colArsip[a][0] || "").trim();
      if (ka) adaDiArsip[ka] = true;
  ...
  var lastRow = shAktif.getLastRow();
  if (start > lastRow) {
    props.deleteProperty(MIGRASI_CURSOR_PROP_TMN);
    return { done: true, disalin: 0, dihapus: 0 };
  }
  var end = Math.min(start + MIGRASI_BATCH - 1, lastRow);
  var data = shAktif.getRange(start, 1, end - start + 1, LEBAR).getValues();
  ...
  if (!MIGRASI_DRY_RUN && akanDisalin.length) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowStart, T.kodePekerjaan + 1).getValue() || "",
    ).trim();
...
function mulaiMigrasiInsTemuan() {
  PropertiesService.getScriptProperties().deleteProperty(
    MIGRASI_CURSOR_PROP_TMN,
  );
  var trs = ScriptApp.getProjectTriggers();
  for (var i = 0; i < trs.length; i++)
    if (trs[i].getHandlerFunction() === "migrasiInsTemuanTick")
      ScriptApp.deleteTrigger(trs[i]);
  ScriptApp.newTrigger("migrasiInsTemuanTick")
    .timeBased()
    .everyMinutes(1)
    .create();
  Logger.log(
    "Migrasi db_INS_Temuan DIMULAI (tiap 1 menit, batch " +
      MIGRASI_BATCH +
      ").",
  );
}
...
function hentikanMigrasiInsTemuan() {
  ...
  Logger.log(
    "Migrasi INS Temuan dihentikan, " + n + " trigger dilepas, cursor direset.",
  );
}
...
function migrasiSatuBarisInsTemuan(kodePekerjaan) {
  var T = COL_INS.TEMUAN,
    LEBAR = T.folderPath + 1,
    NAMA = SHEET_INS.TEMUAN;
  var key = String(kodePekerjaan || "").trim();
  if (!key) return { ok: false, message: "Kode Pekerjaan wajib diisi." };
  ...
  if (shAktif.getLastRow() > 1) {
    var colAktif = shAktif
      .getRange(2, T.kodePekerjaan + 1, shAktif.getLastRow() - 1, 1)
      .getValues();
    for (var i = 0; i < colAktif.length; i++) {
      if (String(colAktif[i][0] || "").trim() === key) {
        rowNum = i + 2;
  ...
  if (rowNum < 0) {
    Logger.log(
      '[migrasiSatuBarisInsTemuan] "' +
        key +
        '" TIDAK ditemukan di file AKTIF — tidak ada yang diproses. Ambil nilai persis dari kolom D db_INS_Temuan.',
    );
    return {
      ok: true,
      mode: "tidak-ada-di-aktif",
  ...
  if (adaDiArsip) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowBaru, T.kodePekerjaan + 1).getValue() || "",
    ).trim();
    if (cek !== key) {
      throw new Error(
        "Verifikasi append GAGAL utk " +
...
function previewMigrasiInsTemuan() {
  ...
  if (lastRow > 1) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var label =
        kode +
        " (" +
        _normTgl(data[i][T.tanggal]) +
        " | " +
        String(data[i][T.status] || "").trim() +
        ")";
      if (adaDiArsip[kode]) {
        sudahAda++;
        if (sampelSudahAda.length < 5) sampelSudahAda.push(label);
        ...
        tetap++;
        if (String(data[i][T.status] || "").trim() !== STATUS_INS.SELESAI)
          belumSelesai++;
        else fotoKurang++;
        if (sampelTetap.length < 5) sampelTetap.push(label);
      }
  ...
  Logger.log(
    "Kriteria: Status (AA) = Selesai + kolom R/T/AI/AK (4 link foto) terisi — tanggal BEBAS.",
  );
  Logger.log("Total baris ber-Kode Pekerjaan di AKTIF : " + total);
  Logger.log("  > Akan DISALIN ke arsip lalu dihapus : " + pindah);
  Logger.log("      contoh: " + (sampelPindah.join(", ") || "-"));
  ...
  Logger.log("      contoh: " + (sampelSudahAda.join(", ") || "-"));
  Logger.log(
    "  > TETAP di aktif (belum memenuhi kriteria): " +
      tetap +
      "  (belum Selesai: " +
      belumSelesai +
      " | Selesai tapi foto kurang: " +
      fotoKurang +
      ")",
  );
  Logger.log("      contoh: " + (sampelTetap.join(", ") || "-"));
  ...
  Logger.log(
    "Deteksi hantu AKTIF : getLastRow=" +
      lastRow +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeInsTemuan_(shAktif),
  );
  Logger.log(
    "Deteksi hantu ARSIP : getLastRow=" +
      shArsip.getLastRow() +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeInsTemuan_(shArsip),
  );
  Logger.log(
    "==================================================================",
  );
...
function _dalamJendelaMigrasiP0_(d) {
  ...
  var m =
    parseInt(hm.substring(0, 2), 10) * 60 + parseInt(hm.substring(3, 5), 10);
...
function _lolosKriteriaMigrasiYandalP0(row) {
  ...
  var terlantar =
    !String(row[COL_P0.fotoSebelum] || "").trim() &&
...
function migrasiYandalP0Batch(force) {
  ...
  if (!_dalamJendelaMigrasiP0_()) {
    if (force !== true) {
      Logger.log(
        "[migrasiYandalP0] Di luar jendela (23:55-00:10 / 07:50-08:10 / 15:50-16:10 WIB) — batch DILEWATI, cursor tidak bergerak.",
      );
      return { done: false, skipped: "di-luar-jendela" };
    }
    Logger.log(
      "⚠️ FORCE: migrasi dijalankan DI LUAR jendela waktu (mode paksa).",
    );
  }
  var C = COL_P0,
    LEBAR = C.folderPath + 1; // 49 kolom (A..AW); kolom A = formula No, TIDAK disalin
  ...
  if (shArsip.getLastRow() > 1) {
    var colArsip = shArsip
      .getRange(2, C.kodeP0 + 1, shArsip.getLastRow() - 1, 1)
      .getValues();
    for (var a = 0; a < colArsip.length; a++) {
      var ka = String(colArsip[a][0] || "").trim();
      if (ka) adaDiArsip[ka] = true;
  ...
  var lastRow = shAktif.getLastRow();
  if (start > lastRow) {
    props.deleteProperty(MIGRASI_CURSOR_PROP_P0);
    return { done: true, disalin: 0, dihapus: 0 };
  }
  var end = Math.min(start + MIGRASI_BATCH - 1, lastRow);
  var data = shAktif.getRange(start, 1, end - start + 1, LEBAR).getValues();
  ...
  if (!MIGRASI_DRY_RUN && akanDisalin.length) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowStart, C.kodeP0 + 1).getValue() || "",
    ).trim();
...
function mulaiMigrasiYandalP0() {
  PropertiesService.getScriptProperties().deleteProperty(
    MIGRASI_CURSOR_PROP_P0,
  );
  var trs = ScriptApp.getProjectTriggers();
  for (var i = 0; i < trs.length; i++)
    if (trs[i].getHandlerFunction() === "migrasiYandalP0Tick")
      ScriptApp.deleteTrigger(trs[i]);
  ScriptApp.newTrigger("migrasiYandalP0Tick")
    .timeBased()
    .everyMinutes(1)
    .create();
  Logger.log(
    "Migrasi db_Yandal_P0 DIMULAI (tiap 1 menit, batch " +
      MIGRASI_BATCH +
      ") — proses HANYA di jendela 23:55-00:10 / 07:50-08:10 / 15:50-16:10 WIB.",
  );
}
...
function hentikanMigrasiYandalP0() {
  ...
  Logger.log(
    "Migrasi Yandal P0 dihentikan, " + n + " trigger dilepas, cursor direset.",
  );
}
...
function migrasiSatuBarisYandalP0(kodeP0) {
  var C = COL_P0,
    LEBAR = C.folderPath + 1,
    NAMA = SHEET_YANDAL.P0;
  ...
  if (shAktif.getLastRow() > 1) {
    var colAktif = shAktif
      .getRange(2, C.kodeP0 + 1, shAktif.getLastRow() - 1, 1)
      .getValues();
    for (var i = 0; i < colAktif.length; i++) {
      if (String(colAktif[i][0] || "").trim() === key) {
        rowNum = i + 2;
  ...
  if (rowNum < 0) {
    Logger.log(
      '[migrasiSatuBarisYandalP0] "' +
        key +
        '" TIDAK ditemukan di file AKTIF — tidak ada yang diproses. Ambil nilai persis dari kolom D db_Yandal_P0.',
    );
    return {
      ok: true,
      mode: "tidak-ada-di-aktif",
  ...
  if (adaDiArsip) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowBaru, C.kodeP0 + 1).getValue() || "",
    ).trim();
    if (cek !== key) {
      throw new Error(
        "Verifikasi append GAGAL utk " +
...
function previewMigrasiYandalP0() {
  ...
  if (lastRow > 1) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var label =
        kode +
        " (" +
        _normTgl(data[i][C.tanggal]) +
        " | " +
        (String(data[i][C.statusApproval] || "").trim() || "status kosong") +
        ")";
      if (adaDiArsip[kode]) {
        sudahAda++;
        if (sampelSudahAda.length < 5) sampelSudahAda.push(label);
        ...
        if (
          !String(data[i][C.statusApproval] || "").trim() &&
          _normTgl(data[i][C.tanggal]) <= batas
        )
          belumAppr++;
        if (sampelTetap.length < 5) sampelTetap.push(label);
      }
  ...
  Logger.log(
    "Kriteria: Tanggal (kolom G) <= " +
      batas +
      " (H-" +
      MIGRASI_H_MINUS +
      ") DAN (Status Approval (kolom AP) terisi ATAU ketiga foto asli X/AC/AH kosong = P0 terlantar).",
  );
  Logger.log(
    "Guard jendela saat ini: " +
      (_dalamJendelaMigrasiP0_()
        ? "DI DALAM jendela (batch boleh jalan)"
        : "DI LUAR jendela (batch DILEWATI)") +
      " — sekarang " +
  ...
  Logger.log(
    "  > Akan DISALIN ke arsip lalu dihapus : " +
      pindah +
      "  (Status Approval terisi: " +
      (pindah - terlantar) +
      " | terlantar tanpa foto: " +
      terlantar +
      ")",
  );
  Logger.log("      contoh: " + (sampelPindah.join(", ") || "-"));
  ...
  Logger.log(
    "  > TETAP di aktif                     : " +
      ...
      ": " +
      (tetap - belumAppr) +
      " | ada foto tapi Status Approval kosong (Menunggu): " +
      belumAppr +
      ")",
  );
  Logger.log("      contoh: " + (sampelTetap.join(", ") || "-"));
  ...
  Logger.log(
    "Deteksi hantu AKTIF : getLastRow=" +
      lastRow +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeYandalP0_(shAktif),
  );
  Logger.log(
    "Deteksi hantu ARSIP : getLastRow=" +
      shArsip.getLastRow() +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeYandalP0_(shArsip),
  );
  Logger.log(
    "==================================================================",
  );
...
var MIGRASI_SEMUA_DAFTAR = [
  ...
  {
    kunci: "header",
    cursor: "MIGRASI_HEADER_CURSOR",
    batch: function () {
      return migrasiGlobalHeaderBatch();
    },
  },
  {
    kunci: "rlz",
    cursor: "MIGRASI_ROW_RLZ_CURSOR",
    batch: function () {
      return migrasiRowRealisasiBatch();
    },
  },
  {
    kunci: "eks",
    cursor: "MIGRASI_ROW_EKS_CURSOR",
    batch: function () {
      return migrasiRowEksekusiBatch();
    },
  },
  {
    kunci: "tmn",
    cursor: "MIGRASI_TEMUAN_CURSOR",
    batch: function () {
      return migrasiInsTemuanBatch();
    },
  },
  {
    kunci: "p0",
    cursor: "MIGRASI_YANDAL_P0_CURSOR",
    batch: function () {
      return migrasiYandalP0Batch();
    },
  },
  {
    kunci: "lh",
    cursor: "MIGRASI_LAP_HARIAN_CURSOR",
    batch: function () {
      return migrasiLapHarianBatch();
    },
  },
  {
    kunci: "ijr",
    cursor: "MIGRASI_INSJAR_RLZ_CURSOR",
    batch: function () {
      return migrasiInsJarRlzBatch();
    },
  },
  {
    kunci: "idr",
    cursor: "MIGRASI_INSDU_RLZ_CURSOR",
    batch: function () {
      return migrasiInsDuRlzBatch();
    },
  },
  {
    kunci: "hpg",
    cursor: "MIGRASI_HARTEK_PG_CURSOR",
    batch: function () {
      return migrasiHartekPGBatch();
    },
  },
  {
    kunci: "hpkj",
    cursor: "MIGRASI_HARTEK_PKJ_CURSOR",
    batch: function () {
      return migrasiHartekPkjBatch();
    },
  },
  {
    kunci: "hmt",
    cursor: "MIGRASI_HARTEK_MAT_CURSOR",
    batch: function () {
      return migrasiHartekMatBatch();
    },
  },
  {
    kunci: "ysh",
    cursor: "MIGRASI_YANDAL_SHIFT_CURSOR",
    batch: function () {
      return migrasiYandalShiftBatch();
    },
  },
  {
    kunci: "ysw",
    cursor: "MIGRASI_YANDAL_SWC_CURSOR",
    batch: function () {
      return migrasiYandalSwcBatch();
    },
  },
];
...
function mulaiMigrasiSemua() {
  ...
  for (var t = 0; t < trs.length; t++)
    if (trs[t].getHandlerFunction() === "migrasiSemuaTick")
      ScriptApp.deleteTrigger(trs[t]);
  ScriptApp.newTrigger("migrasiSemuaTick").timeBased().everyMinutes(1).create(); // SATU trigger utk semua sheet
  Logger.log(
    "Migrasi SEMUA sheet DIMULAI via 1 trigger gabungan (tiap 1 menit, batch " +
      MIGRASI_BATCH +
      "/sheet).",
  );
}
  ...
  try {
    ...
    for (var i = 0; i < MIGRASI_SEMUA_DAFTAR.length; i++) {
      ...
      if (Date.now() - t0 > 270000) {
        Logger.log(
          "[migrasiSemuaTick] plafon waktu tick tercapai — sisa sheet lanjut menit berikutnya.",
        );
        break;
      }
      try {
        var r = item.batch();
        if (r && r.done) {
          state[item.kunci] = false;
          Logger.log("[migrasiSemuaTick] " + item.kunci + " SELESAI.");
        }
      } catch (eB) {
        Logger.log(
          "[migrasiSemuaTick] batch " +
            item.kunci +
            " ERROR — " +
            eB +
...
function hentikanSemuaMigrasi() {
  var handlers = [
    "migrasiSemuaTick",
    "migrasiGlobalHeaderTick",
    "migrasiRowRealisasiTick",
    "migrasiRowEksekusiTick",
    "migrasiInsTemuanTick",
    "migrasiYandalP0Tick",
    "migrasiLapHarianTick",
    "migrasiInsJarRlzTick",
    "migrasiInsDuRlzTick",
    "migrasiHartekPGTick",
    "migrasiHartekPkjTick",
    "migrasiHartekMatTick",
    "migrasiYandalShiftTick",
    "migrasiYandalSwcTick",
  ];
  var trs = ScriptApp.getProjectTriggers(),
    n = 0;
  for (var i = 0; i < trs.length; i++) {
    if (handlers.indexOf(trs[i].getHandlerFunction()) >= 0) {
      ScriptApp.deleteTrigger(trs[i]);
      n++;
    }
  }
  ...
  Logger.log(
    "Semua trigger migrasi dilepas: " +
      n +
      ". Cursor TIDAK direset (resume aman via mulaiMigrasiSemua / trigger harian).",
  );
  return n;
}
...
function lihatTriggerSaya() {
  ...
  Logger.log(
    "Total trigger terpasang: " +
      trs.length +
      "  (BATAS Apps Script: 20 per script)",
  );
  for (var k in hitung) Logger.log("  " + hitung[k] + "x  " + k);
  if (trs.length >= 18)
    Logger.log(
      "⚠️ Sudah dekat batas — hapus trigger yg tidak dipakai via menu ⏰ (Triggers) di editor.",
    );
...
function migrasiLapHarianBatch() {
  ...
  var NAMA = LH.SHEET,
    LEBAR = 8; // A..H — kolom A (No) nilai STATIS -> ikut disalin
  var shAktif = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(NAMA);
  var shArsip =
    SpreadsheetApp.openById(SPREADSHEET_ID_ARSIP).getSheetByName(NAMA);
  if (!shAktif || !shArsip)
  ...
  var lastRow = shAktif.getLastRow();
  if (start > lastRow) {
    props.deleteProperty(MIGRASI_CURSOR_PROP_LH);
    return { done: true, disalin: 0, dihapus: 0 };
  }
  var end = Math.min(start + MIGRASI_BATCH - 1, lastRow);
  var data = shAktif.getRange(start, 1, end - start + 1, LEBAR).getValues();
  ...
  if (!MIGRASI_DRY_RUN && akanDisalin.length) {
    ...
    shArsip
      .getRange(rowStart, 1, akanDisalin.length, LEBAR)
      .setValues(akanDisalin);
    SpreadsheetApp.flush();
    var cek = _normTgl(
      shArsip.getRange(rowStart, LH.COL.tanggal + 1).getValue(),
    );
    if (cek !== _normTgl(akanDisalin[0][LH.COL.tanggal])) {
      // TANPA geser indeks (kolom A ikut disalin)
      throw new Error(
        "Verifikasi append GAGAL — penghapusan dibatalkan utk batch ini.",
      );
    }
...
function mulaiMigrasiLapHarian() {
  PropertiesService.getScriptProperties().deleteProperty(
    MIGRASI_CURSOR_PROP_LH,
  );
  var trs = ScriptApp.getProjectTriggers();
  for (var i = 0; i < trs.length; i++)
    if (trs[i].getHandlerFunction() === "migrasiLapHarianTick")
      ScriptApp.deleteTrigger(trs[i]);
  ScriptApp.newTrigger("migrasiLapHarianTick")
    .timeBased()
    .everyMinutes(1)
    .create();
  Logger.log(
    "Migrasi Teknik_Laporan Harian DIMULAI (tiap 1 menit, batch " +
      MIGRASI_BATCH +
      ").",
  );
}
...
function hentikanMigrasiLapHarian() {
  ...
  Logger.log(
    "Migrasi Lap Harian dihentikan, " + n + " trigger dilepas, cursor direset.",
  );
}
...
function migrasiSatuBarisLapHarian(tanggal) {
  ...
  if (!key)
    return { ok: false, message: "Tanggal tidak terbaca (pakai yyyy-MM-dd)." };
  ...
  if (shAktif.getLastRow() > 1) {
    var colAktif = shAktif
      .getRange(2, LH.COL.tanggal + 1, shAktif.getLastRow() - 1, 1)
      .getValues();
    for (var i = 0; i < colAktif.length; i++) {
      if (_normTgl(colAktif[i][0]) === key) {
        rowNum = i + 2;
        break;
      }
    }
  ...
  if (!rowNum) {
    Logger.log(
      '[migrasiSatuBarisLapHarian] "' +
        key +
        '" TIDAK ditemukan di file AKTIF — tidak ada yang diproses.',
    );
    return {
      ok: true,
      mode: "tidak-ada-di-aktif",
  ...
  if (adaDiArsip) {
    ...
    SpreadsheetApp.flush();
    var cek = _normTgl(
      shArsip.getRange(rowBaru, LH.COL.tanggal + 1).getValue(),
    );
    if (cek !== key) {
      throw new Error(
        "Verifikasi append GAGAL utk " +
...
function previewMigrasiLapHarian() {
  ...
  Logger.log(
    "  > SUDAH ADA di arsip (hanya dihapus) : " +
      sudahAda +
      "  (contoh: " +
      (sampelSudahAda.join(", ") || "-") +
      ")",
  ...
  Logger.log(
    "Deteksi hantu AKTIF : getLastRow=" +
      lastRow +
      " vs baris terakhir bertanggal = " +
      _lastRowTglLapHarian_(shAktif),
  );
  Logger.log(
    "Deteksi hantu ARSIP : getLastRow=" +
      shArsip.getLastRow() +
      " vs baris terakhir bertanggal = " +
      _lastRowTglLapHarian_(shArsip),
  );
  Logger.log("==============================================================");
}
...
function _lastRowKodeInsJarRlz_(sh) {
  ...
  if (last < 2) return 1;
  var col = sh
    .getRange(2, COL_INS.REALISASI.kodePekerjaanPeny + 1, last - 1, 1)
    .getValues();
  for (var i = col.length - 1; i >= 0; i--) {
    if (String(col[i][0] || "").trim() !== "") return i + 2;
  }
...
function migrasiInsJarRlzBatch() {
  ...
  var R = COL_INS.REALISASI,
    LEBAR = R.timestamp + 1; // 13 kolom (A..M); kolom A = formula No, TIDAK disalin
  ...
  if (shArsip.getLastRow() > 1) {
    var colArsip = shArsip
      .getRange(2, R.kodePekerjaanPeny + 1, shArsip.getLastRow() - 1, 1)
      .getValues();
    for (var a = 0; a < colArsip.length; a++) {
      var ka = String(colArsip[a][0] || "").trim();
      if (ka) adaDiArsip[ka] = true;
  ...
  var lastRow = shAktif.getLastRow();
  if (start > lastRow) {
    props.deleteProperty(MIGRASI_CURSOR_PROP_IJR);
    return { done: true, disalin: 0, dihapus: 0 };
  }
  var end = Math.min(start + MIGRASI_BATCH - 1, lastRow);
  var data = shAktif.getRange(start, 1, end - start + 1, LEBAR).getValues();
  ...
  if (!MIGRASI_DRY_RUN && akanDisalin.length) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowStart, R.kodePekerjaanPeny + 1).getValue() || "",
    ).trim();
...
function mulaiMigrasiInsJarRlz() {
  PropertiesService.getScriptProperties().deleteProperty(
    MIGRASI_CURSOR_PROP_IJR,
  );
  var trs = ScriptApp.getProjectTriggers();
  for (var i = 0; i < trs.length; i++)
    if (trs[i].getHandlerFunction() === "migrasiInsJarRlzTick")
      ScriptApp.deleteTrigger(trs[i]);
  ScriptApp.newTrigger("migrasiInsJarRlzTick")
    .timeBased()
    .everyMinutes(1)
    .create();
  Logger.log(
    "Migrasi db_InsJar_Realisasi DIMULAI (tiap 1 menit, batch " +
      MIGRASI_BATCH +
      ").",
  );
}
...
function hentikanMigrasiInsJarRlz() {
  ...
  Logger.log(
    "Migrasi InsJar Realisasi dihentikan, " +
      n +
      " trigger dilepas, cursor direset.",
  );
}
...
function migrasiSatuBarisInsJarRlz(kodePekerjaanPeny) {
  var R = COL_INS.REALISASI,
    LEBAR = R.timestamp + 1,
    NAMA = SHEET_INS.REALISASI;
  var key = String(kodePekerjaanPeny || "").trim();
  if (!key)
    return { ok: false, message: "Kode Pekerjaan Penyulang wajib diisi." };
  ...
  if (shAktif.getLastRow() > 1) {
    var colAktif = shAktif
      .getRange(2, R.kodePekerjaanPeny + 1, shAktif.getLastRow() - 1, 1)
      .getValues();
    for (var i = 0; i < colAktif.length; i++) {
      if (String(colAktif[i][0] || "").trim() === key) {
        rowNum = i + 2;
  ...
  if (rowNum < 0) {
    Logger.log(
      '[migrasiSatuBarisInsJarRlz] "' +
        key +
        '" TIDAK ditemukan di file AKTIF — tidak ada yang diproses. Ambil nilai persis dari kolom C db_InsJar_Realisasi.',
    );
    return {
      ok: true,
      mode: "tidak-ada-di-aktif",
      kodePekerjaanPeny: key,
      sudahDiArsip: adaDiArsip,
    };
  }
  ...
  if (adaDiArsip) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowBaru, R.kodePekerjaanPeny + 1).getValue() || "",
    ).trim();
    if (cek !== key) {
      throw new Error(
        "Verifikasi append GAGAL utk " +
...
function previewMigrasiInsJarRlz() {
  ...
  if (lastRow > 1) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var label =
        kode +
        " (" +
        _normTgl(data[i][R.tanggal]) +
        " | " +
        (String(data[i][R.penyulang] || "").trim() || "-") +
        ")";
      if (adaDiArsip[kode]) {
        sudahAda++;
        if (sampelSudahAda.length < 5) sampelSudahAda.push(label);
  ...
  Logger.log(
    "================ PREVIEW MIGRASI db_InsJar_Realisasi ================",
  );
  Logger.log(
    "Batas tanggal (H-" +
      MIGRASI_H_MINUS +
  ...
  Logger.log(
    "Baris yang sudah ada di ARSIP                   : " +
      Object.keys(adaDiArsip).length,
  );
  Logger.log(
    "Deteksi hantu AKTIF : getLastRow=" +
      lastRow +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeInsJarRlz_(shAktif),
  );
  Logger.log(
    "Deteksi hantu ARSIP : getLastRow=" +
      shArsip.getLastRow() +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeInsJarRlz_(shArsip),
  );
  Logger.log(
    "==================================================================",
  );
...
function _lastRowKodeInsDuRlz_(sh) {
  ...
  if (last < 2) return 1;
  var col = sh
    .getRange(2, COL_INSDU.REALISASI.kodePekerjaanGardu + 1, last - 1, 1)
    .getValues();
  for (var i = col.length - 1; i >= 0; i--) {
    if (String(col[i][0] || "").trim() !== "") return i + 2;
  }
...
function migrasiInsDuRlzBatch() {
  ...
  var R = COL_INSDU.REALISASI,
    LEBAR = R.timestamp + 1; // 12 kolom (A..L); kolom A = formula No, TIDAK disalin
  ...
  if (shArsip.getLastRow() > 1) {
    var colArsip = shArsip
      .getRange(2, R.kodePekerjaanGardu + 1, shArsip.getLastRow() - 1, 1)
      .getValues();
    for (var a = 0; a < colArsip.length; a++) {
      var ka = String(colArsip[a][0] || "").trim();
      if (ka) adaDiArsip[ka] = true;
  ...
  var lastRow = shAktif.getLastRow();
  if (start > lastRow) {
    props.deleteProperty(MIGRASI_CURSOR_PROP_IDR);
    return { done: true, disalin: 0, dihapus: 0 };
  }
  var end = Math.min(start + MIGRASI_BATCH - 1, lastRow);
  var data = shAktif.getRange(start, 1, end - start + 1, LEBAR).getValues();
  ...
  if (!MIGRASI_DRY_RUN && akanDisalin.length) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowStart, R.kodePekerjaanGardu + 1).getValue() || "",
    ).trim();
...
function mulaiMigrasiInsDuRlz() {
  PropertiesService.getScriptProperties().deleteProperty(
    MIGRASI_CURSOR_PROP_IDR,
  );
  var trs = ScriptApp.getProjectTriggers();
  for (var i = 0; i < trs.length; i++)
    if (trs[i].getHandlerFunction() === "migrasiInsDuRlzTick")
      ScriptApp.deleteTrigger(trs[i]);
  ScriptApp.newTrigger("migrasiInsDuRlzTick")
    .timeBased()
    .everyMinutes(1)
    .create();
  Logger.log(
    "Migrasi db_InsDu_Realisasi DIMULAI (tiap 1 menit, batch " +
      MIGRASI_BATCH +
      ").",
  );
}
...
function hentikanMigrasiInsDuRlz() {
  ...
  Logger.log(
    "Migrasi InsDu Realisasi dihentikan, " +
      n +
      " trigger dilepas, cursor direset.",
  );
}
...
function migrasiSatuBarisInsDuRlz(kodePekerjaanGardu) {
  ...
  if (shAktif.getLastRow() > 1) {
    var colAktif = shAktif
      .getRange(2, R.kodePekerjaanGardu + 1, shAktif.getLastRow() - 1, 1)
      .getValues();
    for (var i = 0; i < colAktif.length; i++) {
      if (String(colAktif[i][0] || "").trim() === key) {
        rowNum = i + 2;
  ...
  if (rowNum < 0) {
    Logger.log(
      '[migrasiSatuBarisInsDuRlz] "' +
        key +
        '" TIDAK ditemukan di file AKTIF — tidak ada yang diproses. Ambil nilai persis dari kolom C db_InsDu_Realisasi.',
    );
    return {
      ok: true,
      mode: "tidak-ada-di-aktif",
      kodePekerjaanGardu: key,
      sudahDiArsip: adaDiArsip,
    };
  }
  ...
  if (adaDiArsip) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowBaru, R.kodePekerjaanGardu + 1).getValue() || "",
    ).trim();
    if (cek !== key) {
      throw new Error(
        "Verifikasi append GAGAL utk " +
...
function previewMigrasiInsDuRlz() {
  ...
  if (lastRow > 1) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var label =
        ...
        _normTgl(data[i][R.tanggal]) +
        " | " +
        String(data[i][R.nomorGardu] || "").trim() +
        ")";
      if (adaDiArsip[kode]) {
        sudahAda++;
        if (sampelSudahAda.length < 5) sampelSudahAda.push(label);
  ...
  Logger.log(
    "================ PREVIEW MIGRASI db_InsDu_Realisasi ================",
  );
  Logger.log(
    "Batas tanggal (H-" +
      MIGRASI_H_MINUS +
  ...
  Logger.log(
    "Deteksi hantu AKTIF : getLastRow=" +
      lastRow +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeInsDuRlz_(shAktif),
  );
  Logger.log(
    "Deteksi hantu ARSIP : getLastRow=" +
      shArsip.getLastRow() +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeInsDuRlz_(shArsip),
  );
  Logger.log(
    "==================================================================",
  );
...
function migrasiHartekPGBatch() {
  ...
  var R = COL_HTK.PG,
    LEBAR = R.timeStamp + 1; // 15 kolom (A..O); kolom A = formula No, TIDAK disalin
  ...
  if (shArsip.getLastRow() > 1) {
    var colArsip = shArsip
      .getRange(2, R.kodePG + 1, shArsip.getLastRow() - 1, 1)
      .getValues();
    for (var a = 0; a < colArsip.length; a++) {
      var ka = String(colArsip[a][0] || "").trim();
      if (ka) adaDiArsip[ka] = true;
  ...
  var lastRow = shAktif.getLastRow();
  if (start > lastRow) {
    props.deleteProperty(MIGRASI_CURSOR_PROP_HPG);
    return { done: true, disalin: 0, dihapus: 0 };
  }
  var end = Math.min(start + MIGRASI_BATCH - 1, lastRow);
  var data = shAktif.getRange(start, 1, end - start + 1, LEBAR).getValues();
  ...
  if (!MIGRASI_DRY_RUN && akanDisalin.length) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowStart, R.kodePG + 1).getValue() || "",
    ).trim();
...
function mulaiMigrasiHartekPG() {
  PropertiesService.getScriptProperties().deleteProperty(
    MIGRASI_CURSOR_PROP_HPG,
  );
  var trs = ScriptApp.getProjectTriggers();
  for (var i = 0; i < trs.length; i++)
    if (trs[i].getHandlerFunction() === "migrasiHartekPGTick")
      ScriptApp.deleteTrigger(trs[i]);
  ScriptApp.newTrigger("migrasiHartekPGTick")
    .timeBased()
    .everyMinutes(1)
    .create();
  Logger.log(
    "Migrasi db_Hartek_PenyulangGardu DIMULAI (tiap 1 menit, batch " +
      MIGRASI_BATCH +
      ").",
  );
}
    ...
    if (r && r.done) {
      ...
      for (var i = 0; i < trs.length; i++)
        if (trs[i].getHandlerFunction() === "migrasiHartekPGTick")
          ScriptApp.deleteTrigger(trs[i]);
      Logger.log(
        "Migrasi db_Hartek_PenyulangGardu SELESAI -> trigger dilepas.",
      );
    }
  } finally {
    try {
...
function hentikanMigrasiHartekPG() {
  ...
  Logger.log(
    "Migrasi Hartek PenyulangGardu dihentikan, " +
      n +
      " trigger dilepas, cursor direset.",
  );
}
...
function migrasiSatuBarisHartekPG(kodePG) {
  ...
  if (shAktif.getLastRow() > 1) {
    var colAktif = shAktif
      .getRange(2, R.kodePG + 1, shAktif.getLastRow() - 1, 1)
      .getValues();
    for (var i = 0; i < colAktif.length; i++) {
      if (String(colAktif[i][0] || "").trim() === key) {
        rowNum = i + 2;
  ...
  if (rowNum < 0) {
    Logger.log(
      '[migrasiSatuBarisHartekPG] "' +
        key +
        '" TIDAK ditemukan di file AKTIF — tidak ada yang diproses. Ambil nilai persis dari kolom C db_Hartek_PenyulangGardu.',
    );
    return {
      ok: true,
      mode: "tidak-ada-di-aktif",
  ...
  if (adaDiArsip) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowBaru, R.kodePG + 1).getValue() || "",
    ).trim();
    if (cek !== key) {
      throw new Error(
        "Verifikasi append GAGAL utk " +
...
function previewMigrasiHartekPG() {
  ...
  Logger.log(
    "================ PREVIEW MIGRASI db_Hartek_PenyulangGardu ================",
  );
  Logger.log(
    "Batas tanggal (H-" +
      MIGRASI_H_MINUS +
  ...
  Logger.log(
    "Deteksi hantu AKTIF : getLastRow=" +
      lastRow +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeHartekPG_(shAktif),
  );
  Logger.log(
    "Deteksi hantu ARSIP : getLastRow=" +
      shArsip.getLastRow() +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeHartekPG_(shArsip),
  );
  Logger.log(
    "==================================================================",
  );
...
function _lastRowKodeHartekPkj_(sh) {
  ...
  if (last < 2) return 1;
  var col = sh
    .getRange(2, COL_HTK.PEKERJAAN.kodePekerjaan + 1, last - 1, 1)
    .getValues();
  for (var i = col.length - 1; i >= 0; i--) {
    if (String(col[i][0] || "").trim() !== "") return i + 2;
  }
...
function migrasiHartekPkjBatch() {
  ...
  var R = COL_HTK.PEKERJAAN,
    LEBAR = R.timestamp + 1; // 17 kolom (A..Q); kolom A = formula No, TIDAK disalin
  ...
  var lastRow = shAktif.getLastRow();
  if (start > lastRow) {
    props.deleteProperty(MIGRASI_CURSOR_PROP_HPKJ);
    return { done: true, disalin: 0, dihapus: 0 };
  }
  var end = Math.min(start + MIGRASI_BATCH - 1, lastRow);
  var data = shAktif.getRange(start, 1, end - start + 1, LEBAR).getValues();
...
function mulaiMigrasiHartekPkj() {
  PropertiesService.getScriptProperties().deleteProperty(
    MIGRASI_CURSOR_PROP_HPKJ,
  );
  var trs = ScriptApp.getProjectTriggers();
  for (var i = 0; i < trs.length; i++)
    if (trs[i].getHandlerFunction() === "migrasiHartekPkjTick")
      ScriptApp.deleteTrigger(trs[i]);
  ScriptApp.newTrigger("migrasiHartekPkjTick")
    .timeBased()
    .everyMinutes(1)
    .create();
  Logger.log(
    "Migrasi db_Hartek_Pekerjaan DIMULAI (tiap 1 menit, batch " +
      MIGRASI_BATCH +
      ").",
  );
}
...
function hentikanMigrasiHartekPkj() {
  ...
  Logger.log(
    "Migrasi Hartek Pekerjaan dihentikan, " +
      n +
      " trigger dilepas, cursor direset.",
  );
}
...
function migrasiSatuBarisHartekPkj(kodePekerjaan) {
  ...
  if (rowNum < 0) {
    Logger.log(
      '[migrasiSatuBarisHartekPkj] "' +
        key +
        '" TIDAK ditemukan di file AKTIF — tidak ada yang diproses. Ambil nilai persis dari kolom D db_Hartek_Pekerjaan.',
    );
    return {
      ok: true,
      mode: "tidak-ada-di-aktif",
...
function previewMigrasiHartekPkj() {
  ...
  if (lastRow > 1) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var label =
        ...
        _normTgl(data[i][R.tanggal]) +
        " | " +
        (String(data[i][R.pekerjaan] || "").trim() || "-") +
        ")";
      if (adaDiArsip[kode]) {
        sudahAda++;
        if (sampelSudahAda.length < 5) sampelSudahAda.push(label);
  ...
  Logger.log(
    "================ PREVIEW MIGRASI db_Hartek_Pekerjaan ================",
  );
  Logger.log(
    "Batas tanggal (H-" +
      MIGRASI_H_MINUS +
  ...
  Logger.log(
    "Deteksi hantu AKTIF : getLastRow=" +
      lastRow +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeHartekPkj_(shAktif),
  );
  Logger.log(
    "Deteksi hantu ARSIP : getLastRow=" +
      shArsip.getLastRow() +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeHartekPkj_(shArsip),
  );
  Logger.log(
    "==================================================================",
  );
...
function _lastRowKodeHartekMat_(sh) {
  ...
  if (last < 2) return 1;
  var col = sh
    .getRange(2, COL_HTK.MATERIAL.kodeMaterial + 1, last - 1, 1)
    .getValues();
  for (var i = col.length - 1; i >= 0; i--) {
    if (String(col[i][0] || "").trim() !== "") return i + 2;
  }
...
function migrasiHartekMatBatch() {
  ...
  var R = COL_HTK.MATERIAL,
    LEBAR = R.timestamp + 1; // 19 kolom (A..S); kolom A = formula No, TIDAK disalin
  ...
  if (shArsip.getLastRow() > 1) {
    var colArsip = shArsip
      .getRange(2, R.kodeMaterial + 1, shArsip.getLastRow() - 1, 1)
      .getValues();
    for (var a = 0; a < colArsip.length; a++) {
      var ka = String(colArsip[a][0] || "").trim();
      if (ka) adaDiArsip[ka] = true;
  ...
  var lastRow = shAktif.getLastRow();
  if (start > lastRow) {
    props.deleteProperty(MIGRASI_CURSOR_PROP_HMT);
    return { done: true, disalin: 0, dihapus: 0 };
  }
  var end = Math.min(start + MIGRASI_BATCH - 1, lastRow);
  var data = shAktif.getRange(start, 1, end - start + 1, LEBAR).getValues();
  ...
  if (!MIGRASI_DRY_RUN && akanDisalin.length) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowStart, R.kodeMaterial + 1).getValue() || "",
    ).trim();
...
function mulaiMigrasiHartekMat() {
  PropertiesService.getScriptProperties().deleteProperty(
    MIGRASI_CURSOR_PROP_HMT,
  );
  var trs = ScriptApp.getProjectTriggers();
  for (var i = 0; i < trs.length; i++)
    if (trs[i].getHandlerFunction() === "migrasiHartekMatTick")
      ScriptApp.deleteTrigger(trs[i]);
  ScriptApp.newTrigger("migrasiHartekMatTick")
    .timeBased()
    .everyMinutes(1)
    .create();
  Logger.log(
    "Migrasi db_Hartek_Material DIMULAI (tiap 1 menit, batch " +
      MIGRASI_BATCH +
      ").",
  );
}
...
function hentikanMigrasiHartekMat() {
  ...
  Logger.log(
    "Migrasi Hartek Material dihentikan, " +
      n +
      " trigger dilepas, cursor direset.",
  );
}
...
function migrasiSatuBarisHartekMat(kodeMaterial) {
  ...
  if (shAktif.getLastRow() > 1) {
    var colAktif = shAktif
      .getRange(2, R.kodeMaterial + 1, shAktif.getLastRow() - 1, 1)
      .getValues();
    for (var i = 0; i < colAktif.length; i++) {
      if (String(colAktif[i][0] || "").trim() === key) {
        rowNum = i + 2;
  ...
  if (rowNum < 0) {
    Logger.log(
      '[migrasiSatuBarisHartekMat] "' +
        key +
        '" TIDAK ditemukan di file AKTIF — tidak ada yang diproses. Ambil nilai persis dari kolom E db_Hartek_Material.',
    );
    return {
      ok: true,
      mode: "tidak-ada-di-aktif",
  ...
  if (adaDiArsip) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowBaru, R.kodeMaterial + 1).getValue() || "",
    ).trim();
    if (cek !== key) {
      throw new Error(
        "Verifikasi append GAGAL utk " +
...
function previewMigrasiHartekMat() {
  ...
  Logger.log(
    "================ PREVIEW MIGRASI db_Hartek_Material ================",
  );
  Logger.log(
    "Batas tanggal (H-" +
      MIGRASI_H_MINUS +
  ...
  Logger.log(
    "Deteksi hantu AKTIF : getLastRow=" +
      lastRow +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeHartekMat_(shAktif),
  );
  Logger.log(
    "Deteksi hantu ARSIP : getLastRow=" +
      shArsip.getLastRow() +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeHartekMat_(shArsip),
  );
  Logger.log(
    "==================================================================",
  );
...
function _lastRowKodeYandalShift_(sh) {
  ...
  if (last < 2) return 1;
  var col = sh
    .getRange(2, COL_YANDAL_SHIFT.kodeShift + 1, last - 1, 1)
    .getValues();
  for (var i = col.length - 1; i >= 0; i--) {
    if (String(col[i][0] || "").trim() !== "") return i + 2;
  }
...
function migrasiYandalShiftBatch(force) {
  ...
  if (!_dalamJendelaMigrasiP0_()) {
    if (force !== true) {
      Logger.log(
        "[migrasiYandalShift] Di luar jendela (23:55-00:10 / 07:50-08:10 / 15:50-16:10 WIB) — batch DILEWATI, cursor tidak bergerak.",
      );
      return { done: false, skipped: "di-luar-jendela" };
    }
    Logger.log(
  ...
  if (shArsip.getLastRow() > 1) {
    var colArsip = shArsip
      .getRange(2, R.kodeShift + 1, shArsip.getLastRow() - 1, 1)
      .getValues();
    for (var a = 0; a < colArsip.length; a++) {
      var ka = String(colArsip[a][0] || "").trim();
      if (ka) adaDiArsip[ka] = true;
  ...
  var lastRow = shAktif.getLastRow();
  if (start > lastRow) {
    props.deleteProperty(MIGRASI_CURSOR_PROP_YSH);
    return { done: true, disalin: 0, dihapus: 0 };
  }
  var end = Math.min(start + MIGRASI_BATCH - 1, lastRow);
  var data = shAktif.getRange(start, 1, end - start + 1, LEBAR).getValues();
  ...
  if (!MIGRASI_DRY_RUN && akanDisalin.length) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowStart, R.kodeShift + 1).getValue() || "",
    ).trim();
...
function mulaiMigrasiYandalShift() {
  PropertiesService.getScriptProperties().deleteProperty(
    MIGRASI_CURSOR_PROP_YSH,
  );
  var trs = ScriptApp.getProjectTriggers();
  for (var i = 0; i < trs.length; i++)
    if (trs[i].getHandlerFunction() === "migrasiYandalShiftTick")
      ScriptApp.deleteTrigger(trs[i]);
  ScriptApp.newTrigger("migrasiYandalShiftTick")
    .timeBased()
    .everyMinutes(1)
    .create();
  Logger.log(
    "Migrasi db_Yandal_Shift DIMULAI (tiap 1 menit, batch " +
      MIGRASI_BATCH +
      ") — proses HANYA di jendela 23:55-00:10 / 07:50-08:10 / 15:50-16:10 WIB.",
  );
}
...
function hentikanMigrasiYandalShift() {
  ...
  Logger.log(
    "Migrasi Yandal Shift dihentikan, " +
      n +
      " trigger dilepas, cursor direset.",
  );
}
...
function migrasiSatuBarisYandalShift(kodeShift) {
  ...
  if (shAktif.getLastRow() > 1) {
    var colAktif = shAktif
      .getRange(2, R.kodeShift + 1, shAktif.getLastRow() - 1, 1)
      .getValues();
    for (var i = 0; i < colAktif.length; i++) {
      if (String(colAktif[i][0] || "").trim() === key) {
        rowNum = i + 2;
  ...
  if (rowNum < 0) {
    Logger.log(
      '[migrasiSatuBarisYandalShift] "' +
        key +
        '" TIDAK ditemukan di file AKTIF — tidak ada yang diproses. Ambil nilai persis dari kolom C db_Yandal_Shift.',
    );
    return {
      ok: true,
      mode: "tidak-ada-di-aktif",
  ...
  if (adaDiArsip) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowBaru, R.kodeShift + 1).getValue() || "",
    ).trim();
    if (cek !== key) {
      throw new Error(
        "Verifikasi append GAGAL utk " +
...
function previewMigrasiYandalShift() {
  ...
  if (lastRow > 1) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var label =
        ...
        _normTgl(data[i][R.tanggal]) +
        " | " +
        (String(data[i][R.tim] || "").trim() || "-") +
        " / " +
        (String(data[i][R.shift] || "").trim() || "-") +
        ")";
      if (adaDiArsip[kode]) {
        sudahAda++;
        if (sampelSudahAda.length < 5) sampelSudahAda.push(label);
  ...
  Logger.log(
    "================ PREVIEW MIGRASI db_Yandal_Shift ================",
  );
  Logger.log(
    "Batas tanggal (H-" +
      MIGRASI_H_MINUS +
      ...
      " — baris dgn Tanggal (kolom E) <= " +
      batas +
      " akan dipindah (murni tanggal, tanpa guard status/foto).",
  );
  Logger.log(
    "Guard jendela saat ini: " +
      (_dalamJendelaMigrasiP0_()
  ...
  Logger.log(
    "Deteksi hantu AKTIF : getLastRow=" +
      lastRow +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeYandalShift_(shAktif),
  );
  Logger.log(
    "Deteksi hantu ARSIP : getLastRow=" +
      shArsip.getLastRow() +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeYandalShift_(shArsip),
  );
  Logger.log(
    "==================================================================",
  );
...
function _lastRowKodeYandalSwc_(sh) {
  ...
  if (last < 2) return 1;
  var col = sh
    .getRange(2, COL_SWITCHING.kodeSwitching + 1, last - 1, 1)
    .getValues();
  for (var i = col.length - 1; i >= 0; i--) {
    if (String(col[i][0] || "").trim() !== "") return i + 2;
  }
...
function migrasiYandalSwcBatch(force) {
  ...
  if (!_dalamJendelaMigrasiP0_()) {
    if (force !== true) {
      Logger.log(
        "[migrasiYandalSwc] Di luar jendela (23:55-00:10 / 07:50-08:10 / 15:50-16:10 WIB) — batch DILEWATI, cursor tidak bergerak.",
      );
      return { done: false, skipped: "di-luar-jendela" };
    }
    Logger.log(
  ...
  var R = COL_SWITCHING,
    LEBAR = R.folderPath + 1; // 50 kolom (A..AX); kolom A = formula No, TIDAK disalin
  ...
  if (shArsip.getLastRow() > 1) {
    var colArsip = shArsip
      .getRange(2, R.kodeSwitching + 1, shArsip.getLastRow() - 1, 1)
      .getValues();
    for (var a = 0; a < colArsip.length; a++) {
      var ka = String(colArsip[a][0] || "").trim();
      if (ka) adaDiArsip[ka] = true;
  ...
  var p0DiArsip = {};
  var shArsipP0 = SpreadsheetApp.openById(SPREADSHEET_ID_ARSIP).getSheetByName(
    SHEET_YANDAL.P0,
  );
  if (shArsipP0 && shArsipP0.getLastRow() > 1) {
    var colP0 = shArsipP0
      .getRange(2, COL_P0.kodeP0 + 1, shArsipP0.getLastRow() - 1, 1)
      .getValues();
  ...
  var lastRow = shAktif.getLastRow();
  if (start > lastRow) {
    props.deleteProperty(MIGRASI_CURSOR_PROP_YSW);
    return { done: true, disalin: 0, dihapus: 0 };
  }
  var end = Math.min(start + MIGRASI_BATCH - 1, lastRow);
  var data = shAktif.getRange(start, 1, end - start + 1, LEBAR).getValues();
  ...
  if (!MIGRASI_DRY_RUN && akanDisalin.length) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowStart, R.kodeSwitching + 1).getValue() || "",
    ).trim();
...
function mulaiMigrasiYandalSwc() {
  PropertiesService.getScriptProperties().deleteProperty(
    MIGRASI_CURSOR_PROP_YSW,
  );
  var trs = ScriptApp.getProjectTriggers();
  for (var i = 0; i < trs.length; i++)
    if (trs[i].getHandlerFunction() === "migrasiYandalSwcTick")
      ScriptApp.deleteTrigger(trs[i]);
  ScriptApp.newTrigger("migrasiYandalSwcTick")
    .timeBased()
    .everyMinutes(1)
    .create();
  Logger.log(
    "Migrasi db_Yandal_Pengecekan_Switching DIMULAI (tiap 1 menit, batch " +
      MIGRASI_BATCH +
      ") — proses HANYA di jendela 23:55-00:10 / 07:50-08:10 / 15:50-16:10 WIB.",
  );
}
    ...
    if (r && r.done) {
      ...
      for (var i = 0; i < trs.length; i++)
        if (trs[i].getHandlerFunction() === "migrasiYandalSwcTick")
          ScriptApp.deleteTrigger(trs[i]);
      Logger.log(
        "Migrasi db_Yandal_Pengecekan_Switching SELESAI -> trigger dilepas.",
      );
    }
  } finally {
    try {
...
function hentikanMigrasiYandalSwc() {
  ...
  Logger.log(
    "Migrasi Yandal Switching dihentikan, " +
      n +
      " trigger dilepas, cursor direset.",
  );
}
...
function migrasiSatuBarisYandalSwc(kodeSwitching) {
  var R = COL_SWITCHING,
    LEBAR = R.folderPath + 1,
    NAMA = SHEET_YANDAL.SWITCHING;
  ...
  if (shAktif.getLastRow() > 1) {
    var colAktif = shAktif
      .getRange(2, R.kodeSwitching + 1, shAktif.getLastRow() - 1, 1)
      .getValues();
    for (var i = 0; i < colAktif.length; i++) {
      if (String(colAktif[i][0] || "").trim() === key) {
        rowNum = i + 2;
  ...
  if (rowNum < 0) {
    Logger.log(
      '[migrasiSatuBarisYandalSwc] "' +
        key +
        '" TIDAK ditemukan di file AKTIF — tidak ada yang diproses. Ambil nilai persis dari kolom E db_Yandal_Pengecekan_Switching.',
    );
    return {
      ok: true,
      mode: "tidak-ada-di-aktif",
  ...
  if (adaDiArsip) {
    ...
    SpreadsheetApp.flush();
    var cek = String(
      shArsip.getRange(rowBaru, R.kodeSwitching + 1).getValue() || "",
    ).trim();
    if (cek !== key) {
      throw new Error(
        "Verifikasi append GAGAL utk " +
...
function previewMigrasiYandalSwc() {
  ...
  if (lastRow > 1) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var label =
        ...
        _normTgl(data[i][R.tanggal]) +
        " | " +
        (String(data[i][R.namaSwitching] || "").trim() || "-") +
        ")";
      if (adaDiArsip[kode]) {
        sudahAda++;
        if (sampelSudahAda.length < 5) sampelSudahAda.push(label);
        ...
        if (_normTgl(data[i][R.tanggal]) <= batas && !(kp0 && p0DiArsip[kp0]))
          indukBelum++;
        if (sampelTetap.length < 5) sampelTetap.push(label);
      }
    }
  ...
  Logger.log(
    "================ PREVIEW MIGRASI db_Yandal_Pengecekan_Switching ================",
  );
  Logger.log(
    "Batas tanggal (H-" +
      MIGRASI_H_MINUS +
      ...
      " — baris dgn Tanggal (kolom H) <= " +
      batas +
      " DAN Kode P0 induk (kolom D) sudah ada di arsip db_Yandal_P0 yang akan dipindah.",
  );
  Logger.log(
    "Guard jendela saat ini: " +
      (_dalamJendelaMigrasiP0_()
  ...
  Logger.log(
    "  > TETAP di aktif                     : " +
      tetap +
      "  (lolos tanggal tapi P0 induk belum pindah: " +
      indukBelum +
      " — otomatis menyusul putaran berikutnya)",
  );
  Logger.log("      contoh: " + (sampelTetap.join(", ") || "-"));
  ...
  Logger.log(
    "Deteksi hantu AKTIF : getLastRow=" +
      lastRow +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeYandalSwc_(shAktif),
  );
  Logger.log(
    "Deteksi hantu ARSIP : getLastRow=" +
      shArsip.getLastRow() +
      " vs baris terakhir berisi Kode = " +
      _lastRowKodeYandalSwc_(shArsip),
  );
  Logger.log(
    "==================================================================",
  );
...
function tickGabunganSiSi() {
  ...
  try {
    var last = {};
    try {
      last = JSON.parse(props.getProperty(TICK_GABUNGAN_PROP) || "{}");
    } catch (eJ) {
      last = {};
    }
    var now = Date.now(),
    ...
    for (var i = 0; i < TICK_GABUNGAN_DAFTAR.length; i++) {
      ...
      if (t.berat ? elapsed > 150000 : elapsed > 240000) {
        Logger.log(
          "[tickGabungan] plafon waktu — sisa tugas lanjut menit berikutnya.",
        );
        break;
      }
      var f = this[t.fn];
      if (typeof f !== "function") {
        Logger.log(
          "[tickGabungan] " +
            t.fn +
            " tidak ada (modul belum terpasang?) — diloncati.",
        );
        last[t.fn] = now;
        continue;
      }
      try {
        ...
        Logger.log("[tickGabungan] " + t.fn + " ERROR — " + eF);
      }
      last[t.fn] = Date.now();
    }
    ...
    try {
      props.deleteProperty(TICK_GABUNGAN_MUTEX_PROP);
    } catch (eM) {}
  }
}
...
function harianGabunganSiSi() {
  ...
  for (var i = 0; i < urut.length; i++) {
    var f = this[urut[i]];
    if (typeof f !== "function") {
      Logger.log("[harianGabungan] " + urut[i] + " tidak ada — diloncati.");
      continue;
    }
    try {
      f.call(this);
      Logger.log("[harianGabungan] " + urut[i] + " OK");
    } catch (eF) {
      Logger.log("[harianGabungan] " + urut[i] + " ERROR — " + eF);
    }
    if (Date.now() - t0 > 300000) {
      Logger.log(
        "[harianGabungan] plafon 5 mnt — sisa tugas (" +
          urut.slice(i + 1).join(", ") +
...
function lepasTriggerGabunganSiSi() {
  ...
  for (var i = 0; i < trs.length; i++) {
    var f = trs[i].getHandlerFunction();
    if (f === "tickGabunganSiSi" || f === "harianGabunganSiSi") {
      ScriptApp.deleteTrigger(trs[i]);
      n++;
    }
  }
  ...
  Logger.log(
    "Trigger gabungan dilepas: " +
      n +
      ". Pasang ulang trigger per modul via fungsi create*Trigger masing-masing.",
  );
  return n;
}
