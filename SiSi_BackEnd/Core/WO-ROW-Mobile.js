...
function _woRowCariEks_(sh, kodeWo) {
  ...
  var vals = sh
    .getRange(2, WO_ROW_KODE_WO_COL, sh.getLastRow() - 1, 1)
    .getValues();
...
function _woRowUpload_(kodeWo, tag, b64, mime) {
  ...
  var folder = _getOrCreateFolderByPath(
    _folderRowEksekusiPath(tanggal, kodeWo),
  );
  return _uploadFotoTemuan(
    b64,
    mime || "image/jpeg",
    kodeWo + "_" + tag,
    folder,
...
function _woRowLengkap_(r, T) {
  ...
  for (var i = 0; i < idx.length; i++)
    if (!_woRowNorm_(r[idx[i]])) return false;
  return true;
}
function _woRowMulai_(token, payload) {
  var sesi = _woRowSesi_(token),
    kode = _woRowNorm_(payload.kodePekerjaan);
  if (!kode) return { success: false, message: "Kode WO wajib diisi." };
  var loc = _findRowTemuan(kode);
  if (!loc) return { success: false, message: "WO tidak ditemukan." };
  var T = COL_INS.TEMUAN;
  var wo = loc.sheet.getRange(loc.row, 1, 1, T.folderPath + 1).getValues()[0];
  var status = _woRowNorm_(wo[T.status]),
    tim = _woRowNorm_(wo[T.timEksekusi]);
  if (status !== STATUS_INS.PROGRESS)
    return {
      success: false,
      message: "WO tidak berstatus Progress Pekerjaan.",
    };
  ...
  if (!sh)
    return { success: false, message: "db_ROW_Eksekusi tidak ditemukan." };
  _woRowEnsureSchema_(sh);
  var existing = _woRowCariEks_(sh, kode);
  if (existing) {
    var oldKode = _woRowNorm_(
      sh.getRange(existing.row, COL_ROW.kodeEksekusi + 1).getValue(),
    );
    return {
      success: true,
      started: true,
      kodeEksekusi: oldKode,
  ...
  row[COL_ROW.latPekerjaan] = wo[T.lat];
  row[COL_ROW.longPekerjaan] = wo[T.long];
  ...
  sh.getRange(target, COL_ROW.timestamp + 1).setNumberFormat(
    "dd/MM/yyyy HH:mm:ss",
  );
...
function _woRowUpdate_(token, payload) {
  var sesi = _woRowSesi_(token),
    kode = _woRowNorm_(payload.kodePekerjaan),
    tahap = _woRowNorm_(payload.tahap).toLowerCase();
  if (!kode) return { success: false, message: "Kode WO wajib diisi." };
  if (tahap !== "pekerjaan" && tahap !== "sesudah")
    return { success: false, message: "Tahap foto tidak valid." };
  var loc = _findRowTemuan(kode);
  if (!loc) return { success: false, message: "WO tidak ditemukan." };
  var T = COL_INS.TEMUAN,
  ...
  if (!eks) {
    var mulai = _woRowMulai_(token, payload);
    if (!mulai.success) return mulai;
    eks = _woRowCariEks_(sh, kode);
  }
  var foto = _woRowUpload_(
    kode,
    tahap === "pekerjaan" ? "Foto Pekerjaan" : "Foto Sesudah",
    payload.fotoBase64,
    payload.fotoMime,
  );
  if (!foto.url)
    return {
      success: false,
      message: "Upload foto gagal, status WO tidak diubah.",
    };
  if (tahap === "pekerjaan") {
  ...
  SpreadsheetApp.flush();
  wo = loc.sheet.getRange(loc.row, 1, 1, T.folderPath + 1).getValues()[0];
  var lengkap = _woRowLengkap_(wo, T),
    queued = false,
    kodeEks = _woRowNorm_(
      sh.getRange(eks.row, COL_ROW.kodeEksekusi + 1).getValue(),
    );
  if (lengkap) {
    var now = new Date(),
      tgl = _normTgl(now),
    ...
    loc.sheet
      .getRange(loc.row, T.petugas + 1)
      .setValue(_woRowNorm_(sesi.username));
    loc.sheet
      .getRange(loc.row, T.inputBySelesai + 1)
      .setValue(_woRowNorm_(sesi.username));
    ...
    sh.getRange(eks.row, COL_ROW.inputOleh + 1).setValue(
      _woRowNorm_(sesi.username),
    );
    ...
    queued = _enqueueRecalc_({
      jenis: "eksekusiRow",
      key: "eksekusiRow|" + kodeEks,
      tim: tim,
      tanggal: tgl,
    });
  }
  return {
    ...
    completed: lengkap,
    queued: queued,
    kodeEksekusi: kodeEks,
    fotoPekerjaanUrl: tahap === "pekerjaan" ? foto.url : "",
    fotoSesudahUrl: tahap === "sesudah" ? foto.url : "",
    message: lengkap
      ? "WO selesai dan masuk antrean recalc laporan/WA."
      : "Foto tersimpan, WO masih Progress Pekerjaan.",
  };
}
function woRowMobile_(token, payload) {
  try {
    payload = payload || {};
    var cmd = _woRowNorm_(payload.cmd),
      out;
