/* ============================================================================
   ...
   ---------------------------------------------------------------------------
   Prinsip:
   - Sinkron pertama: client belum punya version -> download semua dataset.
   - Sinkron berikutnya: client kirim version lokal; hanya dataset berubah yang
     diunduh. Manifest di-cache 10 menit untuk mencegah scan berulang.
   - Dataset utama aktif+arsip dibaca lewat _readSheetDual_ dan didedup.
   - Master pendukung hanya dibaca dari sumber aktif.
   - db_Users disanitasi: password tidak pernah dikirim ke perangkat.
   - Payload dipaginasi 500 baris agar respons Apps Script tetap stabil.
   ============================================================================ */
...
function _deltaRows_(token,name,cfg){
  ...
  for(var i=0;i<rows.length;i++){
    var r=rows[i].map(_deltaPlain_);
    if(cfg.sanitize==='users' && r.length>3) r[3]=''; // password
    out.push(r);
  }
  return out;
}
...
function _deltaManifest_(token,force){
  ...
  Object.keys(cfgs).forEach(function(name){
    ...
    datasets.push({name:name,version:_deltaDigest_(rows),count:rows.length,
...
function _deltaFetch_(token,name,offset,limit){
  ...
  return{success:true,apiVersion:1,name:name,version:_deltaDigest_(rows),total:rows.length,
    offset:from,rows:rows.slice(from,from+take),hasMore:from+take<rows.length,
    kind:cfg.support?'support':'main'};
}
