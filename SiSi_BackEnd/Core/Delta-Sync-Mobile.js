...
function _deltaConfigs_() {
  return {
    ...
    db_Hartek_PenyulangGardu: {
      sheet: "db_Hartek_PenyulangGardu",
      key: 2,
      width: 15,
      dual: true,
      main: true,
    ...
    db_Yandal_Pengecekan_Switching: {
      sheet: "db_Yandal_Pengecekan_Switching",
      key: 4,
      width: 50,
      dual: true,
      main: true,
    ...
    db_List_Temuan: {
      sheet: "db_List_Temuan",
      key: 3,
      width: 0,
      support: true,
    },
    db_Hartek_List_Pekerjaan: {
      sheet: "db_Hartek_List_Pekerjaan",
      key: 1,
      width: 0,
      support: true,
    },
...
function _deltaYandalUkurRows_() {
  ...
  if (sh.getLastColumn() < 24)
    throw new Error(
      "Struktur Pengukuran Gardu tidak valid: dibutuhkan 24 kolom.",
    );
  if (sh.getLastRow() < 2) return [];
  return sh
    .getRange(2, 1, sh.getLastRow() - 1, 24)
    .getValues()
    .map(function (row) {
...
function _deltaYandalPetugasRows_() {
  ...
  for (var c = 0; c < headers.length; c++) {
    var h = String(headers[c] || "")
      .trim()
      .toLowerCase();
    if (h.indexOf("petugas") >= 0 || h === "nama" || h === "nama petugas")
      nameCols.push(c);
  }
  if (!nameCols.length) {
    for (var fallback = 1; fallback < width; fallback++)
      nameCols.push(fallback);
  }
  ...
  for (var r = 0; r < data.length; r++) {
    for (var n = 0; n < nameCols.length; n++) {
      ...
      for (var q = 0; q < parts.length; q++) {
        var person = String(parts[q] || "").trim(),
...
function _deltaRows_(token, name, cfg) {
  if (cfg.special === "gardu") {
    ...
    if (!r || r.success !== true)
      throw new Error((r && r.message) || "Master Gardu gagal.");
  ...
  var rows = [];
  if (cfg.dual && typeof _readSheetDual_ === "function")
    rows = _readSheetDual_(cfg.sheet, cfg.key, cfg.width);
  else {
...
function _deltaManifest_(token, force) {
  ...
  var scope = String(sesi.kodeUlp || sesi.ulp || "all").replace(
    /[^a-zA-Z0-9_-]/g,
    "_",
  );
  var cache = CacheService.getScriptCache(),
    ck = "delta_manifest_v1_" + scope;
  if (!force) {
  ...
  try {
    cache.put(ck, JSON.stringify(result), DELTA_SYNC_CACHE_SEC);
  } catch (_) {}
  return result;
}
function _deltaFetch_(token, name, offset, limit) {
  ...
  var take = Math.max(
    1,
    Math.min(DELTA_SYNC_PAGE, Number(limit) || DELTA_SYNC_PAGE),
  );
  return {
    success: true,
    apiVersion: 1,
