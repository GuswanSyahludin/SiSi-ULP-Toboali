...
function _baFindPenyulangHeader_(values) {
  ...
  for (var r = 0; r < limit; r++) {
    ...
    if (penyulang >= 0 && section >= 0)
      return { row: r, penyulang: penyulang, section: section };
  }
  return null;
}
...
function getPenyulangDanSection() {
  try {
    ...
    if (!sh)
      return { ok: false, message: "Sheet db_Penyulang tidak ditemukan." };
    var values = sh.getDataRange().getDisplayValues();
    if (!values.length)
      return { ok: true, penyulangList: [], sectionByPenyulang: {} };
    var found = _baFindPenyulangHeader_(values);
    if (!found)
      return {
        ok: false,
        message:
          "Header db_Penyulang belum dikenali (butuh kolom Nama Penyulang dan Section).",
      };
    var order = [],
      nameByKey = {},
      sectionsByKey = {},
    ...
    for (var r = found.row + 1; r < values.length; r++) {
      ...
      if (section) {
        ...
        if (!sectionSeen[pKey][sKey]) {
          sectionSeen[pKey][sKey] = true;
          sectionsByKey[pKey].push(section);
        }
      }
    }
    var penyulangList = [],
      sectionByPenyulang = {};
    order.forEach(function (pKey) {
      penyulangList.push(nameByKey[pKey]);
      sectionByPenyulang[nameByKey[pKey]] = sectionsByKey[pKey];
    });
    return {
      ok: true,
    ...
    return {
      ok: false,
      message: "Gagal memuat db_Penyulang: " + error.message,
    };
  }
}
...
function _baText_(row, index) {
  return index >= 0 && index < row.length
    ? String(row[index] == null ? "" : row[index]).trim()
    : "";
}
...
function _baDate_(value) {
  ...
  return isNaN(date.getTime())
    ? ""
    : Utilities.formatDate(date, "Asia/Jakarta", "yyyy-MM-dd");
}
...
function _baFindHeader_(values) {
  ...
  for (var r = 0; r < limit; r++) {
    ...
    if (!best || score > best.score)
      best = { score: score, row: r, map: map, headers: values[r] };
  }
  ...
  for (var r2 = 0; r2 < limit; r2++) {
    ...
    if (_baPickIndex_(map2, baAliases) >= 0)
      return { row: r2, map: map2, headers: values[r2] };
  }
  // Fallback 2: kandidat skor terbaik (minimal 2 kolom inti cocok).
  if (best && best.score >= 4)
    return { row: best.row, map: best.map, headers: best.headers };
  return null;
}
/* =========================================================
...
function getOwnerIdDanExternalReference(filter) {
  try {
    ...
    var values = sh.getDataRange().getDisplayValues();
    if (!values.length)
      return _baEmptyOwnerReference_("Sheet OwnerId belum berisi data.");
    ...
    for (var r1 = 0; r1 < values.length; r1++) {
      if (_baKey_(_baText_(values[r1], COL_I)) === pKey) {
        hVal = _baText_(values[r1], COL_H);
        break;
      }
    }
    ...
    if (hVal) {
      ...
      for (var r2 = 0; r2 < values.length; r2++) {
        if (_baKey_(_baText_(values[r2], COL_A)) === hKey) {
          idGI = _baText_(values[r2], COL_B);
          break;
        }
      }
    }
    ...
    if (secKeyId) {
      for (var rs = 0; rs < values.length; rs++) {
        ...
        if (
          !barisSecContains &&
          (jKeyId.indexOf(secKeyId) >= 0 || secKeyId.indexOf(jKeyId) >= 0)
        )
          barisSecContains = values[rs];
      }
    }
    var barisSecFinal = barisSec || barisSecContains;
    if (barisSecFinal) {
      idSection = _baText_(barisSecFinal, COL_K);
    }
    ...
    if (!idGI || !idULP || !idFeeder || !idSection) {
      ...
      return _baEmptyOwnerReference_(
        'Komponen OwnerId belum lengkap untuk penyulang "' +
          penyulang +
          '": ' +
          kurang.join(", ") +
          ".",
    ...
    var ownerId =
      _pad2_(idGI) + _pad2_(idULP) + _pad2_(idFeeder) + _pad2_(idSection);
    ...
    var token =
      sUp.indexOf("GI") === 0 ? "PMT" : sUp.indexOf("DS") === 0 ? "DSC" : "";
    var jenisPemutus = "",
      namaPemutus = "";
    var barisSection = null,
      barisToken = null,
      barisTokenSaja = null;
    if (sKey) {
      for (var rj = 0; rj < values.length; rj++) {
        ...
        var cocokSection =
          jKey === sKey || jKey.indexOf(sKey) >= 0 || sKey.indexOf(jKey) >= 0;
        if (cocokSection) {
          if (!barisSection) barisSection = values[rj];
          if (punyaToken) {
            barisToken = values[rj];
            break;
          }
        }
    ...
    var barisPemutus =
      barisToken || barisSection || (token ? barisTokenSaja : null);
    if (barisPemutus) {
...
/* =========================================================
   ...
   ========================================================= */
var BA_PHOTO_COLUMN = {
  nameplateTrafoAwal: "N",
  fotoFullGarduAwal: "AH",
  nameplatePhbAwal: "AB",
  fotoBoxPhbAwal: "AE",
  nameplateTrafoSesudah: "AP",
  fotoFullGarduSesudah: "BC",
  nameplatePhbSesudah: "AW",
  fotoBoxPhbSesudah: "AZ",
};
...
function _baPhotoFolder_(tanggalBA, jenisPekerjaan, nomorTrafo) {
  ...
  if (!jenisPekerjaan)
    throw new Error("Jenis Pekerjaan wajib dipilih sebelum mengunggah foto.");
  if (!nomorTrafo)
    throw new Error("Nomor Gardu/Trafo wajib diisi sebelum mengunggah foto.");
...
function uploadFotoBeritaAcara(request) {
  try {
    ...
    var fileName = String(request.fileName || slot + ".jpg").replace(
      /[\\/:*?"<>|]/g,
      "-",
    );
    ...
    if (!dataUrl || dataUrl.indexOf(",") < 0)
      return { ok: false, message: "Data gambar tidak valid." };
    var base64 = dataUrl.split(",")[1];
    // Batas 5 MB menjaga proses Apps Script dan request web tetap stabil.
    if (base64.length > 7000000)
      return { ok: false, message: "Ukuran gambar maksimal 5 MB." };
...
function _swCariBarisBaTtd_(idBA) {
  try {
    ...
    if (!sh)
      return { ok: false, message: "Sheet Rekap Switching tidak ditemukan." };
    ...
    for (var r = 0; r < values.length; r++) {
      if (_baNormHeader_(String(values[r][idxIdBA] || "")) === key) {
        return {
          ...
          sheet: sh,
          sheetRow: r + 1,
          col: _swColToIndex_(SW_COL.linkBaTtd),
        };
      }
    }
    return { ok: false };
    ...
    return {
      ok: false,
      message: "Gagal membaca Rekap Switching: " + e.message,
    };
  }
}

function _baLokasiBaTtd_(idBA, peralatan) {
  peralatan = String(peralatan || "")
    .trim()
    .toLowerCase();
    ...
    return sw.message
      ...
      : {
          ...
          message:
            'Baris BA Switching "' +
            idBA +
            '" tidak ditemukan di Rekap Switching.',
        };
  }
  try {
  ...
  return {
    ...
    message:
      'Baris BA "' +
      idBA +
      '" tidak ditemukan di Rekap Gardu maupun Rekap Switching.',
  };
}
...
function uploadBaFinal(request) {
  try {
    ...
    var dataUrl = String(request.dataUrl || "");
    var fileName = String(request.fileName || idBA + ".pdf").replace(
      /[\\/:*?"<>|]/g,
      "-",
    );
...
function getLinkWaBeritaAcara(idBA) {
  try {
    ...
    var sh = ss.getSheetByName(BA_SOURCE.sheetName);
    if (!sh)
      return { ok: false, message: "Sheet Rekap Gardu tidak ditemukan." };
    var values = sh.getDataRange().getValues();
    var found = _baFindHeader_(values);
    if (!found)
      return { ok: false, message: "Header Rekap Gardu belum dikenali." };
    var rowIndex = _baCariBarisNomorBA_(sh, found, target);
    if (rowIndex < 0)
      return {
        ok: false,
        message: 'Baris BA "' + target + '" tidak ditemukan.',
      };
    ...
    function _tgl(letter) {
      ...
      var p = iso.split("-");
      return (
        String(parseInt(p[2], 10)) +
        " " +
        _baBulanIndonesia_(Number(p[1])) +
        " " +
        p[0]
      );
    }
    ...
    if (jl.indexOf("pengoperasian") >= 0) {
      ...
      return {
        ok: false,
        message:
          'Jenis Pekerjaan "' + jenis + '" belum didukung untuk kirim WA.',
      };
    }
    var waUrl =
...
function updateBeritaAcaraDetail(request) {
  try {
    ...
    var rowIndex = _baCariBarisNomorBA_(sh, found, idBA);
    if (rowIndex < 0)
      return { ok: false, message: 'Baris BA "' + idBA + '" tidak ditemukan.' };
    var sheetRow = rowIndex + 1;

    function _set_(letter, value) {
      sh.getRange(sheetRow, _baColLetterToIndex_(letter) + 1).setValue(
        value == null ? "" : value,
      );
    }
    ...
    var has = function (k) {
      return Object.prototype.hasOwnProperty.call(request, k);
    };
    ...
    if (has("koordinatX") && has("koordinatY")) {
      var kNorm = _baNormalisasiKoordinat_(
        request.koordinatX,
        request.koordinatY,
      );
    ...
    if (has("tanggalBA")) {
      ...
      if (cTgl >= 0)
        sh.getRange(sheetRow, cTgl + 1).setValue(
          request.tanggalBA == null ? "" : request.tanggalBA,
        );
    }
    if (has("nomorTrafo")) {
      var cNama = _baPickIndex_(found.map, [
        "Nomor Trafo",
        "Nama Peralatan",
        "Nomor Gardu",
        "Nama Gardu",
      ...
      if (cNama >= 0)
        sh.getRange(sheetRow, cNama + 1).setValue(
          request.nomorTrafo == null ? "" : request.nomorTrafo,
        );
    }
    ...
    if (has("penyulang") || has("section")) {
      var penyulangNow = has("penyulang")
        ? String(request.penyulang || "").trim()
        : String(
            sh.getRange(sheetRow, _baColLetterToIndex_("L") + 1).getValue() ||
              "",
          ).trim();
      var sectionNow = has("section")
        ? String(request.section || "").trim()
        : String(
            sh.getRange(sheetRow, _baColLetterToIndex_("M") + 1).getValue() ||
              "",
          ).trim();
      ownerHasil = getOwnerIdDanExternalReference({
        penyulang: penyulangNow,
        section: sectionNow,
      });
    ...
    return {
      ok: false,
      message: "Gagal memperbarui data BA: " + error.message,
    };
  }
}
/* =========================================================
...
function updateFotoBeritaAcaraDetail(request) {
  try {
    ...
    var kolom =
      BA_PHOTO_COLUMN[slot] ||
      (typeof BA_PRK_FOTO_COLUMN !== "undefined"
        ? BA_PRK_FOTO_COLUMN[slot]
        : "");
    if (!kolom)
      return { ok: false, message: "Slot foto tidak dikenal: " + slot };
    var dataUrl = String(request.dataUrl || "");
    var fileName = String(request.fileName || slot + ".jpg").replace(
      /[\\/:*?"<>|]/g,
      "-",
    ...
    var urlLama = String(
      sh.getRange(sheetRow, kolomIndex + 1).getValue() || "",
    ).trim();
    ...
    return {
      ...
      fileId: file.getId(),
      sheetColumn: kolom,
      fileLamaDihapus: fileLamaDihapus,
    };
  } catch (error) {
    return {
      ok: false,
      message: "Gagal memperbarui foto BA: " + error.message,
    };
  }
}
  ...
  try {
    ...
    if (!found) {
      return {
        ok: false,
        message:
          "Header Rekap Gardu belum dikenali. Diperlukan minimal kolom Tanggal BA, Penyulang, dan Nomor Trafo/Nomor Gardu.",
      };
    }
    var H = found.map;
    var IDX = {
      idBA: _baPickIndex_(H, ["idBA", "ID BA", "Id BA", "Nomor BA", "No BA"]),
      tanggal: _baPickIndex_(H, [
        "Tanggal BA",
        "Tanggal",
        "Tanggal Pekerjaan",
        "Tgl",
      ]),
      jenis: _baPickIndex_(H, [
        "Jenis Pekerjaan",
        "Pekerjaan",
        "Kategori Pekerjaan",
      ]),
      peralatan: _baPickIndex_(H, [
        "Peralatan",
        "Jenis Peralatan",
        "Objek Peralatan",
        "Objek",
      ]),
      nama: _baPickIndex_(H, [
        "Nomor Trafo",
        "Nama Peralatan",
        "Nomor Gardu",
        "Nama Gardu",
    ...
    var peralatanFilter = String(
      filter.kataKunciPeralatan || filter.nomorGardu || "",
    )
      .trim()
      .toLowerCase();
    var dari = _baDate_(filter.tglDari);
    var sampai = _baDate_(filter.tglSampai);
    var jenisFilter = String(filter.jenisPekerjaan || "")
      .trim()
      .toLowerCase();
    var alatFilter = String(filter.peralatan || "")
      .trim()
      .toLowerCase();
    var statusFilter = String(filter.statusBA || "")
      .trim()
      .toLowerCase();
    var rows = [];
    for (var i = found.row + 1; i < values.length; i++) {
      ...
      if (alatFilter) {
        if (alatFilter === "gardu") {
          ...
          if (_jl.indexOf("trafo") === -1 && _jl.indexOf("gardu") === -1)
            continue;
      ...
      for (var c = 0; c < found.headers.length; c++) {
        ...
        detail[label] =
          value instanceof Date
            ? Utilities.formatDate(value, "Asia/Jakarta", "dd/MM/yyyy")
            : String(value);
      }
      rows.push({
        ...
        megger: (function () {
          ...
          Object.keys(BA_PRK_FOTO_COLUMN).forEach(function (slot) {
            out[slot] = String(
              _baCell_(row, _baColLetterToIndex_(BA_PRK_FOTO_COLUMN[slot])) ||
                "",
            ).trim();
          });
          return out;
    ...
    rows.sort(function (a, b) {
      return (
        ...
        String(a.namaPeralatan || "").localeCompare(
          String(b.namaPeralatan || ""),
        )
      );
    });
...
function _baGenerateIdBA_(values, headerRow, idxIdBA, tanggalBA) {
  ...
  return {
    idBA: prefix + ("00" + (maxSeq + 1)).slice(-3),
    year: year,
    month: month,
  };
}
function _baGenerateNoBaFull_(values, headerRow, idxNoFull, year, month) {
  ...
  if (idxNoFull >= 0) {
    for (var r = headerRow + 1; r < values.length; r++) {
      ...
      var mm = v.match(
        /No\.\s*(\d+)\s*\/\s*BA\s*-\s*TRF\s*\/\s*ULP\s*-\s*TBL\.?\s*\/\s*(\d+)\s*\/\s*(\d+)/i,
      );
      if (
        mm &&
        parseInt(mm[2], 10) === parseInt(bulan, 10) &&
        String(mm[3]) === String(year)
      ) {
...
function simpanBeritaAcaraGardu(payload) {
  try {
    ...
    if (col.idBA < 0)
      return {
        ok: false,
        message: "Kolom idBA tidak ditemukan di Rekap Gardu.",
      };
    ...
    for (var r = found.row + 1; r < values.length; r++) {
      ...
      var adaNama =
        idxNama >= 0
          ? String(_baText_(values[r], idxNama) || "") !== ""
          : false;
    ...
    var idBA = gen.idBA;
    var noBaFull = _baGenerateNoBaFull_(
      values,
      found.row,
      col.nomorBAFull,
      gen.year,
    ...
    if (String(jenis).toLowerCase().indexOf("penggantian") >= 0) {
      ...
      var _cPhbDiganti = _baPastikanKolom_(sh, found, [
        "PHB-TR Diganti",
        "Status PHB-TR",
        "PHB Diganti",
      ]);
      sh.getRange(sheetRow, _cPhbDiganti + 1).setValue(
        phbTrSesudah.diganti ? "Ya" : "Tidak",
      );
    }
    ...
    Object.keys(nilai).forEach(function (field) {
      ...
      var c = col[field];
      if (c != null && c >= 0)
        sh.getRange(sheetRow, c + 1).setValue(nilai[field]);
    });
    ...
    var cek = String(
      sh.getRange(sheetRow, col.idBA + 1).getValue() || "",
    ).trim();
    if (cek !== idBA) {
      return {
        ok: false,
        ...
        idBA: idBA,
        message:
          'BA belum tersimpan di sheet: idBA "' +
          idBA +
          '" tidak muncul di baris ' +
          sheetRow +
          " (kolom idBA). Cek apakah kolom idBA / Nomor Trafo di Rekap Gardu berisi ARRAYFORMULA, sedang difilter, atau termasuk Range terproteksi.",
      };
    }
    ...
    phbPeriksa.forEach(function (item) {
      ...
      var tersimpan = String(
        sh
          .getRange(sheetRow, _baColLetterToIndex_(item.letter) + 1)
          .getValue() || "",
      ).trim();
      if (tersimpan !== String(item.nilai).trim())
        phbGagal.push(item.label + " (kolom " + item.letter + ")");
    });
    if (phbGagal.length) {
      return {
        ok: false,
        ...
        idBA: idBA,
        message:
          "Data utama tersimpan, tetapi PHB-TR gagal tersimpan pada: " +
          phbGagal.join(", ") +
...
var BA_MASTER_TARGETS = [
  ...
  {
    spreadsheetId: "1mfgj9Izkh3gy9Xf0LkxyIR8tETMwY2RbjQdZ_U4vN6w",
    sheetName: "1. DATA TRAFO",
  },
...
function getNomorGarduBerikutnya(kodeGardu) {
  try {
    var kode = String(kodeGardu || "")
      .trim()
      .toUpperCase();
    if (!kode)
      return {
        ok: false,
        message: "Kode gardu wajib dipilih (TB / PY / TL / PG).",
      };
    if (BA_KODE_GARDU.indexOf(kode) === -1) {
      return {
        ...
        message:
          'Kode gardu "' +
          kode +
          '" tidak dikenal. Pilih salah satu: ' +
          BA_KODE_GARDU.join(", ") +
          ".",
      };
    }
    ...
    if (maxRekap > terakhir) {
      terakhir = maxRekap;
      sumber = "Rekap Gardu";
    } // Rekap bila lebih tinggi
...
function _baDetailPick_(detail, aliases) {
  ...
  Object.keys(detail).forEach(function (k) {
    var nk = _baNormHeader_(k);
    if (nk && norm[nk] == null) norm[nk] = detail[k];
  });
  for (var i = 0; i < aliases.length; i++) {
...
function syncGarduKeMaster(request) {
  try {
    ...
    var tglPekerjaanBA = _baDate_(
      _baDetailPick_(detail, BA_MG_TANGGAL_ALIASES),
    );
    if (!tglPekerjaanBA)
      return {
        ok: false,
        message: "Tanggal Pekerjaan BA tidak ditemukan pada data baris ini.",
      };

    var ss = SpreadsheetApp.openById(BA_MASTER_GARDU.spreadsheetId);
    var sh = _baResolveSheet_(ss, BA_MASTER_GARDU.sheetName);
    if (!sh)
      return { ok: false, message: "Sheet Master Gardu tidak ditemukan." };
    var values = sh.getDataRange().getValues();
    var found = _baFindKolomGardu_(values);
    if (!found)
      return {
        ok: false,
        message: "Kolom Nomor Gardu di Master Gardu tidak dikenali.",
      };

    var headers = values[found.row] || [];
    var mgHeaderIdx = {};
    for (var c = 0; c < headers.length; c++) {
      var hk = _baNormHeader_(headers[c]);
      if (hk && mgHeaderIdx[hk] == null) mgHeaderIdx[hk] = c;
    }
      ...
      if (arrIdx <= found.row || arrIdx >= values.length) {
        return {
          ...
          message:
            "Index baris " +
            indexNum +
            " di luar jangkauan Master Gardu (baris data " +
            (found.row + 2) +
            ".." +
            values.length +
            ").",
    ...
    if (targetRow < 0) {
      ...
      bangunNilai().forEach(function (pair) {
        if (pair[0] < newRow.length) newRow[pair[0]] = pair[1];
      });
      sh.appendRow(newRow);
      SpreadsheetApp.flush();
      return {
        ...
        action: "insert",
        message:
          "Tidak ada Index tujuan — baris baru ditambahkan ke Master Gardu (Tanggal Pekerjaan " +
          tglPekerjaanBA +
          ").",
      };
    }
    ...
    var tglMaster =
      mgTglCol >= 0 ? _baDate_(_baCell_(values[targetRow], mgTglCol)) : "";
    if (tglMaster && tglPekerjaanBA <= tglMaster) {
      return {
        ok: true,
        action: "skip",
        message:
          "Dilewati: Tanggal Pekerjaan BA (" +
          tglPekerjaanBA +
          ") tidak lebih baru dari Master Gardu (" +
          tglMaster +
          ").",
      };
    }
    ...
    bangunNilai().forEach(function (pair) {
      sh.getRange(sheetRow, pair[0] + 1).setValue(pair[1]);
    });
    SpreadsheetApp.flush();
    return {
      ok: true,
      ...
      message:
        ...
        " diperbarui" +
        (tglMaster
          ? " (" + tglMaster + " → " + tglPekerjaanBA + ")"
          : " (Tanggal Pekerjaan " + tglPekerjaanBA + ")") +
        ".",
    };
  } catch (error) {
...
function _baNormalisasiKoordinat_(latRaw, longRaw) {
  ...
  if (lat === "" || lng === "") {
    return {
      ...
      lng: "",
      gabung: "",
      pesan: "Koordinat tidak lengkap / tidak terbaca (butuh Lat dan Long).",
    };
  }

  var B = BA_KOORDINAT_BATAS;
  function _dlm(la, ln) {
    return la >= B.latMin && la <= B.latMax && ln >= B.lngMin && ln <= B.lngMax;
  }

  // Auto-tukar bila tampak tertukar (Lat berisi nilai Long & sebaliknya).
  if (!_dlm(lat, lng) && _dlm(lng, lat)) {
    var t = lat;
    lat = lng;
    lng = t;
  }
  ...
  if (!_dlm(lat, lng)) {
    return {
      ...
      pesan:
        ...
        ", " +
        lng +
        ") di luar wilayah Kepulauan Bangka Belitung (Lat " +
        B.latMin +
        ".." +
        B.latMax +
        ", Long " +
...
function updateMasterGarduDariBA(idBA) {
  try {
    ...
    var idxNomorBA = _baPickIndex_(foundBA.map, [
      "idBA",
      "ID BA",
      "Nomor BA",
      "No BA",
    ...
    if (idxNomorBA < 0)
      return {
        ok: false,
        message: "Kolom idBA / Nomor BA di Rekap Gardu tidak ditemukan.",
      };
    var keyBA = target.toLowerCase();
    var rowBA = -1;
    for (var rb = foundBA.row + 1; rb < baValues.length; rb++) {
      if (String(_baText_(baValues[rb], idxNomorBA)).toLowerCase() === keyBA) {
        rowBA = rb;
        break;
      }
    }
    ...
    function _b(letter) {
      return _baText_(ba, _baColLetterToIndex_(letter));
    }
    ...
    if (!isPengoperasian && !isPenggantian) {
      return {
        ...
        action: "skip",
        message:
          'Dilewati: Jenis Pekerjaan "' +
          jenis +
          '" tidak meng-update Master Gardu (hanya Pengoperasian & Penggantian Trafo).',
      };
    }
    ...
    if (!nomorGardu)
      return {
        ok: false,
        message: "Nomor Gardu (kolom B) kosong pada baris BA ini.",
      };
    ...
    if (!tanggalBA)
      return {
        ok: false,
        message: "Tanggal BA tidak ditemukan/kosong pada baris BA ini.",
      };
    ...
    for (var ti = 0; ti < BA_MASTER_TARGETS.length; ti++) {
      ...
      var res;
      try {
        res = _baTerapkanUpdateMaster_(tgt, ctx);
      } catch (eT) {
        res = {
          ok: false,
          message: "Gagal: " + (eT && eT.message ? eT.message : eT),
        };
      }
    ...
    var pesanGabung = hasil.map(function (rr) {
      var ket = rr.message
        ? rr.message
        : (rr.action === "insert"
            ? "baris " + rr.baris + " ditambahkan (gardu baru)"
            : "baris " + rr.baris + " diperbarui") +
          (rr.jumlahFieldDiubah != null
            ? ", " + rr.jumlahFieldDiubah + " field"
            : "");
    ...
    return {
      ok: false,
      message: "Gagal update Master Gardu dari BA: " + error.message,
    };
  }
}
...
function _baTerapkanUpdateMaster_(tgt, ctx) {
  ...
  if (!shMG)
    return {
      ok: false,
      message: 'Sheet "' + tgt.sheetName + '" tidak ditemukan.',
    };
  var mgLastRow = shMG.getLastRow();
  var mgLastCol = shMG.getLastColumn();
  if (mgLastRow < 1 || mgLastCol < 1)
    return { ok: false, message: '"' + tgt.sheetName + '" kosong.' };
  ...
  if (!foundMG)
    return {
      ok: false,
      message: 'Kolom Nomor Gardu di "' + tgt.sheetName + '" tidak dikenali.',
    };
  var colNomorMG = foundMG.col; // umumnya kolom C (index 2)
  var nomorColValues = shMG
    .getRange(1, colNomorMG + 1, mgLastRow, 1)
    .getValues();
  var keyNomor = ctx.nomorGardu.trim().toLowerCase();
  var targetRow = -1;
  for (var r = foundMG.row + 1; r < nomorColValues.length; r++) {
    if (
      String(nomorColValues[r][0] || "")
        .trim()
        .toLowerCase() === keyNomor
    ) {
      targetRow = r;
      break;
    }
    ...
    if (!ctx.isPengoperasian) {
      return {
        ...
        message:
          'Nomor Gardu "' +
          ctx.nomorGardu +
          '" belum ada. Tambahkan dulu barisnya sebelum update (khusus BA Penggantian).',
      };
    }
    barisBaru = true;
  }
  ...
  if (barisBaru) {
    ...
    var formulaBaris = shMG
      .getRange(sumberSheetRow, 1, 1, mgLastCol)
      .getFormulas()[0];
    for (var ci = 0; ci < formulaBaris.length; ci++) {
      if (formulaBaris[ci]) {
        shMG
          .getRange(sumberSheetRow, ci + 1)
          .copyTo(shMG.getRange(sheetRow, ci + 1));
      }
    }
  ...
  return {
    ok: true,
    action: barisBaru ? "insert" : "update",
    baris: sheetRow,
    jumlahFieldDiubah: diubah.length,
    tglMaster: tglMaster,
  };
}
...
var BA_PDF_ENGINE = {
  url:
    PropertiesService.getScriptProperties().getProperty("BA_PDF_ENGINE_URL") ||
    "https://ba-pdf-engine-1011716929576.asia-southeast2.run.app",
  secret:
    PropertiesService.getScriptProperties().getProperty("BA_PDF_SECRET") ||
...
function _baCariBarisNomorBA_(sh, found, nomorBA) {
  var idxNomor = _baPickIndex_(found.map, [
    "idBA",
    "ID BA",
    "Nomor BA",
    "No BA",
  ...
  for (var r = found.row + 1; r < values.length; r++) {
    if (String(_baText_(values[r], idxNomor)).toLowerCase() === target)
      return r; // 0-based
  }
  return -1;
}
...
function _baTulisHasilPdf_(nomorBA, hasil) {
  ...
  if (rowIndex < 0)
    throw new Error("Baris BA " + nomorBA + " tidak ditemukan.");
  ...
  var sheetRow = rowIndex + 1;
  if (hasil.fileUrl != null)
    sh.getRange(sheetRow, cols.fileUrl + 1).setValue(hasil.fileUrl);
  if (hasil.fileId != null)
    sh.getRange(sheetRow, cols.fileId + 1).setValue(hasil.fileId);
  if (hasil.status != null)
    sh.getRange(sheetRow, cols.status + 1).setValue(hasil.status);
  if (hasil.selesai != null)
    sh.getRange(sheetRow, cols.selesai + 1).setValue(hasil.selesai);
}
...
function _baBacaFileIdTetap_(nomorBA) {
  try {
    ...
    return String(
      sh.getRange(rowIndex + 1, _baColLetterToIndex_("CL") + 1).getValue() ||
        "",
    ).trim();
  } catch (e) {
    return "";
...
function generatePdfBaPengoperasian(nomorBA) {
  try {
    ...
    try {
      _baTulisHasilPdfTetap_(nomorBA, { status: "Proses", selesai: "" });
    } catch (e) {}

    // File ID PDF lama -> dikirim ke engine untuk dihapus saat generate ulang.
    var oldFileId = "";
    try {
      oldFileId = _baBacaFileIdTetap_(nomorBA);
    } catch (e) {}
    ...
    if (jenisRow.indexOf("penggantian") >= 0) {
      endpointPath = "/ba/penggantian";
      jenisParam = "penggantian";
    } else if (jenisRow.indexOf("pemeriksaan") >= 0) {
      endpointPath = "/ba/pemeriksaan-trafo";
      jenisParam = "pemeriksaan";
    }
    ...
    try {
      UrlFetchApp.fetch(BA_PDF_ENGINE.url + "/", {
        method: "get",
    ...
    for (var attempt = 1; attempt <= maksPercobaan; attempt++) {
      ...
      var transient =
        ...
        code === 503 ||
        code === 504;
      if (transient && attempt < maksPercobaan) {
    ...
    if (code !== 200 || !data || !data.ok) {
      ...
      var pesan =
        (data && data.message) ||
        (code === 0
          ? "Tidak dapat menghubungi mesin PDF" +
            (lastErr ? ": " + lastErr : "") +
            "."
          : "Cloud Run gagal (HTTP " +
            code +
            ")." +
            (code === 503 || code === 502 || code === 504
              ? " Mesin PDF sedang tidak siap/menyala. Coba lagi beberapa saat; bila terus terjadi, periksa deploy & Logs Cloud Run ba-pdf-engine (kemungkinan container gagal start / kurang memori)."
              : ""));
...
function testPdfBa() {
  Logger.log(
    JSON.stringify(generatePdfBaPengoperasian("BA-GRD-TBL-202607-001")),
  );
}
...
var SW_COL = {
  ...
  koordinatSesudah: "X",
  asalSwitching: "Y",
  fotoNameplateSesudah: "Z",
  fotoKonstruksiSesudah: "AA",
...
/* ---------------- Helpers Switching ---------------- */
function _swNormHeader_(value) {
  return String(value == null ? "" : value)
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]/g, "");
}
function _swColToIndex_(letter) {
  var r = 0,
    s = String(letter || "").toUpperCase();
  for (var i = 0; i < s.length; i++) {
    r = r * 26 + (s.charCodeAt(i) - 64);
  }
  return r - 1;
}
function _swText_(row, index) {
  return index >= 0 && index < row.length
    ? String(row[index] == null ? "" : row[index]).trim()
    : "";
}
function _swCell_(row, letter) {
  return _swText_(row, _swColToIndex_(letter));
}
function _swSafeFolderName_(value) {
  return (
    String(value || "")
      .trim()
      .replace(/[\\/:*?"<>|]/g, "-") || "Tanpa Nama"
...
function _swBulanIndonesia_(month) {
  return (
    [
      "Januari",
      "Februari",
...
function _swFolder_(parent, name) {
...
function _swDate_(value) {
  ...
  return isNaN(d.getTime())
    ? ""
    : Utilities.formatDate(d, "Asia/Jakarta", "yyyy-MM-dd");
}
function _swNow_() {
  return Utilities.formatDate(
    new Date(),
...
function _swHeaderLabels_() {
  ...
  return [
    ...
    "Nomor Seri",
    "Koordinat Awal",
    "Foto Nameplate Switching",
    "Foto Konstruksi Switching",
    "SLD Sebelum",
    "SLD Sesudah",
    "Jenis Switching Sesudah",
    "Nama Switching Sesudah",
    "Penyulang Sesudah",
    "Section Sesudah",
    "Merk Switching Sesudah",
    "Tipe Switching Sesudah",
    "Nomor Seri Switching Sesudah",
    "Koordinat Sesudah",
    "Asal Switching",
    "Foto Nameplate Switching Sesudah",
    "Foto Konstruksi Switching Sesudah",
    "Keterangan Tambahan",
    "Keterangan Kerusakan / Pemeriksaan",
    "Petugas 1",
    "Jabatan Petugas 1",
    "Petugas 2",
    "Jabatan Petugas 2",
...
function _swEnsureSheet_(ss) {
  ...
  for (var i = 0; i < firstRow.length; i++) {
    if (_swNormHeader_(firstRow[i]) === "idba") {
      punyaHeader = true;
      break;
    }
  }
...
function setupSheetSwitching() {
  try {
    ...
    return { ok: false, message: "Gagal setup sheet: " + error.message };
  }
}
...
function _swGenNoFull_(values, headerRow, idxNoFull, year, month) {
  ...
  if (idxNoFull >= 0) {
    for (var r = headerRow + 1; r < values.length; r++) {
      ...
      var mm = v.match(
        /No\.\s*(\d+)\s*\/\s*BA\s*-\s*SWC\s*\/\s*ULP\s*-\s*TBL\.?\s*\/\s*(\d+)\s*\/\s*(\d+)/i,
      );
      if (
        mm &&
        parseInt(mm[2], 10) === parseInt(bulan, 10) &&
...
function simpanBeritaAcaraSwitching(payload) {
  try {
    ...
    var jl = jenis.toLowerCase();
    var perluSesudah =
      jl.indexOf("penggantian") >= 0 || jl.indexOf("relokasi") >= 0;

    function _set_(letter, value) {
      sh.getRange(sheetRow, _swColToIndex_(letter) + 1).setValue(
        value == null ? "" : value,
      );
    }
    function _setText_(letter, value) {
      var rng = sh.getRange(sheetRow, _swColToIndex_(letter) + 1);
      rng.setNumberFormat("@");
      rng.setValue(String(value == null ? "" : value));
    }
    ...
    Object.keys(SW_PHOTO_COLUMN).forEach(function (slot) {
      ...
      if (url)
        sh.getRange(
          sheetRow,
          _swColToIndex_(SW_PHOTO_COLUMN[slot]) + 1,
        ).setValue(url);
    });

    SpreadsheetApp.flush();
    var cek = String(
      sh.getRange(sheetRow, idxIdBA + 1).getValue() || "",
    ).trim();
    if (cek !== idBA) {
      return {
        ok: false,
        ...
        idBA: idBA,
        message:
          "BA belum tersimpan: idBA tidak muncul di baris " +
          sheetRow +
          ". Cek proteksi/ARRAYFORMULA pada kolom AG.",
      };
    }
    ...
    return {
      ok: false,
      message: "Gagal menyimpan BA Switching: " + error.message,
    };
  }
}
...
function getDataSwitching(filter) {
  try {
    ...
    var kata = String(filter.kataKunci || filter.namaSwitching || "")
      .trim()
      .toLowerCase();
    var dari = _swDate_(filter.tglDari),
      sampai = _swDate_(filter.tglSampai);
    var jenisFilter = String(filter.jenisPekerjaan || "")
      .trim()
      .toLowerCase();
    var rows = [];
    for (var i = headerRow + 1; i < values.length; i++) {
      ...
      rows.push({
        idBA: idBA,
        noBaFull: _swCell_(row, SW_COL.noBaFull),
        tanggal: tanggal,
        jenisPekerjaan: jenis,
        namaSwitching: nama,
        jenisSwitching: _swCell_(row, SW_COL.jenisSwitching),
        penyulang: _swCell_(row, SW_COL.penyulang),
        section: _swCell_(row, SW_COL.section),
        merk: _swCell_(row, SW_COL.merk),
        tipe: _swCell_(row, SW_COL.tipe),
        nomorSeri: _swCell_(row, SW_COL.nomorSeri),
        sldSebelum: _swCell_(row, SW_COL.sldSebelum),
        sldSesudah: _swCell_(row, SW_COL.sldSesudah),
        koordinat: _swCell_(row, SW_COL.koordinat),
        filePdfUrl: _swCell_(row, SW_COL.filePdfUrl),
        fileId: _swCell_(row, SW_COL.fileId),
    ...
    return { ok: false, message: "Error Switching: " + error.message };
  }
}
...
function _swPhotoFolder_(tanggalBA, jenisPekerjaan, namaSwitching) {
  ...
  if (!jenisPekerjaan)
    throw new Error("Jenis Pekerjaan wajib dipilih sebelum mengunggah foto.");
  if (!namaSwitching)
    throw new Error("Nama Switching wajib diisi sebelum mengunggah foto.");
...
function uploadFotoSwitching(request) {
  try {
    ...
    if (base64.length > 7000000)
      return { ok: false, message: "Ukuran gambar maksimal 5 MB." };
    var folder = _swPhotoFolder_(
      request.tanggalBA,
      request.jenisPekerjaan,
      request.namaSwitching,
    );
    var timestamp = Utilities.formatDate(
      new Date(),
      "Asia/Jakarta",
    ...
    return {
      ...
      fileId: file.getId(),
      folderId: folder.getId(),
      sheetColumn: SW_PHOTO_COLUMN[slot] || "",
    };
  } catch (error) {
    return { ok: false, message: "Upload foto gagal: " + error.message };
  }
...
function _swCariBaris_(sh, idBA) {
  ...
  for (var r = headerRow + 1; r < values.length; r++) {
    if (String(_swText_(values[r], idx)).toLowerCase() === target) return r;
  }
  return -1;
}
function _swTulisHasil_(idBA, hasil) {
  ...
  var sheetRow = rowIndex + 1;
  function _set_(letter, value) {
    sh.getRange(sheetRow, _swColToIndex_(letter) + 1).setValue(value);
  }
...
function generatePdfBaSwitching(idBA) {
  try {
    ...
    try {
      _swTulisHasil_(idBA, { status: "Proses", selesai: "" });
    } catch (e) {}
    // File ID PDF lama -> dikirim ke engine untuk dihapus saat generate ulang.
    var oldFileId = "";
    try {
      oldFileId = _swBacaFileId_(idBA);
    } catch (e) {}
    try {
      UrlFetchApp.fetch(SW_PDF_ENGINE.url + "/", {
        method: "get",
        muteHttpExceptions: true,
      });
    } catch (e) {}
    ...
    if (code !== 200 || !data || !data.ok) {
      try {
        _swTulisHasil_(idBA, { status: "Gagal", selesai: _swNow_() });
      } catch (e) {}
      var pesan =
        (data && data.message) ||
        (code === 0
          ? "Tidak dapat menghubungi mesin PDF" +
            (lastErr ? ": " + lastErr : "") +
            ". Endpoint /ba/switching mungkin belum tersedia."
          : "Cloud Run gagal (HTTP " +
            code +
            "). Endpoint /ba/switching mungkin belum di-deploy.");
      return { ok: false, message: pesan };
    }
...
function getDataTrafoMaster(nomorGardu) {
  try {
    ...
    for (var r = found.row + 1; r < values.length; r++) {
      if (
        String(values[r][found.col] == null ? "" : values[r][found.col])
          .trim()
          .toLowerCase() !== kunci
      )
        continue;
    ...
    return {
      ...
      message:
        'Nomor Gardu "' +
        String(nomorGardu).trim() +
        '" belum ada di Master Gardu.',
    };
  } catch (error) {
    return {
      ok: false,
      message: "Gagal membaca Master Gardu: " + error.message,
    };
  }
}
  ...
  try {
    ...
    if (!kolom)
      return {
        ok: false,
        message:
          "Slot foto megger tidak dikenal: " +
          slot +
          " (gunakan megger1..megger6).",
      };
    var dataUrl = String(request.dataUrl || "");
    var fileName = String(request.fileName || slot + ".jpg").replace(
      /[\\/:*?"<>|]/g,
    ...
    var folder = _baPhotoFolder_(
      request.tanggalBA,
      request.jenisPekerjaan || BA_PRK_JENIS,
      request.nomorTrafo,
    );
    var timestamp = Utilities.formatDate(
      new Date(),
...
function simpanBaPemeriksaanTrafo(payload) {
  try {
    ...
    Object.keys(viaAlias).forEach(function (field) {
      var c = col[field];
      if (c != null && c >= 0)
        sh.getRange(sheetRow, c + 1).setValue(viaAlias[field]);
    });
    ...
    if (cek !== idBA) {
      return {
        ...
        idBA: idBA,
        message:
          'BA belum tersimpan: idBA "' +
          idBA +
          '" tidak muncul di baris ' +
          sheetRow +
          ". Cek ARRAYFORMULA / filter / Range terproteksi di Rekap Gardu.",
      };
    }
    return {
      ok: true,
    ...
    return {
      ok: false,
      message: "Gagal menyimpan BA Pemeriksaan Trafo: " + error.message,
    };
  }
}
...
function updateKesimpulanPemeriksaanTrafo(request) {
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(request, "kesimpulan")) {
      sh.getRange(
        sheetRow,
        _baColLetterToIndex_(BA_PRK_KESIMPULAN_COL) + 1,
      ).setValue(String(request.kesimpulan || ""));
    }
    if (Object.prototype.hasOwnProperty.call(request, "catatan")) {
      sh.getRange(
        sheetRow,
        _baColLetterToIndex_(BA_PRK_CATATAN_COL) + 1,
      ).setValue(String(request.catatan || ""));
    }
    SpreadsheetApp.flush();
    ...
    return {
      ok: false,
      message: "Gagal memperbarui kesimpulan: " + error.message,
    };
  }
}
