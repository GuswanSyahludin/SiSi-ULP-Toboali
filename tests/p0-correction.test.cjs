...
test('predeploy audit fails closed without the P0 correction wrapper',()=>{const ctx={};vm.createContext(ctx);vm.runInContext(predeployAudit,ctx);const checks=ctx._auditP0CorrectionPredeploy_();assert.equal(checks.length,3);assert.ok(checks.every(c=>!c.ok));});
test('predeploy audit accepts the installed P0 correction wrapper',()=>{const f=fixture();vm.runInContext(predeployAudit,f.ctx);const checks=f.ctx._auditP0CorrectionPredeploy_();assert.equal(checks.length,3);assert.ok(checks.every(c=>c.ok));});
