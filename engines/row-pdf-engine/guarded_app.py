"""Conservative daily traffic guard for row-pdf-engine.

Not a billing hard cap: Cloud Run cost also depends on CPU, memory and egress.
The local counter can reset with an instance, so configuration and Billing
alerts remain mandatory.
"""
import json
import logging
import os
...
class CostGuardMiddleware:
    ...
    def _increment(self):
        with self.lock:
            ...
            if state.get("day") != self._day():
                state = {"day": self._day(), "count": 0}; self.warned.clear()
            state["count"] = int(state.get("count", 0)) + 1
            tmp = self.path.with_suffix(".tmp")
            tmp.write_text(json.dumps(state, separators=(",", ":"))); tmp.replace(self.path)
            return state["count"]
    ...
    def _reply(environ, start_response, status, code, message, headers):
        return Response(json.dumps({"ok": False, "code": code, "message": message}),
                        status=status, content_type="application/json", headers=headers)(environ, start_response)

    def __call__(self, environ, start_response):
        if environ.get("PATH_INFO", "/") == "/": return self.wrapped(environ, start_response)
        ...
        if content_length > self.max_body:
            return self._reply(environ, start_response, 413, "PAYLOAD_TOO_LARGE",
                               "Payload melewati batas aman engine.", {"X-SiSi-Cost-Guard": "PAYLOAD"})
        ...
        if level and level not in self.warned:
            logging.warning("COST_GUARD service=%s level=%s daily_count=%s budget=%s local_counter=true",
                            self.service_name, level, count, self.daily_budget); self.warned.add(level)
        ...
        if ratio >= .95:
            return self._reply(environ, start_response, 503, "DAILY_GUARD_95",
                               "Batas harian lokal tercapai. Coba kembali besok.", {**headers, "Retry-After": "3600"})
        if ratio >= .85:
            now = time.monotonic()
            with self.lock:
            ...
            if wait > 0:
                return self._reply(environ, start_response, 429, "DAILY_GUARD_85",
                                   "Permintaan dibatasi sementara.", {**headers, "Retry-After": str(max(1, int(wait) + 1))})
        def guarded_start(status, response_headers, exc_info=None):
            response_headers.extend(headers.items()); return start_response(status, response_headers, exc_info)
        return self.wrapped(environ, guarded_start)


app = CostGuardMiddleware(flask_app, "row-pdf-engine")
