"""Conservative daily traffic guard for wm-engine.

Not a billing hard cap: CPU, memory and egress also affect Cloud Run billing.
Health endpoints remain available. The local counter may reset with instances,
so Billing alerts and Cloud Run limits remain mandatory.
"""
import json
import logging
import os
...
class CostGuardMiddleware:
    ...
    def __call__(self, environ, start_response):
        ...
        if path in ("/", "/watermark/v1"): return self.wrapped(environ, start_response)
