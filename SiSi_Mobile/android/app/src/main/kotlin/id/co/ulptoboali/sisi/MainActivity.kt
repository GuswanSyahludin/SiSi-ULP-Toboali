        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "id.co.ulptoboali.sisi/photo_export")
            .setMethodCallHandler { call, result ->
                if (call.method == "privatePhotoDirectory") {
                    val directory = java.io.File(filesDir, "petugas_photos")
                    if (directory.isDirectory || directory.mkdirs()) result.success(directory.absolutePath)
                    else result.error("DIRECTORY_FAILED", "Folder foto privat tidak dapat dibuat.", null)
                    return@setMethodCallHandler
                }
                if (call.method != "saveWatermarkedJpeg") { result.notImplemented(); return@setMethodCallHandler }
                if (pendingSave != null) { result.error("BUSY", "Penyimpanan foto sedang berjalan.", null); return@setMethodCallHandler }
                ...
                pendingSave = result; pendingJpeg = bytes
                try {
                ...
                } catch (error: Exception) { finishSave("OPEN_FAILED", "Pemilih lokasi penyimpanan tidak dapat dibuka.") }
            }
    }
    private fun finishSave(code: String? = null, message: String? = null, saved: Boolean = false) {
        val result = pendingSave; pendingSave = null; pendingJpeg = null
        if (code != null) result?.error(code, message, null) else result?.success(mapOf("saved" to saved))
    }
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode != saveRequest) { super.onActivityResult(requestCode, resultCode, data); return }
        if (resultCode != Activity.RESULT_OK) { finishSave(saved = false); return }
        val uri = data?.data; val bytes = pendingJpeg
        if (uri == null || bytes == null) { finishSave("NO_DATA", "Data foto tidak tersedia. Buka foto dan ulangi penyimpanan."); return }
        val resolver = contentResolver
        Thread {
            try {
                val stream = resolver.openOutputStream(uri, "w") ?: throw IllegalStateException("Output unavailable")
