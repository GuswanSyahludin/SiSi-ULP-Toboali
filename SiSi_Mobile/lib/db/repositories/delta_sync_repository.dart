...
class DeltaSyncRepository {
  ...
  Future<Map<String, dynamic>> _send(
    ...
    try {
      ...
      throw Exception(
        'Respons sinkron bukan JSON valid (HTTP ${response?.statusCode}): $preview',
      );
    }
    if (decoded is! Map) throw Exception('Respons delta sync tidak valid.');
    final out = Map<String, dynamic>.from(decoded);
    if (out['success'] != true) {
      throw Exception(out['message'] ?? 'Delta sync gagal.');
    }
    return out;
  }
  ...
  Future<Map<String, String>> _versions() async {
    final rows = await db
        .customSelect('SELECT dataset, version FROM local_dataset_state')
        .get();
    return {
  ...
  Future<DeltaSyncResult> sync(String token, {bool force = false}) async {
    ...
    return DeltaSyncResult(
      ...
      message: changed.isEmpty
          ...
          : '${changed.length} tabel diperbarui$warningText.',
    );
  }
