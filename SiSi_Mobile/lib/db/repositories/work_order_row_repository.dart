...
class WorkOrderRowRepository {
    ...
    final response = await http
        .post(uri,
            ...
            body: jsonEncode({
              ...
              'token': token,
              'ulp': 'WO_ROW:${jsonEncode(payload)}',
            }))
        .timeout(const Duration(seconds: 60));
    final decoded = jsonDecode(response.body);
    if (decoded is! Map) throw Exception('Respons WO ROW tidak valid.');
    final out = Map<String, dynamic>.from(decoded);
    if (out['module'] != 'wo-row' || out['apiVersion'] != 1) {
      throw Exception('Backend WO ROW belum di-deploy.');
    }
    if (out['success'] != true) {
      throw Exception(out['message'] ?? 'Proses WO ROW gagal.');
    }
    return out;
  }
  ...
  Future<Map<String, dynamic>> start({
    ...
    required String kodePekerjaan,
      ...
      _send(token, {'cmd': 'start', 'kodePekerjaan': kodePekerjaan});
