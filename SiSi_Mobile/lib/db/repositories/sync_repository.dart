...
class SyncRepository {
  ...
  Future<Map<String, dynamic>> sinkronSemua(String token) async {
    ...
    try {
      ...
      try {
        await TeknikToRepository().flushOutbox(activeToken);
      } catch (_) {}

      progress.update('Menyiapkan Data Master', completed: 5, total: 10);
      ...
      final materializer = LocalMasterMaterializer();

      progress.update('Menyusun data lokal', completed: 9, total: 10);
      if (initial || changed.contains('db_Penyulang')) {
        await materializer.penyulang(await deltaRepo.rows('db_Penyulang'));
      }
      if (initial || changed.contains('Master_Gardu')) {
        await materializer.gardu(await deltaRepo.rows('Master_Gardu'));
      }
      if (initial || changed.contains('db_List_Temuan')) {
        await materializer.listTemuan(await deltaRepo.rows('db_List_Temuan'));
      }
      ...
      if (changed.contains('db_Users')) {
        try {
          await TeknikToRepository().refreshTeams(activeToken);
        } catch (_) {}
      }
      ...
      await DbProvider.instance.syncDao.tandaiTersinkron(
        modul,
        jumlah: delta.changed.length,
        keterangan: delta.message,
      );
      final message =
          '${delta.message} • Outbox: P0 ${p0['terkirim'] ?? 0}, Gardu ${garduEdit['terkirim'] ?? 0}, Inspeksi ${inspeksi['terkirim'] ?? 0}';
      if (ok) {
      ...
      final message = 'Koneksi bermasalah: $error';
      progress.failure(message);
