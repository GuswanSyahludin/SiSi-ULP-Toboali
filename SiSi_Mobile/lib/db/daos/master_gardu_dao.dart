...
class MasterGarduDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<void> gantiSemua(List<MasterGardusCompanion> data) => transaction(() async {
        ...
        if (data.isNotEmpty) await batch((b) => b.insertAllOnConflictUpdate(masterGardus, data));
      });
  ...
  Future<List<MasterGardu>> cari(String kata, {String ulp = '', int limit = 500}) {
  ...
  Future<void> hapusAntrean(String gardu) =>
      (delete(garduOutboxes)..where((t) => t.gardu.equals(gardu))).go();

  Future<void> tandaiGagal(String gardu, int percobaan, String pesan) =>
      (update(garduOutboxes)..where((t) => t.gardu.equals(gardu))).write(
