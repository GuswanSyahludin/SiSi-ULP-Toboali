...
@DriftDatabase(
  tables: [
    GlobalHeaders, MasterPenyulangs, LaporanHarians, SyncInfos, P0Lokals,
    P0Outboxes, MasterGardus, GarduOutboxes, InsGarduHeaders,
    InsGarduRealisasis, InsGarduTemuans, ListTemuans,
  ],
  daos: [MasterDao, LaporanDao, SyncDao, HeaderDao, P0Dao, MasterGarduDao, InspeksiGarduDao],
)
class AppDatabase extends _$AppDatabase {
  ...
  Future<Set<String>> _columns(String table) async {
    ...
    return rows.map((row) => row.data['name']?.toString() ?? '').where((name) => name.isNotEmpty).toSet();
  }
  ...
  @override MigrationStrategy get migration => MigrationStrategy(
    beforeOpen: (_) async { await _ensureTeknikToTables(); await _ensureLocalMirrorTables(); },
    onUpgrade: (m, from, to) async {
      ...
      if (from < 3 && !tables.contains('master_gardu')) await m.createTable(masterGardus);
      tables = await _tables();
      if (from < 4 && !tables.contains('gardu_outbox')) await m.createTable(garduOutboxes);
      if (from < 5 || from < 6) {
        final columns = await _columns('master_gardu');
        if (!columns.contains('arus_max_per_fasa')) await m.addColumn(masterGardus, masterGardus.arusMaxPerFasa);
        if (!columns.contains('pembebanan_kva')) await m.addColumn(masterGardus, masterGardus.pembebananKva);
        if (!columns.contains('pembebanan_kw')) await m.addColumn(masterGardus, masterGardus.pembebananKw);
        if (!columns.contains('persentase_beban')) await m.addColumn(masterGardus, masterGardus.persentaseBeban);
        if (!columns.contains('kategori_beban')) await m.addColumn(masterGardus, masterGardus.kategoriBeban);
        if (!columns.contains('penyulang')) await m.addColumn(masterGardus, masterGardus.penyulang);
        if (!columns.contains('section')) await m.addColumn(masterGardus, masterGardus.section);
        if (!columns.contains('berat_trafo')) await m.addColumn(masterGardus, masterGardus.beratTrafo);
        if (!columns.contains('volume_minyak')) await m.addColumn(masterGardus, masterGardus.volumeMinyak);
      }
      if (from < 7) {
        final columns = await _columns('master_gardu');
        if (!columns.contains('latitude')) await m.addColumn(masterGardus, masterGardus.latitude);
        if (!columns.contains('longitude')) await m.addColumn(masterGardus, masterGardus.longitude);
      }
      if (from < 6) {
        tables = await _tables();
        if (!tables.contains('ins_gardu_header')) await m.createTable(insGarduHeaders);
        if (!tables.contains('ins_gardu_realisasi')) await m.createTable(insGarduRealisasis);
        if (!tables.contains('ins_gardu_temuan')) await m.createTable(insGarduTemuans);
        if (!tables.contains('list_temuan')) await m.createTable(listTemuans);
      }
    },
  );
}
