        ...
        child: Padding(
          ...
          child: Column(children: [
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Expanded(
                  child: Column(
                    ...
                    Text(
                        '${_show(gardu.merk)} · ${_show(gardu.kapasitasKva)} kVA',
                        style: const TextStyle(
                            fontSize: 13,
                            fontWeight: FontWeight.w700,
                            color: Color(0xFF475569))),
                    const SizedBox(height: 5),
                    Text(_show(gardu.alamat),
                        maxLines: 2,
                    ...
                    InkWell(
                      onTap: gardu.latitude.trim().isNotEmpty &&
                              gardu.longitude.trim().isNotEmpty
                      ...
                      child: Padding(
                        ...
                        child: Row(children: [
                          ...
                          if (gardu.latitude.trim().isNotEmpty &&
                              gardu.longitude.trim().isNotEmpty)
                            const Icon(Icons.open_in_new_rounded,
                                size: 12, color: AppColors.cyan600),
                        ]),
                      ),
                    ),
                    const SizedBox(height: 7),
            ...
            const Padding(
                ...
                child: Divider(height: 1, color: Color(0xFFE8ECF3))),
            Row(children: [
              Expanded(
                  child: _Metric(
                      label: 'BEBAN',
                      value: '${_show(gardu.pembebananKva)} kVA')),
              Expanded(
                  child: _Metric(
                      label: 'DAYA', value: '${_show(gardu.pembebananKw)} kW')),
              Expanded(
                  child: _Metric(
                      label: 'ARUS MAX',
                      value: '${_show(gardu.arusMaxPerFasa)} A')),
...
class _Category extends StatelessWidget {
  final String text;
  final Color color;
  const _Category({required this.text, required this.color});
  @override
...
class PendingGarduBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(20)),
        child: const Text('BELUM SINKRON',
            style: TextStyle(
                fontSize: 9,
                fontWeight: FontWeight.w900,
...
Future<void> showGarduDetailSheet(BuildContext context, MasterGardu g,
    {required bool pending, required VoidCallback onEdit}) {
            ...
            child: ListView(
                ...
                children: [
                  ...
                  _DetailSection(
                    ...
                    coordinateTap: g.latitude.trim().isNotEmpty &&
                            g.longitude.trim().isNotEmpty
                        ? () => _openGarduMaps(g)
                        : null,
                  ),
                  _DetailSection(title: 'Data Trafo', rows: [
                    ('Merk', g.merk),
                    ('Kapasitas', '${_show(g.kapasitasKva)} kVA'),
                    ('No. Seri', g.noSeri),
                    ('Tahun Trafo', g.tahunTrafo),
                    ('Type Seal', g.typeSeal)
                  ]),
                  _DetailSection(title: 'Data PHB-TR', rows: [
                    ('Merk PHB-TR', g.merkPhbTr),
                    ('Nomor Seri', g.nomorSeriPhbTr),
                    ('Tahun', g.tahunPhbTr)
                  ]),
                  _DetailSection(title: 'Pengukuran', rows: [
                    ('Jam Ukur WBP', g.jamUkurWbp),
                    ('Tanggal', g.tanggalPengukuran)
                  ]),
                  _ElectricalSection(title: 'WBP (17-21 WIB)', voltages: [
                    g.wbpRs,
                    g.wbpSt,
                    g.wbpTr,
                    g.wbpRn,
                  ...
                  _ElectricalSection(title: 'LWBP', voltages: [
                    g.lwbpRs,
                    g.lwbpSt,
                    g.lwbpTr,
                    g.lwbpRn,
              ...
              child: Container(
                ...
                child: SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: const Text('Edit Data Gardu',
...
class _LoadOverview extends StatelessWidget {
  final MasterGardu g;
  const _LoadOverview({required this.g});
  @override
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(
            child:
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _Category(text: garduCategory(g), color: color),
                  ...
                  child: r.$1 == 'Koordinat' && coordinateTap != null
                      ? InkWell(
                          ...
                          child: Row(children: [
                            ...
                            const SizedBox(width: 4),
                            const Icon(Icons.open_in_new_rounded,
                                size: 13, color: AppColors.cyan600),
                          ]),
                        )
                      : Text(_show(r.$2),
                          style: const TextStyle(
...
class _ElectricalSection extends StatelessWidget {
  final String title;
  final List<String> voltages, currents;
  const _ElectricalSection(
      {required this.title, required this.voltages, required this.currents});
  @override
  Widget build(BuildContext context) => Container(
        margin: const EdgeInsets.only(bottom: 14),
        padding: const EdgeInsets.all(16),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const Text('Tegangan (V)',
              style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800)),
          const SizedBox(height: 8),
          _MeasureGrid(
              labels: const ['R-S', 'S-T', 'T-R', 'R-N', 'S-N', 'T-N'],
              values: voltages,
              columns: 3),
          const SizedBox(height: 14),
          const Text('Beban Arus Utama (A)',
              style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800)),
          const SizedBox(height: 8),
          _MeasureGrid(
              labels: const ['R', 'S', 'T', 'N'], values: currents, columns: 4),
        ]),
      );
}
...
class _MeasureGrid extends StatelessWidget {
  ...
  final int columns;
  const _MeasureGrid(
      {required this.labels, required this.values, required this.columns});
  @override
