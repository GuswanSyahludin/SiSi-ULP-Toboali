...
class ApiService {
  ...
  static Map<String, dynamic> _decodeResponse(
    ...
    if (body.isEmpty) {
      return {
        'success': false,
        'message':
            '$operation gagal: backend mengirim respons kosong (HTTP ${response.statusCode}).',
      };
    }
    try {
      ...
      return {
        'success': false,
        'message':
            '$operation gagal: backend tidak mengirim JSON valid (HTTP ${response.statusCode}).',
      };
    }
  }
  ...
  static bool _belumAdaEndpoint(Map<String, dynamic> response) {
    final message = (response['message'] ?? '').toString().toLowerCase();
    return message.contains('tidak dikenal');
  }
  ...
  static Future<Map<String, dynamic>> loginPerangkat(
    ...
    final result = await _call('loginPerangkat', {
      'username': username,
      'password': password,
  ...
  static Future<Map<String, dynamic>> cekPerangkat() async {
    ...
    if (_belumAdaEndpoint(result)) {
      final local = await SesiStore.muat();
      final token = (local?['token'] ?? '').toString();
      if (token.isNotEmpty) {
        final legacy = await cekSesi(token);
        if (legacy['success'] == true) {
          final merged = Map<String, dynamic>.from(local!);
          final server = legacy['sesi'];
          if (server is Map) merged.addAll(Map<String, dynamic>.from(server));
          await SesiStore.simpan(merged);
  ...
  static Future<Map<String, dynamic>> login(
    ...
    String password,
      ...
      _call('login', {'username': username, 'password': password});

  static Future<Map<String, dynamic>> cekSesi(String token) => _call(
        'cekSesi',
        {'token': token},
        timeout: const Duration(seconds: 15),
      );
  ...
  static Future<Map<String, dynamic>> getDropdownRow({required String token}) =>
      _call(
        'getMobileDropdownRow',
        {'token': token},
        timeout: const Duration(seconds: 15),
      );
  ...
  static Future<Map<String, dynamic>> getApprovalP0List({
    ...
    return _call('getMobileApprovalP0List', {
  ...
  static Future<Map<String, dynamic>> setApprovalP0({
    ...
    final session = await SesiStore.muat();
    return _call('setMobileApprovalP0', {
      'token': (session?['token'] ?? '').toString(),
      'kodeP0': kodeP0,
      'keputusan': keputusan,
      'username': username,
  ...
  static Future<Map<String, dynamic>> getLampiranPengecekanP0(
    ...
    final session = await SesiStore.muat();
    return _call('getMobileLampiranPengecekanP0', {
      'token': (session?['token'] ?? '').toString(),
      'kodeP0': kodeP0,
    });
  }
  ...
  static Future<Map<String, dynamic>> getLaporanUp3Uiw({String? tanggal}) async {
    final session = await SesiStore.muat();
    return _call('getMobileLaporanUp3Uiw', {
      'token': (session?['token'] ?? '').toString(),
      'tanggal': tanggal ?? '',
    });
  }
