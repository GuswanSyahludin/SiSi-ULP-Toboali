...
class _LaporanRowScreenState extends State<LaporanRowScreen> {
  ...
  Future<void> _load() async {
    ...
    if (!mounted) return;
    final raw = response['success'] == true
    ...
    setState(() {
      ...
      _error = response['success'] == true
          ? null
          : (response['message'] ?? 'Gagal memuat laporan ROW').toString();
      _loading = false;
    });
  }
    ...
    if (_laporan.isEmpty) {
      return const Center(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(Icons.assignment_outlined, size: 54, color: AppColors.neutral400),
            SizedBox(height: 12),
            Text('Belum ada laporan ROW hari ini.', style: TextStyle(color: AppColors.neutral500, fontWeight: FontWeight.w600)),
          ],
        ),
      );
    }
  ...
  void _bukaFormTambahLaporan() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => _FormInputLaporanHeaderScreen(
          sesi: widget.sesi,
          judulMenu: 'Tambah Laporan Harian ROW',
          subTim: _subTim,
          onSimpan: (dataBaru) {
            setState(() {
...
class _FormInputLaporanHeaderScreen extends StatefulWidget {
  final Map<String, dynamic> sesi;
  final String judulMenu;
  final String subTim;
  final Function(Map<String, dynamic>) onSimpan;

  const _FormInputLaporanHeaderScreen({
    required this.sesi,
  ...
  @override
  State<_FormInputLaporanHeaderScreen> createState() =>
      _FormInputLaporanHeaderScreenState();
}

class _FormInputLaporanHeaderScreenState
    extends State<_FormInputLaporanHeaderScreen> {
  ...
  void _simpan() {
    ...
    final data = {
      'kodeHeader': 'DRAFT-${DateTime.now().millisecondsSinceEpoch.toString().substring(8)}',
      'hari': hari,
      'tanggal': today,
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: const Icon(Icons.badge_outlined, color: AppColors.navy700, size: 22),
                ),
