...
class _TemuanState extends State<InsGarduTemuanEditor> { final repo=InspeksiGarduRepository(),picker=ImagePicker(); String tier='Tier 1'; List<ListTemuan> options=[]; final Map<String,Map<String,String>> selected={}; @override void initState(){super.initState();_load();} Future<void> _load()async{options=await repo.pilihan(tier);if(mounted)setState((){});} Future<void> _save()async{final rows=selected.entries.map((e)=>{'tier':tier,'temuan':e.key,...e.value}).toList();await repo.setGardu(gardu:widget.g,tier:tier,temuan:rows);if(mounted)Navigator.pop(context,true);} @override Widget build(BuildContext context)=>Scaffold(appBar:AppBar(title:Text(widget.g.nomorGardu),actions:[TextButton(onPressed:_save,child:const Text('SIMPAN'))]),body:ListView(children:[DropdownButtonFormField<String>(initialValue:tier,items:['Tier 1','Tier 2','Tier 1 & Tier 2'].map((x)=>DropdownMenuItem(value:x,child:Text(x))).toList(),onChanged:(v){tier=v!;_load();}),...options.map((o)=>CheckboxListTile(value:selected.containsKey(o.temuan),title:Text(o.temuan),onChanged:(v)=>setState((){if(v==true)selected[o.temuan]={};else selected.remove(o.temuan);}))) ])); }
