...
Future<void> mount(WidgetTester tester, GangguanLoader loader) =>
    tester.pumpWidget(MaterialApp(
      home: Scaffold(
        body: SingleChildScrollView(
          child: Padding(
            padding: const EdgeInsets.all(16),
            child: GangguanBerandaSection(
              sesi: const {'token': 'test', 'ulp': 'Toboali'},
              loader: loader,
...
void main() {
  ...
  test('counts handle numeric strings and reject invalid negatives', () {
    final c = GangguanViewData.counts({
      'series': [
  ...
  testWidgets('bar selection shows inline detail without source chips',
      (tester) async {
    await mount(
      tester,
      ({required token, required from, required to, String ulp = ''}) async =>
          response(),
    );
    await tester.pumpAndSettle();
    expect(find.byKey(const ValueKey('source-ar')), findsNothing);
  ...
  testWidgets('late request cannot overwrite a newer period', (tester) async {
    ...
    await mount(tester,
        ({required token, required from, required to, String ulp = ''}) {
    ...
    requests[1].complete(response(feeder: 'Sadai', date: '2026-09-06'));
    await tester.pumpAndSettle();
    requests[0].complete(response(feeder: 'Lama'));
    await tester.pumpAndSettle();
    expect(find.text('Sadai'), findsOneWidget);
  ...
  testWidgets('failed fetch is not presented as zero events', (tester) async {
    await mount(
      tester,
      ({required token, required from, required to, String ulp = ''}) async =>
          {'ok': false, 'message': 'Koneksi gagal'},
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('missing source total is not manufactured as zero',
      (tester) async {
    await mount(
      tester,
      ({required token, required from, required to, String ulp = ''}) async =>
        ...
        'ok': true,
        'datasets': [
          {'key': 'gangguan', 'title': 'Data Gangguan', 'series': [], 'details': []}
        ]
      },
    );
    await tester.pumpAndSettle();
    expect(find.text('Belum ada ringkasan dari sumber ini'), findsOneWidget);
    expect(find.text('0 catatan'), findsNothing);
  });
