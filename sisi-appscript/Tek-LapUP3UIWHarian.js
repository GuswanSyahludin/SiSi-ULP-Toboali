...
var _LAP_HARI  = ['Minggu','Senin','Selasa','Rabu','Kamis','Jumat','Sabtu'];
var _LAP_BULAN = ['Januari','Februari','Maret','April','Mei','Juni','Juli',
                  'Agustus','September','Oktober','November','Desember'];

/* ===== Helper format ===== */
function _lapTglIndo(tglIso){
  var d = new Date(tglIso + 'T00:00:00');
  return _LAP_HARI[d.getDay()] + ', ' + d.getDate() + ' ' + _LAP_BULAN[d.getMonth()] + ' ' + d.getFullYear();
}
function _lapUlpNama(ulp){ return String(ulp||'').trim().replace(/^ULP\s+/i,''); }
...
function _lapAngka(v, satuan){
  if(v===null || v===undefined || v==='' || Number(v)===0) return '- ' + satuan;
  return v + ' ' + satuan;
}
...
function _lapPenyBlok(nama, baris){
  ...
  for(var i=0;i<baris.length;i++){
    var b = baris[i];
    L.push('- ' + b.label + ' : ' + _lapAngka(b.target, b.satuan) + ' / ' + _lapAngka(b.rlz, b.satuan));
  }
  return L.join('\n');
}
...
function _lapBagiTarget(total, n){
  var out = [];
  if(n<=0) return out;
  var base = Math.floor(total / n), sisa = total - base * n;
...
function _lapC4A(c4a){
  ...
  if(typeof c4a === 'string'){ var s = c4a.trim(); return s ? s : '- Nihil'; }
...
function _lapUlpMap_(ss){
  var sh = ss.getSheetByName(LAP_UP3.HEADER), H = COL_INS.HEADER, map = {};
...
function _lapDataROW_(ss, ulp, tglIso){
  var R = COL_ROW_RLZ;
  var ulpMap = _lapUlpMapDual_(), bulan = tglIso.substring(0,7);
  var hari = {}, kom = { rabas:0, tebang:0 };
  ...
  for(var i=0;i<d.length;i++){
    ...
    kom.rabas += rabas; kom.tebang += tebang;
    if(t === tglIso){
      var p = String(d[i][R.penyulang]||'').trim() || '-';
      if(!hari[p]) hari[p] = { rabas:0, tebang:0 };
      hari[p].rabas += rabas; hari[p].tebang += tebang;
    }
  }
...
function _lapDataHartek_(ss, ulp, tglIso){
  var sh = ss.getSheetByName(LAP_UP3.HTK_PG), C = COL_HTK.PG;
  var bulan = tglIso.substring(0,7);
  var hari = {}, kom = { jaringan:0, gardu:0 };
  if(sh && sh.getLastRow()>1){
    var d = sh.getDataRange().getValues();
    for(var i=1;i<d.length;i++){
      ...
      if(t.substring(0,7) !== bulan) continue;
      var jenis = String(d[i][C.jenisPekerjaan]||'').trim().toLowerCase();
      var isJaringan  = (jenis === 'jaringan');
      var isNonTeknik = (jenis.indexOf('non')>=0 && jenis.indexOf('teknik')>=0);
      var p = String(d[i][C.penyulang]||'').trim() || '-';
      if(isJaringan){
        var gw = Number(d[i][C.jumlahGawang]) || 0;
        kom.jaringan += gw;
        if(t === tglIso){ if(!hari[p]) hari[p]={jaringan:0,gardu:0}; hari[p].jaringan += gw; }
      } else if(!isNonTeknik){
        kom.gardu += 1; // 1 baris gardu = 1 Unit
        if(t === tglIso){ if(!hari[p]) hari[p]={jaringan:0,gardu:0}; hari[p].gardu += 1; }
      }
    }
  }
  return { hari:hari, komulatif:kom };
...
function _lapDataInspeksi_(ss, ulp, tglIso){
  var ulpMap = _lapUlpMapDual_(), bulan = tglIso.substring(0,7);
  var hari = {}, kom = { jaringan:0, gardu:0 };
  function _ensure(p){ if(!hari[p]) hari[p] = { jaringan:0, gardu:0 }; return hari[p]; }

  var shJ = ss.getSheetByName(LAP_UP3.INSJAR);
  if(shJ && shJ.getLastRow()>1){
    var RJ = COL_INS.REALISASI, dj = shJ.getDataRange().getValues();
    for(var i=1;i<dj.length;i++){
      ...
      var isTier2 = (String(dj[i][RJ.tier]||'').trim().toLowerCase().indexOf('tier 2') >= 0);
      ...
      if(effDay.substring(0,7) === bulan && effDay <= tglIso) kom.jaringan += gw;
      // HARIAN: hari efektif == tanggal laporan.
      if(effDay === tglIso) _ensure(String(dj[i][RJ.penyulang]||'').trim() || '-').jaringan += gw;
    }
  }

  var shG = ss.getSheetByName(LAP_UP3.INSDU);
  if(shG && shG.getLastRow()>1){
    var RG = COL_INSDU.REALISASI, dg = shG.getDataRange().getValues();
    for(var k=1;k<dg.length;k++){
      ...
      if(!String(dg[k][RG.nomorGardu]||'').trim() && !String(dg[k][RG.kodePekerjaanGardu]||'').trim()) continue;
      kom.gardu += 1;
      if(t2 === tglIso) _ensure(String(dg[k][RG.penyulang]||'').trim() || '-').gardu += 1;
    }
  }
  return { hari:hari, komulatif:kom };
}
...
function _lapP0KriteriaOk_(nama){
  var s = String(nama||'').toLowerCase().replace(/\s+/g,' ').trim();
...
function _lapShiftTagMap_(ss){
  var sh = ss.getSheetByName(LAP_UP3.YANDAL_SHIFT), S = COL_YANDAL_SHIFT, map = {};
  if(sh && sh.getLastRow()>1){
    var d = sh.getDataRange().getValues();
    for(var i=1;i<d.length;i++){
...
function _lapShiftTagDariP0_(rowP0, shiftMap){
  ...
  if(up.indexOf('SHF1')>=0 || up.indexOf('SHIFT-1')>=0 || up.indexOf('SHIFT1')>=0) return 'SHF1';
  if(up.indexOf('SHF2')>=0 || up.indexOf('SHIFT-2')>=0 || up.indexOf('SHIFT2')>=0) return 'SHF2';
  if(up.indexOf('SHF3')>=0 || up.indexOf('SHIFT-3')>=0 || up.indexOf('SHIFT3')>=0) return 'SHF3';
  return '';
}
...
function _lapDataP0_(ss, ulp, tglIso){
  var sh = ss.getSheetByName(LAP_UP3.YANDAL_P0), C = COL_P0, n = 0;
  if(sh && sh.getLastRow()>1){
...
function buildLaporanUP3(tanggal, ulp, opts){
  ...
  for(var i=0;i<rowPeny.length;i++){
    ...
    L.push(_lapPenyBlok(rowPeny[i], [
      { label:'Rabas',  satuan:'Gawang', target:tgtRabas[i],  rlz:h.rabas },
      { label:'Tebang', satuan:'Batang', target:tgtTebang[i], rlz:h.tebang }
    ]));
    L.push('');
  }
  if(rowPeny.length === 0){ // tidak ada realisasi -> tetap tampil, nilai "-"
    L.push(_lapPenyBlok(null, [
  ...
  for(var j=0;j<htkPeny.length;j++){
    ...
    L.push(_lapPenyBlok(htkPeny[j], [
      { label:'Gardu',    satuan:'Unit',   target:hh.gardu,    rlz:hh.gardu },
      { label:'Jaringan', satuan:'Gawang', target:hh.jaringan, rlz:hh.jaringan }
    ]));
    L.push('');
  }
  if(htkPeny.length === 0){ // tidak ada realisasi -> tetap tampil, nilai "-"
    L.push(_lapPenyBlok(null, [
  ...
  for(var k=0;k<insPeny.length;k++){
    ...
    L.push(_lapPenyBlok(insPeny[k], [
      { label:'Gardu',    satuan:'Unit',   target:garduTgt,    rlz:ih.gardu },
      { label:'Jaringan', satuan:'Gawang', target:ih.jaringan, rlz:ih.jaringan }
    ]));
    L.push('');
  }
  if(insPeny.length === 0){ // tidak ada realisasi -> tetap tampil, nilai "-"
    L.push(_lapPenyBlok(null, [
      { label:'Gardu',    satuan:'Unit',   target:null, rlz:null },
      { label:'Jaringan', satuan:'Gawang', target:null, rlz:null }
    ]));
...
function debugLaporanUP3(){
  ...
  Logger.log(buildLaporanUP3(tgl, 'Toboali', { c4a:'Nihil', tindakLanjut:'Nihil' }));
}
...
function _wilTimLabel(tim){
  var m = String(tim||'').match(/(\d+)/);
  return m ? ('TIM ROW ' + ('0'+m[1]).slice(-2)) : ('TIM ' + String(tim||'').toUpperCase());
}
...
function _wilTemuanByPenyulang_(ss, ulp, tglIso){
  var sh = ss.getSheetByName(LAP_WIL.TEMUAN), T = COL_INS.TEMUAN, map = {};
  if(sh && sh.getLastRow()>1){
    var d = sh.getDataRange().getValues();
    for(var i=1;i<d.length;i++){
      if(String(d[i][T.ulp]||'').trim() !== ulp) continue;
      if(_normTgl(d[i][T.tanggal]) !== tglIso) continue;
      if(String(d[i][T.objekInspeksi]||'').trim().toLowerCase() !== 'jaringan') continue;
      var nama = String(d[i][T.temuan]||'').trim();
      if(!nama) continue;
      var key = String(d[i][T.penyulang]||'').trim().toLowerCase();
      if(!map[key]) map[key] = { peralatan:0, row:0 };
      if(_wilIsTemuanRow(nama)) map[key].row++; else map[key].peralatan++;
    }
  }
  return map;
}
...
function _wilDataCareForAsset_(ss, ulp, tglIso){
  var sh = ss.getSheetByName(LAP_WIL.INSJAR), RJ = COL_INS.REALISASI;
  var ulpMap = _lapUlpMapDual_(), agg = {}, order = [];
  if(sh && sh.getLastRow()>1){
    var d = sh.getDataRange().getValues();
    for(var i=1;i<d.length;i++){
      ...
      if(!agg[p]){ agg[p] = { tiang:0, sections:[], seen:{} }; order.push(p); }
      agg[p].tiang += Number(d[i][RJ.totalTiang]) || 0;
      var sec = String(d[i][RJ.section]||'').trim();
      if(sec && !agg[p].seen[sec]){ agg[p].seen[sec]=1; agg[p].sections.push(sec); }
    }
  }
  var temuanMap = _wilTemuanByPenyulang_(ss, ulp, tglIso);
  var out = [];
  for(var k=0;k<order.length;k++){
    var p = order[k], a = agg[p];
    var tem = temuanMap[p.toLowerCase()] || { peralatan:0, row:0 };
    out.push({
      penyulang: p,
      section:   (a.sections.length>1) ? _sectionRange(p, a.sections) : (a.sections[0]||'-'),
      tiang:     a.tiang,
...
function _wilDataRow_(ss, ulp, tglIso){
  var R = COL_ROW_RLZ;
  var ulpMap = _lapUlpMapDual_(), tims = {}, order = [];
  ...
  for(var i=0;i<d.length;i++){
    ...
    if(!tims[tim]){ tims[tim] = { peny:{}, order:[] }; order.push(tim); }
    var p = String(d[i][R.penyulang]||'').trim() || '-';
    if(!tims[tim].peny[p]){ tims[tim].peny[p] = { rabas:0, tebang:0, sections:[], seen:{} }; tims[tim].order.push(p); }
    ...
    if(sec && !ob.seen[sec]){ ob.seen[sec]=1; ob.sections.push(sec); }
  }
  order.sort(function(a,b){ var na=_wilTimNum(a), nb=_wilTimNum(b); return (na!==nb)?(na-nb):a.localeCompare(b); });
  return { tims:tims, order:order };
}

// Ada realisasi Hartek hari itu? (untuk hitung Tim Hartek)
function _wilHartekAda_(ss, ulp, tglIso){
  var sh = ss.getSheetByName(LAP_WIL.HTK_PG), C = COL_HTK.PG;
  if(sh && sh.getLastRow()>1){
    var d = sh.getDataRange().getValues();
    for(var i=1;i<d.length;i++){
      if(String(d[i][C.ulp]||'').trim() !== ulp) continue;
...
function _wilDataHartek_(ss, ulp, tglIso){
  var sh = ss.getSheetByName(LAP_WIL.HTK_PG), C = COL_HTK.PG;
  var penyOrder = [], penySeen = {}, sectionByPeny = {};
  var jaringan = [], gardu = [], nonTeknik = [];
  if(sh && sh.getLastRow()>1){
    var d = sh.getDataRange().getValues();
    for(var i=1;i<d.length;i++){
      if(String(d[i][C.ulp]||'').trim() !== ulp) continue;
      ...
      if(pen){
        ...
        if(!penySeen[pk]){ penySeen[pk]=true; penyOrder.push(pen); }
      ...
      else if(isNonTeknik) nonTeknik.push(entry);   // Non-Teknik -> seksi tersendiri
  ...
  return { penyulang:penyOrder.join(', '), section:secList.join(', '), jaringan:jaringan, gardu:gardu, nonTeknik:nonTeknik };
}
...
function _wilRenderHartekSeksi_(judul, arr, withGardu){
  ...
  for(var i=0;i<arr.length;i++){
    ...
    if(e.body){ b.push('Pekerjaan :'); b.push(e.body); }
    else { b.push('Pekerjaan : -'); b.push('Material : -'); }
    blocks.push(b.join('\n'));
  }
  L.push(blocks.join('\n\n'));
  return L.join('\n');
...
function buildLaporanWilayah(tanggal, ulp, opts){
  ...
  if(rowData.order.length){
    for(var t=0;t<rowData.order.length;t++){
      var timRaw = rowData.order[t], tim = rowData.tims[timRaw];
      L.push('');
      L.push('🔹 ' + _wilTimLabel(timRaw));
      for(var p=0;p<tim.order.length;p++){
        if(p>0) L.push('');
        var pn = tim.order[p], ob = tim.peny[pn];
        var sec = (ob.sections.length>1) ? _sectionRange(pn, ob.sections) : (ob.sections[0]||'-');
        L.push('- Penyulang : ' + pn);
        L.push('- Section : ' + sec);
        L.push('- Realisasi');
  ...
  L.push(_WIL_SEP);
  L.push(_wilRenderHartekSeksi_('🔧 PEKERJAAN JARINGAN', hartek.jaringan, false));
  L.push(_WIL_SEP);
  L.push(_wilRenderHartekSeksi_('📌 PEKERJAAN GARDU', hartek.gardu, true));
  L.push(_WIL_SEP);
  L.push(_wilRenderHartekSeksi_('📋 PEKERJAAN NON-TEKNIK', hartek.nonTeknik, true));
  L.push(_WIL_SEP);
...
function _lhToday(){ return Utilities.formatDate(new Date(), LH.TZ, 'yyyy-MM-dd'); }
...
function _lhSheet(ss){
  ...
  if(!sh){
    ...
    sh.appendRow(['No','Tanggal','Penyulang','Panjang kmS Inspeksi','Temuan','Eksekusi','Laporan UP3','Laporan UIW']);
  }
  return sh;
}
...
function _lhFindRow(sh, tglIso){
  ...
  for(var i=0;i<col.length;i++){ if(_normTgl(col[i][0]) === tglIso) return i + 2; }
  return 0;
}
...
function _lhEnsureRow(sh, tglIso){
  ...
  if (typeof _tglSudahDiarsip_ === 'function' && _tglSudahDiarsip_(tglIso)) return 0;
...
function _lhRecalcRow(ss, sh, rowNum, opts){
  ...
  var up3 = buildLaporanUP3(tglIso, ulp, { c4a: c4a, tindakLanjutGangguan: opts.tindakLanjutGangguan });
...
function ensureLaporanHarianHariIni(){
  var ss = SpreadsheetApp.openById(SPREADSHEET_ID), sh = _lhSheet(ss);
...
function simpanLaporanHarianWeb(params){
  try{
    ...
    var today  = _lhToday();
    if(tglIso !== today){
      return { ok:false, readOnly:true, message:'Pengeditan hanya untuk hari ini ('+today+'). Tanggal lain hanya dapat dilihat.' };
    }
    var ss = SpreadsheetApp.openById(SPREADSHEET_ID), sh = _lhSheet(ss);
    ...
    sh.getRange(rowNum, LH.COL.penyulang+1).setValue(String(c.penyulang||'').trim());
    sh.getRange(rowNum, LH.COL.panjang+1).setValue(String(c.realisasi||'').trim());
    sh.getRange(rowNum, LH.COL.temuan+1).setValue(String(c.temuan||'').trim());
    sh.getRange(rowNum, LH.COL.eksekusi+1).setValue(String(c.eksekusi||'').trim());
    var res = _lhRecalcRow(ss, sh, rowNum, { ulp: params.ulp, cuaca: params.cuaca, tindakLanjutGangguan: params.tindakLanjutGangguan });
...
function getLaporanHarianRow(params){
  try{
    ...
    if (!rowNum && typeof SPREADSHEET_ID_ARSIP !== 'undefined' && SPREADSHEET_ID_ARSIP) {
      try {
        var shA = SpreadsheetApp.openById(SPREADSHEET_ID_ARSIP).getSheetByName(LH.SHEET);
        if (shA) { var rA = _lhFindRow(shA, tglIso); if (rA) { sh = shA; rowNum = rA; sumber = 'arsip'; } }
      } catch (eA) { Logger.log('getLaporanHarianRow: fallback arsip gagal — ' + eA); }
    }
    var c4a = { penyulang:'', realisasi:'', temuan:'', eksekusi:'' }, up3='', uiw='';
    if(rowNum){
      var v = sh.getRange(rowNum, 1, 1, 8).getValues()[0];
      c4a = _lhC4aFromRow(v);
    ...
    return { ok:true, tanggal: tglIso, editable:(tglIso===today && sumber==='aktif'), exists:!!rowNum, sumber:sumber, c4a:c4a, waText:up3, waTextUiw:uiw };
  }catch(e){ return { ok:false, message:e.message }; }
}
...
function refreshLaporanHarian(params){
  try{
    ...
    if (typeof _tglSudahDiarsip_ === 'function' && _tglSudahDiarsip_(tglIso)) {
      return { ok:true, skipped:'diarsip', tanggal: tglIso, message:'Tanggal <= H-2 sudah diarsip — tidak dibuat ulang di file aktif.' };
    }
    var ss = SpreadsheetApp.openById(SPREADSHEET_ID), sh = _lhSheet(ss);
    var rowNum = _lhEnsureRow(sh, tglIso);
    if (!rowNum) return { ok:true, skipped:'diarsip', tanggal: tglIso };
    var res = _lhRecalcRow(ss, sh, rowNum, { ulp: params.ulp, cuaca: params.cuaca });
    return { ok:true, tanggal: tglIso, waText: res.up3, waTextUiw: res.uiw };
  }catch(e){ return { ok:false, message:e.message }; }
}

// Refresh khusus HARI INI (utk trigger berkala "selalu update saat tim ubah data").
function refreshLaporanHarianHariIni(){ return refreshLaporanHarian({ tanggal: _lhToday() }); }
...
function markLaporanDirty_(tglIso){
  ...
  try { lock.waitLock(2000); } catch(e) {}
  try {
    var props = PropertiesService.getScriptProperties();
    var map = {};
    try { map = JSON.parse(props.getProperty(LAPORAN_DIRTY_PROP) || '{}'); } catch(e2) { map = {}; }
    map[tglIso] = Date.now();
...
function drainLaporanDirty(){
  var props = PropertiesService.getScriptProperties();
  var map = {};
  try { map = JSON.parse(props.getProperty(LAPORAN_DIRTY_PROP) || '{}'); } catch(e) { map = {}; }
  ...
  for(var j=0;j<dd.length;j++){
    try { refreshLaporanHarian({ tanggal: dd[j] }); }   // build UP3 + UIW lalu tulis sheet
    catch(eRun) { gagal[dd[j]] = Date.now(); }
  }
  ...
  if(Object.keys(gagal).length){
    ...
    try {
      var cur = {};
      try { cur = JSON.parse(props.getProperty(LAPORAN_DIRTY_PROP) || '{}'); } catch(e3) { cur = {}; }
      var gk = Object.keys(gagal);
      for(var g=0;g<gk.length;g++) if(!cur[gk[g]]) cur[gk[g]] = gagal[gk[g]];
...
function createLaporanDrainTrigger(){
  ...
  for(var i=0;i<all.length;i++){
    if(all[i].getHandlerFunction() === 'drainLaporanDirty') ScriptApp.deleteTrigger(all[i]);
  }
  ScriptApp.newTrigger('drainLaporanDirty').timeBased().everyMinutes(1).create();
...
function hapusLaporanDrainTrigger(){
  ...
  var n = 0;
  for(var i=0;i<all.length;i++){
    if(all[i].getHandlerFunction() === 'drainLaporanDirty'){ ScriptApp.deleteTrigger(all[i]); n++; }
  }
...
function _lhHandleWebhook(body){
  body = body || {};
  if(String(body.secret||'') !== WEBHOOK_SECRET) return { ok:false, message:'Unauthorized' };
...
function pasangTriggerLaporanHarian(){
  ScriptApp.getProjectTriggers().forEach(function(t){
    ...
    if(f==='ensureLaporanHarianHariIni' || f==='refreshLaporanHarianHariIni' || f==='drainLaporanDirty') ScriptApp.deleteTrigger(t);
  });
  // 00:00-01:00: buat baris hari baru
  ScriptApp.newTrigger('ensureLaporanHarianHariIni').timeBased().atHour(0).everyDays(1).inTimezone(LH.TZ).create();
  ...
  ScriptApp.newTrigger('refreshLaporanHarianHariIni').timeBased().everyMinutes(15).create();
}