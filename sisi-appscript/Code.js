/* ═════════════════════════════════════
   ...
   Rev: 31 Mei 2026 (konsolidasi bersih + modul Inspeksi)
...
var FULL_ACCESS_PAGES = [
  'Tek-Dashboard','Tek-ROW','Tek-Inspeksi',
  'Tek-InsDu','Tek-PengukuranGardu','Tek-InsJar','Temuan-Inspeksi',
  'Tek-Yandal','Tek-Hartek','Tek-Gangguan','SIE-Teknik'
];
...
var COL_USERS = {
  no: 0, email: 1, userName: 2, password: 3,
  role: 4, ulp: 5, kodeUlp: 6, bidang: 7, tim: 8, subTim: 9, aksesMenu: 10
};
...
const COL_INS = {
  ...
  REALISASI: { kodeHeader:1, kodePekerjaanPeny:2, hari:3, tanggal:4,
               penyulang:5, section:6, segmen:7, tier:8,
               totalTiang:9, jumlahTemuan:10, inputBy:11, timestamp:12 },
  ...
  TEMUAN: {
    no:0, kodeHeader:1, kodePekerjaanPeny:2, kodePekerjaan:3, ulp:4,
    hari:5, tanggal:6, timInspeksi:7, objekInspeksi:8, penyulang:9,
    section:10, segmen:11, nomorTiang:12, nomorGardu:13, tier:14,
    temuan:15, fotoTemuan:16, fotoTemuanUrl:17, fotoTiang:18, fotoTiangUrl:19,
    deskripsi:20, koordinat:21, lat:22, long:23, inputBy:24, timestamp:25,
    status:26, forwardBy:27, tglForward:28, timEksekusi:29, catatan:30,
    diameter:31, jenisPekerjaan:32, fotoPekerjaan:33, fotoPekerjaanUrl:34,
    fotoSesudah:35, fotoSesudahUrl:36, tglSelesai:37, petugas:38,
    inputBySelesai:39, timestampSelesai:40, tampilFotoTemuan:41,
    tampilFotoPekerjaan:42, tampilFotoSesudah:43, folderPath:44
  },
};
...
function _cacheIns(key, ttlSec, producer) {
  ...
  const val = producer();
  try { cache.put(key, JSON.stringify(val), ttlSec); } catch (e) {}
  return val;
}
...
function _findRowTemuan(kodePekerjaan) {
  ...
  const idx = data.findIndex(function (r) {
    return String(r[COL_INS.TEMUAN.kodePekerjaan]).trim() === String(kodePekerjaan).trim();
  });
...
function doGet(e) {
  ...
  if (e && e.parameter && e.parameter.pdf) {
    ...
    if (String(e.parameter.pdf).trim().toLowerCase() === 'gardu') return unduhPdfInsGardu(e);
  ...
  if (!token) {
    ...
    t1.error = ''; t1.scriptUrl = svcUrl;
    return t1.evaluate().setXFrameOptionsMode(HtmlService.XFrameOptionsMode.DEFAULT);
  }

  var sesi = getSesiByToken(token);
  if (!sesi) {
    var t2 = HtmlService.createTemplateFromFile('login-page');
    t2.error = 'Sesi habis, silakan login ulang.'; t2.scriptUrl = svcUrl;
    return t2.evaluate().setXFrameOptionsMode(HtmlService.XFrameOptionsMode.DEFAULT);
  }
  ...
  try { CacheService.getUserCache().put('userToken', token, SESSION_TTL_SEC); } catch (e) {}
  ...
  try {
    ...
    if (action === 'recalcRow') {
      ...
      if (!kodeHeader) {
        ...
        if (cache.get(dbKeyW)) {
          out = { ok: true, action: action, mode: 'sync', skipped: true, reason: 'debounce', kodeHeader: kodeHeader };
        } else {
          cache.put(dbKeyW, '1', 8);
          var hasilW = recalcWaByHeader(kodeHeader);                  // PROSES LANGSUNG (sinkron)
          out = { ok: true, action: action, mode: 'sync', kodeHeader: kodeHeader, hasil: hasilW };
        }
      }
      ...
      if (cache.get('wh_refreshwa')) {
        ...
        var hasilR = (typeof refreshWaHarian === 'function')
          ? refreshWaHarian()
      ...
      if (cache.get('wh_validasifoto')) {
        ...
        var hasilV = (typeof validasiUlangFotoTemuan === 'function')
      ...
      if (typeof markLaporanDirty_ === 'function') {
        ...
        out = { ok: true, action: action, mode: 'queued', queued: true, tanggal: tandaLap };
      } else {
        out = { ok: false, message: 'markLaporanDirty_ tidak tersedia; pasang fungsi antrean laporan (Tek-LaporanHarianSheet.gs) & jalankan createLaporanDrainTrigger() dulu.' };
      }
      ...
      if (!kodeP0) {
        ...
        out = { ok: true, action: action, mode: 'queued', queued: antre, kodeP0: kodeP0, foto: fotoTg };
      } else {
        out = { ok: false, message: 'enqueueP0Yandal_ tidak tersedia; jalankan createWmDrainTriggerY dulu agar antrean WM diproses.' };
      }
      ...
      if (!kodeSwc) {
        out = { ok: false, message: 'prosesSwitchingYandal butuh kodeSwitching' };
      } else if (typeof enqueueP0Yandal_ === 'function') {
        var antreSwc = enqueueP0Yandal_(kodeSwc, fotoSwc);           // antrean sama; drainAntreanP0 arahkan ke prosesSwitchingYandal (kode -SWT.)
        out = { ok: true, action: action, mode: 'queued', queued: antreSwc, kodeSwitching: kodeSwc, foto: fotoSwc };
      } else {
        out = { ok: false, message: 'enqueueP0Yandal_ tidak tersedia; jalankan createWmDrainTriggerY dulu agar antrean WM diproses.' };
      }
      ...
      if (!kodePekH) {
        out = { ok: false, message: 'prosesHartekPekerjaan butuh kodePekerjaan' };
      ...
      if (!kodeGndH) {
        out = { ok: false, message: 'prosesHartekHarGrounding butuh kodeHarGrounding' };
      ...
      if (!kodePmrH) {
        out = { ok: false, message: 'prosesHartekPemerataanBeban butuh kodePemerataan' };
      } else if (typeof prosesHartekPemerataanBeban === 'function') {
        out = prosesHartekPemerataanBeban(kodePmrH);
      } else {
        out = { ok: false, message: 'prosesHartekPemerataanBeban tidak tersedia' };
      }
...
var RQ = { jenis: 0, key: 1, tim: 2, tanggal: 3, kodeHeader: 4, dirtyAt: 5, status: 6, lastTriedAt: 7, attempts: 8 };
var RECALC_QUEUE_COLS   = 9;
var RECALC_QUEUE_HEADER = ['jenis', 'key', 'tim', 'tanggal', 'kodeHeader', 'dirtyAt', 'status', 'lastTriedAt', 'attempts'];
...
function markRecalcRowDirty_(tim, tanggal) {
  ...
  return _enqueueRecalc_({ jenis: 'row', key: 'row|' + tim + '|' + tanggal, tim: tim, tanggal: tanggal });
}
...
function markWaDirty_(kodeHeader) {
  ...
  return _enqueueRecalc_({ jenis: 'wa', key: 'wa|' + kodeHeader, kodeHeader: kodeHeader });
}
...
function recalcTick() {
  ...
  try {
    ...
    var byKey = {};
    for (var r = 0; r < data2.length; r++) byKey[String(data2[r][RQ.key]).trim()] = { row: r + 2, v: data2[r] };
    ...
    for (var k = 0; k < claimed.length; k++) {
      ...
      if (curDirty > c.dirtyAt) {
        ...
        sh.getRange(hit.row, RQ.status + 1).setValue(att >= RECALC_MAX_ATTEMPTS ? 'failed' : 'pending');
      }
    }
...
function createRecalcTrigger() {
  ...
  for (var i = 0; i < all.length; i++) {
    ...
    if (fn === 'recalcTick' || fn === 'recalcRowTick') ScriptApp.deleteTrigger(all[i]);
  }
...
function hapusRecalcTrigger() {
  ...
  for (var i = 0; i < all.length; i++) {
    var fn = all[i].getHandlerFunction();
    if (fn === 'recalcTick' || fn === 'recalcRowTick') { ScriptApp.deleteTrigger(all[i]); n++; }
  }
...
function recalcRowTick() { return recalcTick(); }
function createRecalcRowTrigger() { return createRecalcTrigger(); }
function hapusRecalcRowTrigger()  { return hapusRecalcTrigger(); }
...
function createRefreshWaTrigger() {
  var all = ScriptApp.getProjectTriggers();
  for (var i = 0; i < all.length; i++) {
    if (all[i].getHandlerFunction() === 'refreshWaHarian') ScriptApp.deleteTrigger(all[i]);
  }
  ScriptApp.newTrigger('refreshWaHarian').timeBased().everyMinutes(REFRESH_WA_INTERVAL_MIN).create();
  return 'Trigger backstop refreshWaHarian dipasang (tiap ' + REFRESH_WA_INTERVAL_MIN + ' menit).';
}
...
function hapusRefreshWaTrigger() {
  ...
  var n = 0;
  for (var i = 0; i < all.length; i++) {
    if (all[i].getHandlerFunction() === 'refreshWaHarian') { ScriptApp.deleteTrigger(all[i]); n++; }
  }
...
function doLogin(username, password) {
  try {
    ...
    if (!sh) return { success: false, message: 'Sheet db_Users tidak ditemukan' };
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      if (uPw !== password) return { success: false, message: 'Password salah' };

      var token = Utilities.getUuid();
      var sesi  = {
        token: token, username: uNm, email: eml, role: role,
        ulp: ulp, kodeUlp: kodeUlp, bidang: bidang, tim: tim, subTim: subTim,
        aksesMenu: aksesMenu, loginAt: new Date().toISOString()
      };

      CacheService.getScriptCache().put('sesi_' + token, JSON.stringify(sesi), SESSION_TTL_SEC);
      CacheService.getUserCache().put('userToken', token, SESSION_TTL_SEC);

      return { success: true, token: token, username: uNm, email: eml,
               role: role, ulp: ulp, kodeUlp: kodeUlp, bidang: bidang, tim: tim, subTim: subTim,
               aksesMenu: aksesMenu };
    }
...
function getPageContent(token, pageName) {
  try {
    ...
    return {
      ...
      sesi: { username: sesi.username, email: sesi.email, role: sesi.role,
              ulp: sesi.ulp, kodeUlp: sesi.kodeUlp, bidang: sesi.bidang, tim: sesi.tim, subTim: sesi.subTim || '',
...
function _pagesByBidang(bidang) {
  switch (String(bidang || '').trim()) {
    case 'Teknik': return FULL_ACCESS_PAGES;
    case 'PP':     return PP_PAGES;
    case 'TE':     return TE_PAGES;
    case 'K3':     return K3_PAGES;
    default:       return [];
  }
}
...
function _findRowAkun(sh, username) {
  ...
  for (var i = 1; i < data.length; i++) {
    if (String(data[i][COL_USERS.userName] || '').trim().toLowerCase() === target) return i + 1;
  }
  return -1;
}
...
function tambahAkun(token, data) {
  ...
  if (!String(data.password || '').trim()) return { ok: false, message: 'Password wajib diisi.' };
  if (_findRowAkun(sh, uname) !== -1) return { ok: false, message: 'Username sudah dipakai.' };

  var akses = Array.isArray(data.aksesMenu) ? data.aksesMenu.join(', ') : String(data.aksesMenu || '');
  var baris = [];
...
function updateAkun(token, data) {
  ...
  if (row === -1) return { ok: false, message: 'Akun tidak ditemukan: ' + target };

  var unameBaru = String(data.username || '').trim() || target;
  if (unameBaru.toLowerCase() !== target.toLowerCase() && _findRowAkun(sh, unameBaru) !== -1)
    return { ok: false, message: 'Username baru sudah dipakai.' };

  sh.getRange(row, COL_USERS.userName + 1).setValue(unameBaru);
  if (data.email   != null) sh.getRange(row, COL_USERS.email   + 1).setValue(String(data.email).trim());
  if (data.role    != null) sh.getRange(row, COL_USERS.role    + 1).setValue(String(data.role).trim());
  if (data.ulp     != null) sh.getRange(row, COL_USERS.ulp     + 1).setValue(String(data.ulp).trim());
  if (data.kodeUlp != null) sh.getRange(row, COL_USERS.kodeUlp + 1).setValue(String(data.kodeUlp).trim());
  if (data.bidang  != null) sh.getRange(row, COL_USERS.bidang  + 1).setValue(String(data.bidang).trim());
  if (data.tim     != null) sh.getRange(row, COL_USERS.tim     + 1).setValue(String(data.tim).trim());
  if (data.subTim  != null) sh.getRange(row, COL_USERS.subTim  + 1).setValue(String(data.subTim).trim());
  if (data.aksesMenu != null)
    sh.getRange(row, COL_USERS.aksesMenu + 1)
      .setValue(Array.isArray(data.aksesMenu) ? data.aksesMenu.join(', ') : String(data.aksesMenu));
  if (data.password != null && String(data.password).trim())
    sh.getRange(row, COL_USERS.password + 1).setValue(String(data.password).trim());
  SpreadsheetApp.flush();
  return { ok: true };
}
...
function hapusAkun(token, username) {
  ...
  if (target.toLowerCase() === String(sesi.username || '').trim().toLowerCase())
    return { ok: false, message: 'Tidak dapat menghapus akun yang sedang login.' };
  var row = _findRowAkun(sh, target);
  if (row === -1) return { ok: false, message: 'Akun tidak ditemukan: ' + target };
  sh.deleteRow(row);
  SpreadsheetApp.flush();
...
function resetPasswordAkun(token, username, passwordBaru) {
  ...
  if (!String(passwordBaru || '').trim()) return { ok: false, message: 'Password baru wajib diisi.' };
  sh.getRange(row, COL_USERS.password + 1).setValue(String(passwordBaru).trim());
  SpreadsheetApp.flush();
  return { ok: true };
}
...
function gantiPassword(token, passwordLama, passwordBaru) {
  try {
    ...
    if (!pwLama || !pwBaru) return { ok: false, message: 'Password lama dan baru wajib diisi.' };
    if (pwBaru.length < 4)  return { ok: false, message: 'Password baru minimal 4 karakter.' };
    if (pwBaru === pwLama)  return { ok: false, message: 'Password baru tidak boleh sama dengan password lama.' };

    var sh  = _akunSheet();
    var row = _findRowAkun(sh, String(sesi.username || '').trim());
    if (row === -1) return { ok: false, message: 'Akun tidak ditemukan.' };

    var pwTersimpan = String(sh.getRange(row, COL_USERS.password + 1).getValue() || '').trim();
    if (pwTersimpan !== pwLama) return { ok: false, message: 'Password lama salah.' };
