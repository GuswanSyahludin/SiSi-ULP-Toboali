...
// ====== KONFIG ======
var SHEET_YANDAL = { HEADER: "db_Global_Header", SHIFT: "db_Yandal_Shift", P0: "db_Yandal_P0", SWITCHING: "db_Yandal_Pengecekan_Switching", LIST_P0: "db_Yandal_List_P0" };
...
var COL_YANDAL_SHIFT = {
  no:0, kodeHeader:1, kodeShift:2, hari:3, tanggal:4, ulp:5,
  tim:6, shift:7, petugas:8, jumlahP0:9, inputBy:10, timestamp:11
};
var COL_P0 = {
  no:0, kodeHeader:1, kodeShift:2, kodeP0:3, ulp:4, hari:5, tanggal:6,
  namaPekerjaan:7, pekerjaanLainnya:8, penyulang:9, section:10, daerah:11,
  koordinat:12, lat:13, long:14,
  koordinatClosing:15, latClosing:16, longClosing:17, jarak:18,
  ...
  fotoSebelum:23, fotoSebelumWm:24, fotoSebelumUrl:25, linkDownloadSebelum:26,
  timestampPekerjaan:27,
  fotoPekerjaan:28, fotoPekerjaanWm:29, fotoPekerjaanUrl:30, linkDownloadPekerjaan:31,
  timestampSesudah:32,
  fotoSesudah:33, fotoSesudahWm:34, fotoSesudahUrl:35, linkDownloadSesudah:36,
  catatan:37,
  inputBy:38, timestampWebsite:39, durasi:40,
  statusApproval:41, alasanRejected:42,   // (kolom AQ) "Alasan Rejected" — diisi setApprovalP0 (12 Agu 2026)
  approvedBy:43, timestampApprove:44,
  point:45, tampilSebelum:46, tampilPekerjaan:47, tampilSesudah:48, folderPath:49
};
...
var COL_SWITCHING = {
  no:0, kodeHeader:1, kodeShift:2, kodeP0:3, kodeSwitching:4,
  ulp:5, hari:6, tanggal:7,
  tim:8, petugas:9,
  penyulang:10, namaSwitching:11, jamPengecekan:12,
  koordinat:13, lat:14, long:15,
  indikatorRemote:16, indikatorLocal:17, indicatorProtection:18, indicatorReclose:19,
  arusR:20, arusS:21, arusT:22,
  fotoArus:23, fotoArusWm:24, fotoArusUrl:25, linkDownloadArus:26,
  fotoG1:27, fotoG1Wm:28, fotoG1Url:29, linkDownloadG1:30,
  fotoG2:31, fotoG2Wm:32, fotoG2Url:33, linkDownloadG2:34,
  fotoG3:35, fotoG3Wm:36, fotoG3Url:37, linkDownloadG3:38,
  fotoG4:39, fotoG4Wm:40, fotoG4Url:41, linkDownloadG4:42,
  fotoG5:43, fotoG5Wm:44, fotoG5Url:45, linkDownloadG5:46,
  inputBy:47, timestamp:48, folderPath:49
};
...
// ====== HELPER UMUM ======
function _ssY_()        { return SpreadsheetApp.openById(SPREADSHEET_ID); }
function _shY_(name)    { return _ssY_().getSheetByName(name); }
function _nowY_()       { return new Date(); }
function _fmtY_(d, pat) { return Utilities.formatDate(d, YANDAL_TZ, pat); }
function _hariY_(d)     { return HARI_ID[d.getDay()]; }
function _jamHHmm_(v)   { var ms = _toMillisY_(v); return isNaN(ms) ? "" : _fmtY_(new Date(ms), "HH:mm"); }   // Time Stamp (per foto) -> "15:16"
function _tglDMY_(v)    { var ms = _toMillisY_(v); var d = isNaN(ms) ? (Object.prototype.toString.call(v) === "[object Date]" ? v : null) : new Date(ms); return d ? (_padY_(d.getDate(),2) + " " + BULAN_ID[d.getMonth()] + " " + d.getFullYear()) : String(v || ""); }   // kolom G -> "04 Juni 2026"
function _padY_(n, len) { var s = "" + n; while (s.length < len) s = "0" + s; return s; }
function _allY_(sh)     { return sh.getDataRange().getValues(); }
function _setY_(sh, rowNum, col0, val) { sh.getRange(rowNum, col0 + 1).setValue(val); }
function _setDateFmtY_(sh, rowNum, col0, d) { var c = sh.getRange(rowNum, col0 + 1); c.setNumberFormat("dd/MM/yyyy HH.mm.ss"); c.setValue(d); }   // tulis Date + paksa format Indonesia (dd/MM/yyyy HH.mm.ss) agar tampil konsisten dgn baris lama
function _setTextY_(sh, rowNum, col0, val) { var c = sh.getRange(rowNum, col0 + 1); c.setNumberFormat("@"); c.setValue(val); }   // tulis sebagai TEKS murni (anti dikonversi jadi serial waktu)

function _findRowY_(sh, col0, value) {
  var d = _allY_(sh);
  for (var i = 1; i < d.length; i++) {
    if (String(d[i][col0]) === String(value)) return { rowNum: i + 1, row: d[i] };
  }
  return null;
}
function _countByValueY_(sh, col0, value) {
  var d = _allY_(sh), c = 0;
  for (var i = 1; i < d.length; i++) if (String(d[i][col0]) === String(value)) c++;
  return c;
}
function _countPrefixY_(sh, col0, prefix) {
  var d = _allY_(sh), c = 0;
  for (var i = 1; i < d.length; i++) if (String(d[i][col0]).indexOf(prefix) === 0) c++;
  return c;
}
...
var YDL_TAG_SHIFT = {
  "Shift 1":"SHF1", "Shift 2":"SHF2", "Shift 3":"SHF3",
  "Shift 1 (08:00 - 16:00)":"SHF1", "Shift 2 (16:00 - 00:00)":"SHF2", "Shift 3 (00:00 - 08:00)":"SHF3",
  "Pagi":"SHF1", "Siang":"SHF2", "Malam":"SHF3"
};
function _shiftTagY_(shift) { return YDL_TAG_SHIFT[String(shift || "").trim()] || "SHF0"; }
// Prefix modul = "Y" + 2 huruf terakhir Sub-Tim (padanan USERSETTINGS("Sub-Tim") di AppSheet). Fallback "YDL" bila Sub-Tim kosong.
function _modulTagY_(subTim) { var s = String(subTim || "").trim(); return s.length >= 2 ? ("Y" + s.slice(-2)) : YDL_MODUL; }
...
function genKodeHeaderYandal_(kodeUlp, tgl, subTim, shift) {
  ...
  var base = _modulTagY_(subTim) + "-" + String(kodeUlp || "").trim() + _fmtY_(new Date(tgl), "yyMMdd");
  var no = _shiftNoY_(shift);
  return base + _padY_(no > 0 ? no : (_countPrefixY_(sh, COL_INS.HEADER.kodeHeader, base) + 1), 3);
}
...
function genKodeShiftYandal_(kodeHeader, shift) {
  ...
  return prefix + _padY_(_countPrefixY_(sh, COL_YANDAL_SHIFT.kodeShift, prefix) + 1, 3);
}
...
function genKodeSwitchingYandal_(kodeP0) {
  ...
  return prefix + _padY_(_countPrefixY_(sh, COL_SWITCHING.kodeSwitching, prefix) + 1, 3);
}
...
function _appsheetRootY_() {
  if (YANDAL_APPSHEET_ROOT_ID) return DriveApp.getFolderById(YANDAL_APPSHEET_ROOT_ID);
...
function _folderFromRelPathY_(relPath) {
  ...
  var cache = CacheService.getScriptCache(), ckey = "fldY_" + clean;
  var cid = cache.get(ckey);
  if (cid) { try { return DriveApp.getFolderById(cid); } catch (eC) {} }   // id basi -> telusur ulang
  ...
  try { cache.put(ckey, folder.getId(), 21600); } catch (eP) {}   // simpan 6 jam
  return folder;
}
...
function pingEngineY() {
  try {
    UrlFetchApp.fetch("https://wm-engine-1011716929576.asia-southeast2.run.app/", {
      method: "get",
...
function _wmFotoY_(sh, rowNum, kSrc, kWm, kUrl, info, rowFolder, folderRel) {
  ...
  if (!path) {                                           // TIDAK ada foto sumber -> kosongkan WM & URL (buang nilai nyasar, mis. tanggal dari AppSheet)
    if (String(sh.getRange(rowNum, kWm + 1).getValue())  !== "") sh.getRange(rowNum, kWm + 1).clearContent();
    if (String(sh.getRange(rowNum, kUrl + 1).getValue()) !== "") sh.getRange(rowNum, kUrl + 1).clearContent();
    return;
  }
  ...
  if (!fileId) { Logger.log("Foto tak ditemukan: " + path); return; }
  try {
    ...
    var wmId = null;
    try {
      var outFolder = rowFolder || (YANDAL_WM_FOLDER_ID ? DriveApp.getFolderById(YANDAL_WM_FOLDER_ID) : null);
      if (outFolder) wmId = _fotoIdInFolderY_(outFolder, wmName);
      if (!wmId) wmId = _resolveFotoIdY_(wmName);
    } catch (eFind) { Logger.log("Cari fileId WM gagal: " + eFind); }
    if (wmId) {
      try { DriveApp.getFileById(wmId).setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW); } catch (eShare) { Logger.log("Set sharing WM gagal: " + eShare); }
      _setTextY_(sh, rowNum, kUrl, "https://drive.usercontent.google.com/download?id=" + wmId + "&export=download");   // URL WM = link download PAKSA (Content-Disposition attachment) -> tombol Download AppSheet
    } else {
      sh.getRange(rowNum, kUrl + 1).clearContent();
    }
  } catch (e) { Logger.log("Watermark gagal (" + path + "): " + e); }
}
...
function _durasiStr_(ms) {
  ...
  if (totalMin < 1) { var det = Math.round(ms / 1000); return det + " detik"; }   // < 1 menit -> tampilkan detik
  var jam = Math.floor(totalMin / 60), menit = totalMin % 60;
  var parts = [];
...
function _recalcDurasiRowY_(sh, rowNum) {
  ...
  var durNow = String(sh.getRange(rowNum, COL_P0.durasi + 1).getValue() || "").trim();
  if (durNow) return;                                                         // sudah pernah dihitung → jangan timpa
  var startMs = _toMillisY_(sh.getRange(rowNum, COL_P0.timestampPembuatan + 1).getValue());     // START = Time Stamp Pembuatan (V) = waktu DEVICE saat Foto Sebelum
  if (isNaN(startMs)) return;                                                 // device start belum ada → tunggu sync berikutnya
  var endMs = _toMillisY_(sh.getRange(rowNum, COL_P0.timestampSesudah + 1).getValue());          // END = Time Stamp Sesudah (AF) = waktu DEVICE saat Foto Sesudah diambil (beku saat closing)
  ...
  var diff = endMs - startMs; if (diff < 0) diff = 0;                         // jaga-jaga bila jam device tidak sinkron
...
function _haversineKmY_(lat1, lon1, lat2, lon2) {
  var R = 6371, toRad = function (d) { return d * Math.PI / 180; };   // radius bumi (km)
  var dLat = toRad(lat2 - lat1), dLon = toRad(lon2 - lon1);
  var a = Math.sin(dLat/2)*Math.sin(dLat/2) + Math.cos(toRad(lat1))*Math.cos(toRad(lat2))*Math.sin(dLon/2)*Math.sin(dLon/2);
  return R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
}
function _jarakKmStr_(km) { return (km == null || isNaN(km)) ? "" : km.toFixed(2).replace(".", ",") + " km"; }   // "1,24 km"
...
function _recalcJarakAntarP0RowY_(sh, rowNum) {
  ...
  var prevSeq = -1, prevLat = NaN, prevLon = NaN;
  for (var i = 1; i < d.length; i++) {
    ...
    var la = _toNumY_(d[i][COL_P0.lat]), lo = _toNumY_(d[i][COL_P0.long]);
    if (isNaN(la) || isNaN(lo)) continue;                      // koordinat P0 sebelumnya belum ada → lewati
    if (s > prevSeq) { prevSeq = s; prevLat = la; prevLon = lo; }
  }
  if (isNaN(prevLat) || isNaN(prevLon)) {                      // tidak ada P0 sebelumnya → P0 PERTAMA
    _setTextY_(sh, rowNum, COL_P0.jarakAntarP0, "0 km");
    return "0 km";
  }
  var lat2 = _toNumY_(row[COL_P0.lat]), lon2 = _toNumY_(row[COL_P0.long]);
...
function recalcJumlahP0_(kodeShift) {
  var jml = _countByValueY_(_shY_(SHEET_YANDAL.P0), COL_P0.kodeShift, kodeShift);
  var f = _findRowY_(_shY_(SHEET_YANDAL.SHIFT), COL_YANDAL_SHIFT.kodeShift, kodeShift);
  if (f) _setY_(_shY_(SHEET_YANDAL.SHIFT), f.rowNum, COL_YANDAL_SHIFT.jumlahP0, jml);
}
...
function _wmQueueSheet_() {
  ...
  if (!sh) {
    ...
    sh.getRange(1, 1, 1, 7).setValues([["id", "status", "kodeP0", "foto", "enqueuedAt", "lastTriedAt", "attempts"]]);
    sh.setFrozenRows(1);
  }
  return sh;
}
...
function enqueueP0Yandal_(kodeP0, fotoTarget) {
  ...
  var foto = String(fotoTarget || "").toLowerCase().trim();
  ...
  try { lock.waitLock(10000); } catch (e) { Logger.log("enqueueP0Yandal_: gagal lock — " + e); }
  try {
    var sh = _wmQueueSheet_();
    var d = sh.getDataRange().getValues();
    for (var i = 1; i < d.length; i++) {
      var st = String(d[i][1]);
      if ((st === "pending" || st === "processing") && String(d[i][2]) === String(kodeP0) && String(d[i][3]) === foto) {
...
function sweepWmBacklogY() {
  ...
  var pairs = [
    ...
    { src: COL_P0.fotoPekerjaan, wm: COL_P0.fotoPekerjaanWm, foto: "pekerjaan" },
  ...
  for (var i = 1; i < d.length; i++) {
    ...
    for (var p = 0; p < pairs.length; p++) {
      ...
      if (src && !wm) { if (enqueueP0Yandal_(kodeP0, pairs[p].foto)) n++; }   // ada foto, belum ada WM → antre ulang
    }
  }
  ...
  if (shSwc) {
    var ds = _allY_(shSwc);
    var pairsSwc = [
      { src: COL_SWITCHING.fotoArus, wm: COL_SWITCHING.fotoArusWm, foto: "arus" },
      { src: COL_SWITCHING.fotoG1,   wm: COL_SWITCHING.fotoG1Wm,   foto: "gangguan1" },
      { src: COL_SWITCHING.fotoG2,   wm: COL_SWITCHING.fotoG2Wm,   foto: "gangguan2" },
      { src: COL_SWITCHING.fotoG3,   wm: COL_SWITCHING.fotoG3Wm,   foto: "gangguan3" },
      { src: COL_SWITCHING.fotoG4,   wm: COL_SWITCHING.fotoG4Wm,   foto: "gangguan4" },
      { src: COL_SWITCHING.fotoG5,   wm: COL_SWITCHING.fotoG5Wm,   foto: "gangguan5" }
    ];
    for (var r = 1; r < ds.length; r++) {
      ...
      for (var q = 0; q < pairsSwc.length; q++) {
        ...
        if (srcS && !wmS) { if (enqueueP0Yandal_(kodeSwc, pairsSwc[q].foto)) n++; }
      }
    }
  }
  if (n) Logger.log("sweepWmBacklogY: enqueue ulang " + n + " foto belum ter-watermark");
  return n;
}

// Trigger backend (tiap 1 menit): sweep backlog → proses antrean WM. Klaim → proses tanpa lock → finalisasi.
function drainAntreanP0() {
  try { sweepWmBacklogY(); } catch (eSw) { Logger.log("drainAntreanP0: sweep backlog gagal — " + eSw); }
  var sh = _wmQueueSheet_();
  ...
  var lock = LockService.getUserLock();
  try { lock.waitLock(15000); } catch (e) { Logger.log("drainAntreanP0: gagal lock klaim — " + e); return; }
  try {
    var d = sh.getDataRange().getValues();
    var nowMs = Date.now();
    for (var i = 1; i < d.length && claimed.length < WM_QUEUE_BATCH; i++) {
      ...
      var staleProc = (st === "processing" && !isNaN(lastMs) && (nowMs - lastMs) > WM_QUEUE_STALE_MS);
      if (st === "pending" || staleProc) {
        d[i][1] = "processing";
        d[i][5] = new Date(nowMs);
        claimed.push({ id: String(d[i][0]), kodeP0: String(d[i][2]), foto: String(d[i][3]) });
      }
    }
  ...
  for (var j = 0; j < claimed.length; j++) {
    var it = claimed[j];
    try {
      if (String(it.kodeP0).indexOf("-SWC.") >= 0 || String(it.kodeP0).indexOf("-SWT.") >= 0) prosesSwitchingYandal(it.kodeP0, it.foto);   // item Pengecekan Switching (kode mengandung -SWC. / -SWT. lama utk data lama)
      else prosesP0Yandal(it.kodeP0, it.foto);
      hasil[it.id] = "done";
    }
    catch (e) { Logger.log("drainAntreanP0: proses gagal kode=" + it.kodeP0 + " — " + e); hasil[it.id] = "retry"; }
  }

  // Fase 3 — FINALISASI (lock singkat): hapus yang "done", kembalikan "pending" utk retry, "failed" bila lewat batas.
  try { lock.waitLock(15000); } catch (e) { Logger.log("drainAntreanP0: gagal lock final — " + e); return; }
  try {
    ...
    delRows.sort(function (a, b) { return b - a; });   // hapus dari bawah agar indeks tidak bergeser
...
function createWmDrainTriggerY() {
  ...
  for (var i = 0; i < trs.length; i++) if (trs[i].getHandlerFunction() === "drainAntreanP0") ScriptApp.deleteTrigger(trs[i]);
...
function hapusWmDrainTriggerY() {
  ...
  for (var i = 0; i < trs.length; i++) if (trs[i].getHandlerFunction() === "drainAntreanP0") { ScriptApp.deleteTrigger(trs[i]); n++; }
...
function prosesShiftYandal(kodeShift) {
  ...
  if (!f) return;
  if (!f.row[COL_YANDAL_SHIFT.timestamp]) _setY_(sh, f.rowNum, COL_YANDAL_SHIFT.timestamp, _nowY_());
...
function prosesP0Yandal(kodeP0, fotoTarget) {   // fotoTarget opsional: "sebelum" | "pekerjaan" | "sesudah" (kosong = proses semua foto yang ada)
  var lock = LockService.getScriptLock();
  try { lock.waitLock(30000); } catch (eLock) { Logger.log("prosesP0Yandal: gagal ambil lock — " + eLock); }
  try {
    var sh = _shY_(SHEET_YANDAL.P0);
    if (!sh) { Logger.log("prosesP0Yandal: sheet '" + SHEET_YANDAL.P0 + "' TIDAK ADA"); return; }
    var f = _findRowY_(sh, COL_P0.kodeP0, kodeP0);
    if (!f) { Logger.log("prosesP0Yandal: baris kodeP0 tak ditemukan — " + kodeP0); return; }
    var rowNum = f.rowNum, row = f.row, kodeShift = row[COL_P0.kodeShift];
    ...
    try { _recalcDurasiRowY_(sh, rowNum); } catch (eDur) { Logger.log("prosesP0Yandal: recalc durasi gagal — " + eDur); }
    ...
    try {
      ...
      var fS = _findRowY_(_shY_(SHEET_YANDAL.SHIFT), COL_YANDAL_SHIFT.kodeShift, kodeShift);
      var tim = "", petugas = "", kodeHeader = row[COL_P0.kodeHeader];
      if (fS) {
        ...
        if (!kodeHeader) { kodeHeader = fS.row[COL_YANDAL_SHIFT.kodeHeader]; _setY_(sh, rowNum, COL_P0.kodeHeader, kodeHeader); }
      }
      ...
      if (row[COL_P0.tanggal]) { hariVal = _hariY_(new Date(row[COL_P0.tanggal])); _setY_(sh, rowNum, COL_P0.hari, hariVal); }
      ...
      var jamPekerjaan = _jamHHmm_(row[COL_P0.timestampPekerjaan]) || jamPembuatan; // Foto Pekerjaan
      ...
      var info = {
        ...
        lat:       row[COL_P0.lat], long: row[COL_P0.long] // utk mini-map
      };
      // Salinan info Foto Pekerjaan dengan jam-nya sendiri (+ jarak antar P0 / kolom T → tampil di watermark Foto Pekerjaan).
      var infoPekerjaan = {}; for (var kp in info) infoPekerjaan[kp] = info[kp]; infoPekerjaan.jam = jamPekerjaan;
      // Salinan info khusus Foto Sesudah: jam sendiri + durasi (AO) & jarak Closing→Pekerjaan (S) — dibaca ulang setelah recalc.
      var durasiVal = String(sh.getRange(rowNum, COL_P0.durasi + 1).getValue() || "");
      var jarakVal = "";
      try { jarakVal = _recalcJarakRowY_(sh, rowNum); } catch (eJrk) { Logger.log("prosesP0Yandal: recalc jarak gagal — " + eJrk); }
      var jarakP0Val = "";
      try { jarakP0Val = _recalcJarakAntarP0RowY_(sh, rowNum); } catch (eJ2) { Logger.log("prosesP0Yandal: recalc jarak antar P0 gagal — " + eJ2); }   // jarak P0 terbaru → P0 sebelumnya (shift sama) → kolom T
      infoPekerjaan.jarakP0 = jarakP0Val;   // Jarak Antar P0 (T) tampil di watermark FOTO PEKERJAAN
      var infoSesudah = {}; for (var k in info) infoSesudah[k] = info[k]; infoSesudah.jam = jamSesudah; infoSesudah.durasi = durasiVal; infoSesudah.jarak = jarakVal;
      ...
      var koorClosingY = row[COL_P0.koordinatClosing], latClosingY = row[COL_P0.latClosing], lonClosingY = row[COL_P0.longClosing];
      if (String(koorClosingY || "") !== "" || (String(latClosingY || "") !== "" && String(lonClosingY || "") !== "")) {
      ...
      } catch (eFld) { Logger.log("Resolve folderPath gagal (pakai fallback): " + eFld); }
      ...
      var t = String(fotoTarget || "").toLowerCase().trim();
      if (!t || t === "sebelum")   _wmFotoY_(sh, rowNum, COL_P0.fotoSebelum,   COL_P0.fotoSebelumWm,   COL_P0.linkDownloadSebelum,   info,        rowFolder, folderRel);
      if (!t || t === "pekerjaan") _wmFotoY_(sh, rowNum, COL_P0.fotoPekerjaan, COL_P0.fotoPekerjaanWm, COL_P0.linkDownloadPekerjaan, infoPekerjaan,        rowFolder, folderRel);
      if (!t || t === "sesudah")   _wmFotoY_(sh, rowNum, COL_P0.fotoSesudah,   COL_P0.fotoSesudahWm,   COL_P0.linkDownloadSesudah,   infoSesudah, rowFolder, folderRel);
      // Catatan: kolom "Tampilan Foto ..." (=IMAGE di Sheet) & folderPath TIDAK ditimpa skrip.
    } catch (ePre) {
      Logger.log("prosesP0Yandal: error pra-recalc (dilewati, recalc tetap jalan) — " + ePre);
    }

    // 4) Jumlah P0 — SELALU dijalankan, walau langkah di atas gagal
    if (kodeShift) {
      recalcJumlahP0_(kodeShift);
      Logger.log("prosesP0Yandal: recalc OK utk kodeShift=" + kodeShift + " (kodeP0=" + kodeP0 + ")");
    } else {
      Logger.log("prosesP0Yandal: kodeShift KOSONG di baris P0 " + kodeP0 + " → Jumlah P0 tidak bisa dihitung");
    }
  } catch (eMain) {
...
function prosesSwitchingYandal(kodeSwitching, fotoTarget) {   // fotoTarget opsional: "arus" | "gangguan1".."gangguan5" (kosong = semua foto yang ada)
  var lock = LockService.getScriptLock();
  try { lock.waitLock(30000); } catch (eLock) { Logger.log("prosesSwitchingYandal: gagal ambil lock — " + eLock); }
  try {
    var sh = _shY_(SHEET_YANDAL.SWITCHING);
    if (!sh) { Logger.log("prosesSwitchingYandal: sheet '" + SHEET_YANDAL.SWITCHING + "' TIDAK ADA"); return; }
    var f = _findRowY_(sh, COL_SWITCHING.kodeSwitching, kodeSwitching);
    if (!f) { Logger.log("prosesSwitchingYandal: baris tak ditemukan — " + kodeSwitching); return; }
    var rowNum = f.rowNum, row = f.row, kodeP0 = row[COL_SWITCHING.kodeP0];
    ...
    var ulp = row[COL_SWITCHING.ulp], tanggalRaw = row[COL_SWITCHING.tanggal], hari = row[COL_SWITCHING.hari];
    var tim = row[COL_SWITCHING.tim], petugas = row[COL_SWITCHING.petugas], daerah = "";
    var fP = kodeP0 ? _findRowY_(_shY_(SHEET_YANDAL.P0), COL_P0.kodeP0, kodeP0) : null;
    if (fP) {
      daerah  = fP.row[COL_P0.daerah];
      if (!ulp)        { ulp = fP.row[COL_P0.ulp]; if (ulp) _setY_(sh, rowNum, COL_SWITCHING.ulp, ulp); }
      if (!tanggalRaw) { tanggalRaw = fP.row[COL_P0.tanggal]; if (tanggalRaw) _setY_(sh, rowNum, COL_SWITCHING.tanggal, tanggalRaw); }
      if (!tim)        { tim = fP.row[COL_P0.tim]; if (tim) _setY_(sh, rowNum, COL_SWITCHING.tim, tim); }
      if (!petugas)    { petugas = fP.row[COL_P0.petugas]; if (petugas) _setY_(sh, rowNum, COL_SWITCHING.petugas, petugas); }
    }
    if (!hari && tanggalRaw) { hari = _hariY_(new Date(tanggalRaw)); _setY_(sh, rowNum, COL_SWITCHING.hari, hari); }
    var tanggal = _tglDMY_(tanggalRaw);

    // Jam watermark = "Jam Pengecekan" (satu nilai utk semua foto). Dukung Date/serial/teks.
    var jam = _jamHHmm_(row[COL_SWITCHING.jamPengecekan]) || String(row[COL_SWITCHING.jamPengecekan] || "");

    // Bullet Arus R/S/T (hanya nilai yang ada)
    var arusArr = [];
    if (String(row[COL_SWITCHING.arusR] || "") !== "") arusArr.push("R " + row[COL_SWITCHING.arusR]);
    if (String(row[COL_SWITCHING.arusS] || "") !== "") arusArr.push("S " + row[COL_SWITCHING.arusS]);
    if (String(row[COL_SWITCHING.arusT] || "") !== "") arusArr.push("T " + row[COL_SWITCHING.arusT]);
    ...
    var info = {
      ...
      lat:       row[COL_SWITCHING.lat], long: row[COL_SWITCHING.long]
    };

    // Folder per-baris dari kolom folderPath (path relatif AppSheet)
    var rowFolder = null, folderRel = row[COL_SWITCHING.folderPath];
    try { if (folderRel) rowFolder = _folderFromRelPathY_(folderRel); } catch (eFld) { Logger.log("Resolve folderPath switching gagal: " + eFld); }

    // 6 foto: Arus + Gangguan 1..5 (independen & idempoten; kosong dilewati/dibersihkan)
    var fotos = [
      { key:"arus",      src:COL_SWITCHING.fotoArus, wm:COL_SWITCHING.fotoArusWm, url:COL_SWITCHING.linkDownloadArus },
      { key:"gangguan1", src:COL_SWITCHING.fotoG1,   wm:COL_SWITCHING.fotoG1Wm,   url:COL_SWITCHING.linkDownloadG1 },
      { key:"gangguan2", src:COL_SWITCHING.fotoG2,   wm:COL_SWITCHING.fotoG2Wm,   url:COL_SWITCHING.linkDownloadG2 },
      { key:"gangguan3", src:COL_SWITCHING.fotoG3,   wm:COL_SWITCHING.fotoG3Wm,   url:COL_SWITCHING.linkDownloadG3 },
      { key:"gangguan4", src:COL_SWITCHING.fotoG4,   wm:COL_SWITCHING.fotoG4Wm,   url:COL_SWITCHING.linkDownloadG4 },
      { key:"gangguan5", src:COL_SWITCHING.fotoG5,   wm:COL_SWITCHING.fotoG5Wm,   url:COL_SWITCHING.linkDownloadG5 }
    ];
    var t = String(fotoTarget || "").toLowerCase().trim();
    for (var i = 0; i < fotos.length; i++) {
      if (!t || t === fotos[i].key) _wmFotoY_(sh, rowNum, fotos[i].src, fotos[i].wm, fotos[i].url, info, rowFolder, folderRel);
    }
  } catch (eMain) {
...
function _p0FotoObj_(row, linkCol, urlCol){
  ...
  for (var i = 0; i < cand.length && !id; i++){
    var m = cand[i].match(/[?&]id=([-\w]{20,})/) || cand[i].match(/\/d\/([-\w]{20,})/);
    if (m) id = m[1];
  }
...
// P0 yang masuk antrean approval = ke-3 foto sudah ada URL & Status Approval masih kosong (efektif "Menunggu").
  ...
  try {
    ...
    for (var i = 1; i < d.length; i++){
      ...
      out.push({
        ...
        tanggal:       tgl,
        namaPekerjaan: String(d[i][C.namaPekerjaan] || '').trim() || String(d[i][C.pekerjaanLainnya] || '').trim(),
        ...
        timestampApprove: d[i][C.timestampApprove] ? _tglDMY_(d[i][C.timestampApprove]) : ''
      });
    }
    out.sort(function(a, b){
      var ra = (a.status === 'Menunggu') ? 0 : 1, rb = (b.status === 'Menunggu') ? 0 : 1;
...
function setApprovalP0(params){
  try {
    ...
    if (['Approved','Rejected'].indexOf(keputusan) < 0) return { ok:false, error:'Keputusan harus Approved atau Rejected.' };
    var sh = _shY_(SHEET_YANDAL.P0);
    var f = _findRowY_(sh, COL_P0.kodeP0, kodeP0);
    ...
    _setTextY_(sh, f.rowNum, COL_P0.alasanRejected, keputusan === 'Rejected' ? alasan : '');
    // TRIGGER 1 (web): saat Approved → hitung & set Point (AR); saat Rejected → kosongkan Point.
    var pointRes = null;
    if (keputusan === 'Approved') {
      try { pointRes = hitungPointP0Yandal(kodeP0); } catch (ePt) { Logger.log('setApprovalP0: hitung point gagal — ' + ePt); }
    } else {
      try { _setY_(sh, f.rowNum, COL_P0.point, ''); } catch (eClr) {}
    }
    return { ok:true, kodeP0:kodeP0, status:keputusan, approvedBy:approver, point:(pointRes && pointRes.ok ? pointRes.point : null) };
  } catch (e) { return { ok:false, error:e.message }; }
}
...
function updateNamaPekerjaanP0(params){
  try {
    ...
    var cocok = false, arr = (lst && lst.list) || [];
    for (var i = 0; i < arr.length; i++) if (String(arr[i]).toLowerCase() === namaBaru.toLowerCase()) { cocok = true; break; }
    if (!cocok) return { ok:false, error:'Nama Pekerjaan tidak ada di master db_Yandal_List_P0.' };
    var sh = _shY_(SHEET_YANDAL.P0);
    var f = _findRowY_(sh, COL_P0.kodeP0, kodeP0);
    if (!f) return { ok:false, error:'Baris P0 tidak ditemukan: ' + kodeP0 };
    ...
    var pointRes = null, status = String(f.row[COL_P0.statusApproval] || '').trim();
    if (status === 'Approved') {
      try { pointRes = hitungPointP0Yandal(kodeP0); } catch (ePt) { Logger.log('updateNamaPekerjaanP0: hitung point gagal — ' + ePt); }
    }
    Logger.log('updateNamaPekerjaanP0: ' + kodeP0 + ' "' + namaLama + '" → "' + namaBaru + '" oleh=' + String(params.username || '-'));
    return { ok:true, kodeP0:kodeP0, namaLama:namaLama, namaPekerjaan:namaBaru,
...
var COL_UKUR_GARDU = {
  no:0, kodeHeader:1, kodeShift:2, kodeP0:3, kodeUkur:4,
  ulp:5, hari:6, tanggal:7, penyulang:8, section:9, noGardu:10, alamat:11, jamUkur:12,
  bebanR:13, bebanS:14, bebanT:15, bebanN:16,
  tegRS:17, tegRT:18, tegST:19, tegRN:20, tegSN:21, tegTN:22,
  petugas:23
};
...
function _sheetUkurGardu_(ssU){
  ...
  for (var i = 0; i < sheets.length; i++){
    ...
    if (String(head || '').trim().toLowerCase() === 'kode pengukuran gardu') return sh;
  }
  return null;
}
function getLampiranPengecekanP0(params){
  try {
    ...
    if (!shP0) return { ok:false, error:"sheet '" + SHEET_YANDAL.P0 + "' tidak ada." };
    var fP = _findRowY_(shP0, COL_P0.kodeP0, kodeP0);
    if (!fP) return { ok:false, error:'Baris P0 tidak ditemukan: ' + kodeP0 };
    var rp = fP.row;
    var p0 = {
      kodeP0:        kodeP0,
      namaPekerjaan: String(rp[COL_P0.namaPekerjaan] || '').trim() || String(rp[COL_P0.pekerjaanLainnya] || '').trim(),
      penyulang:     String(rp[COL_P0.penyulang] || ''),
      daerah:        String(rp[COL_P0.daerah] || ''),
      tanggal:       _normTgl(rp[COL_P0.tanggal]),
      fotoSebelum:   _p0FotoObj_(rp, COL_P0.linkDownloadSebelum,   COL_P0.fotoSebelumUrl),
      fotoPekerjaan: _p0FotoObj_(rp, COL_P0.linkDownloadPekerjaan, COL_P0.fotoPekerjaanUrl),
      fotoSesudah:   _p0FotoObj_(rp, COL_P0.linkDownloadSesudah,   COL_P0.fotoSesudahUrl)
    };
    ...
    if (shS && shS.getLastRow() > 1) {
      var ds = _allY_(shS), S = COL_SWITCHING;
      for (var i = 1; i < ds.length; i++) {
        ...
        switching.push({
          ...
          jamPengecekan:       _jamHHmm_(ds[i][S.jamPengecekan]) || String(ds[i][S.jamPengecekan] || ''),
          ...
          arusR: String(ds[i][S.arusR] || ''), arusS: String(ds[i][S.arusS] || ''), arusT: String(ds[i][S.arusT] || ''),
    ...
    try {
      ...
      if (shU && shU.getLastRow() > 1) {
        var du = shU.getDataRange().getValues(), U = COL_UKUR_GARDU;
        for (var u = 1; u < du.length; u++) {
          ...
          gardu.push({
            ...
            jamUkur:   _jamHHmm_(du[u][U.jamUkur]) || String(du[u][U.jamUkur] || ''),
            bebanR: String(du[u][U.bebanR] || ''), bebanS: String(du[u][U.bebanS] || ''),
            bebanT: String(du[u][U.bebanT] || ''), bebanN: String(du[u][U.bebanN] || ''),
            tegRS: String(du[u][U.tegRS] || ''), tegRT: String(du[u][U.tegRT] || ''),
            tegST: String(du[u][U.tegST] || ''), tegRN: String(du[u][U.tegRN] || ''),
            tegSN: String(du[u][U.tegSN] || ''), tegTN: String(du[u][U.tegTN] || ''),
            petugas:   String(du[u][U.petugas] || '').trim()
          });
        }
      }
    } catch (eU) { Logger.log('getLampiranPengecekanP0: baca pengukuran gardu gagal — ' + eU); }
...
function _durasiToMenitY_(v) {
  ...
  if (!s) return NaN;
  var menit = 0, found = false;
  var mJam = s.match(/(\d+)\s*jam/);   if (mJam) { menit += parseInt(mJam[1], 10) * 60; found = true; }
  var mMnt = s.match(/(\d+)\s*menit/); if (mMnt) { menit += parseInt(mMnt[1], 10);       found = true; }
  var mDtk = s.match(/(\d+)\s*detik/); if (mDtk) { menit += parseInt(mDtk[1], 10) / 60;  found = true; }
...
function _hitungPoinDariRowY_(row, bobotMap) {
  ...
  if (status !== 'Approved') return { ok:true, skipped:true, reason:'Status Approval P0 bukan Approved', status:status };
  ...
  if (!bobotMap || !bobotMap.hasOwnProperty(key)) {
    return { ok:false, skipped:true, reason:'Nama Pekerjaan tidak ada/tak berbobot di db_Yandal_List_P0 (atur Point manual)', namaPekerjaan:nama };
  }
  ...
  if (nama.toUpperCase().indexOf('ROW') >= 0) {
    ...
    if (jamBuat) {
      var menitHari = parseInt(jamBuat.slice(0, 2), 10) * 60 + parseInt(jamBuat.slice(3, 5), 10);
      if (menitHari >= 1320 || menitHari <= 300) pengali = 2.5;   // 22:00 s/d 05:00, lewat tengah malam
    }
  }
  var poin = skorD + (bobot * pengali);
  return { ok:true, namaPekerjaan:nama, durasiMenit:(isNaN(menit)?null:menit), skorDurasi:skorD, bobot:bobot, pengali:pengali, point:poin };
}
...
function hitungPointP0Yandal(kodeP0) {
  try {
    ...
    if (!kodeP0) return { ok:false, error:'kodeP0 wajib diisi.' };
    var sh = _shY_(SHEET_YANDAL.P0);
    if (!sh) return { ok:false, error:"sheet '" + SHEET_YANDAL.P0 + "' tidak ada." };
    var f = _findRowY_(sh, COL_P0.kodeP0, kodeP0);
    if (!f) return { ok:false, error:'Baris P0 tidak ditemukan: ' + kodeP0 };
    var rowNum = f.rowNum, row = f.row;

    var r = _hitungPoinDariRowY_(row, _bobotPekerjaanMapY_());
    if (!r.ok || r.skipped) {
      return { ok:r.ok, skipped:(r.skipped || false), reason:r.reason, kodeP0:kodeP0, status:r.status, namaPekerjaan:r.namaPekerjaan };
    }
    _setY_(sh, rowNum, COL_P0.point, r.point);   // tulis ke kolom Point (AR)

    Logger.log('hitungPointP0Yandal: ' + kodeP0 + ' → D=' + r.skorDurasi + ' bobot=' + r.bobot + ' pengali=' + r.pengali + ' point=' + r.point);
    return { ok:true, kodeP0:kodeP0, namaPekerjaan:r.namaPekerjaan, durasiMenit:r.durasiMenit, skorDurasi:r.skorDurasi, bobot:r.bobot, pengali:r.pengali, point:r.point };
  } catch (e) {
...
function sweepPointP0Yandal(opts) {
  ...
  var sh = _shY_(SHEET_YANDAL.P0);
  if (!sh || sh.getLastRow() <= 1) return { ok:true, processed:0, skipped:0 };
  var d = _allY_(sh);
  ...
  for (var i = 1; i < d.length; i++) {
    ...
    if (!kodeP0 || status !== 'Approved' || (point && !force)) { skipped++; continue; }
    try {
  ...
  if (processed || errors.length) Logger.log('sweepPointP0Yandal: processed=' + processed + ' errors=' + JSON.stringify(errors));
...
function recalcPaksaPointP0Yandal() { return sweepPointP0Yandal({ force:true }); }
...
function _resetRecalcPointCursorY_() { PropertiesService.getScriptProperties().deleteProperty(RECALC_POINT_PROP); }
...
function recalcPointP0Bertahap(opts) {
  opts = opts || {};
  var batch = Number(opts.batch || RECALC_POINT_BATCH); if (isNaN(batch) || batch < 1) batch = RECALC_POINT_BATCH;
  var sh = _shY_(SHEET_YANDAL.P0);
  if (!sh || sh.getLastRow() <= 1) { _resetRecalcPointCursorY_(); return { ok:true, done:true, processed:0, skipped:0, errors:[] }; }
  var d = _allY_(sh);
  ...
  var processed = 0, skipped = 0, errors = [], i = start;
  for (; i < d.length; i++) {
    if (processed >= batch) break;   // batas per putaran → sisanya di putaran berikutnya
    var row = d[i];
    var kodeP0 = String(row[COL_P0.kodeP0] || '').trim();
    if (!kodeP0) { skipped++; continue; }
    try {
      var r = _hitungPoinDariRowY_(row, bobotMap);
      if (r.ok && !r.skipped) { _setY_(sh, i + 1, COL_P0.point, r.point); processed++; }
      else { skipped++; if (!r.ok) errors.push({ kodeP0:kodeP0, reason:r.reason }); }
    } catch (e) { errors.push({ kodeP0:kodeP0, error:e.message }); }
  }
  var done = (i >= d.length);
  if (done) _resetRecalcPointCursorY_(); else props.setProperty(RECALC_POINT_PROP, String(i));
  Logger.log('recalcPointP0Bertahap: start=' + start + ' next=' + (done ? '-' : i) + ' processed=' + processed + ' skipped=' + skipped + ' done=' + done + (errors.length ? ' errors=' + JSON.stringify(errors) : ''));
  return { ok:true, done:done, start:start, next:(done ? null : i), processed:processed, skipped:skipped, errors:errors };
}
...
function jalankanRecalcPointBertahap() {
  var lock = LockService.getUserLock();
  if (!lock.tryLock(1000)) { Logger.log('Recalc Point bertahap: putaran sebelumnya masih jalan → lewati tick ini.'); return; }
  try {
    var r = recalcPointP0Bertahap();
    if (r && r.done) {
      var trs = ScriptApp.getProjectTriggers();
      for (var i = 0; i < trs.length; i++) if (trs[i].getHandlerFunction() === 'jalankanRecalcPointBertahap') ScriptApp.deleteTrigger(trs[i]);
...
function mulaiRecalcPointBertahap() {
  ...
  var trs = ScriptApp.getProjectTriggers();
  for (var i = 0; i < trs.length; i++) if (trs[i].getHandlerFunction() === 'jalankanRecalcPointBertahap') ScriptApp.deleteTrigger(trs[i]);
  ScriptApp.newTrigger('jalankanRecalcPointBertahap').timeBased().everyMinutes(1).create();
  Logger.log('Recalc Point bertahap DIMULAI: batch=' + RECALC_POINT_BATCH + '/menit, berhenti otomatis saat selesai.');
}

// HENTIKAN/batalkan recalc bertahap (lepas trigger + reset cursor).
function hentikanRecalcPointBertahap() {
  var trs = ScriptApp.getProjectTriggers(), n = 0;
  for (var i = 0; i < trs.length; i++) if (trs[i].getHandlerFunction() === 'jalankanRecalcPointBertahap') { ScriptApp.deleteTrigger(trs[i]); n++; }
  _resetRecalcPointCursorY_();
  Logger.log('Recalc Point bertahap DIHENTIKAN: trigger dilepas=' + n + ', cursor direset.');
}
...
function createPointP0DrainTriggerY() {
  ...
  for (var i = 0; i < trs.length; i++) {
    if (trs[i].getHandlerFunction() === 'sweepPointP0Yandal') ScriptApp.deleteTrigger(trs[i]);
  }
  ScriptApp.newTrigger('sweepPointP0Yandal').timeBased().everyMinutes(1).create();
...
function hapusPointP0DrainTriggerY() {
  var trs = ScriptApp.getProjectTriggers(), n = 0;
  for (var i = 0; i < trs.length; i++) {
    if (trs[i].getHandlerFunction() === 'sweepPointP0Yandal') { ScriptApp.deleteTrigger(trs[i]); n++; }
  }
...
function _rangeStartMsY_(v) { var ms = _toMillisY_(v); if (isNaN(ms)) return NaN; var d = new Date(ms); d.setHours(0,0,0,0); return d.getTime(); }
function _rangeEndMsY_(v)   { var ms = _toMillisY_(v); if (isNaN(ms)) return NaN; var d = new Date(ms); d.setHours(23,59,59,999); return d.getTime(); }
...
function getRekapPointPetugasY(params) {
  try {
    ...
    var d = _readSheetDual_(SHEET_YANDAL.P0, COL_P0.kodeP0, COL_P0.folderPath + 1), C = COL_P0;
    var agg = {};                                    // nama → { petugas, jumlahP0, totalPoint }
    var totalP0 = 0, totalPoint = 0;
    ...
    list.sort(function (a, b) { return (b.totalPoint - a.totalPoint) || (b.jumlahP0 - a.jumlahP0); });
    for (var r = 0; r < list.length; r++) {
      list[r].rank = r + 1;
      list[r].rataRata = list[r].jumlahP0 ? Math.round((list[r].totalPoint / list[r].jumlahP0) * 100) / 100 : 0;
    }
    return { ok:true, tglAwal:params.tglAwal||'', tglAkhir:params.tglAkhir||'', ulp:ulpFilter,
             totalBarisP0:totalP0, totalPoint:totalPoint, jumlahPetugas:list.length, list:list };
  } catch (e) { return { ok:false, error:e.message }; }
}
...
function getListPetugasYandal(params) {
  try {
    ...
    if (shShift && shShift.getLastRow() > 1) {
      ...
      for (var i = 1; i < ds.length; i++) {
        if (ulpFilter && String(ds[i][COL_YANDAL_SHIFT.ulp] || '').trim() !== ulpFilter) continue;
        var orang = _splitPetugasY_(ds[i][COL_YANDAL_SHIFT.petugas]);
        for (var k = 0; k < orang.length; k++) if (orang[k]) set[orang[k]] = true;
      }
    }
    ...
    var dp = _readSheetDual_(SHEET_YANDAL.P0, COL_P0.kodeP0, COL_P0.folderPath + 1);
    if (dp.length > 0) {
      for (var j = 0; j < dp.length; j++) {
        if (ulpFilter && String(dp[j][COL_P0.ulp] || '').trim() !== ulpFilter) continue;
        var orang2 = _splitPetugasY_(dp[j][COL_P0.petugas]);
        for (var m = 0; m < orang2.length; m++) if (orang2[m]) set[orang2[m]] = true;
      }
    }
    var list = [];
...
function _padNY_(n) { return (n < 10 ? '0' : '') + n; }
...
function _unitPetugasMapY_(ulpFilter) {
  ...
  if (dp.length > 0) {
    for (var j = 0; j < dp.length; j++) {
      ...
      for (var m = 0; m < o2.length; m++) if (o2[m] && !map[o2[m]] && u2) map[o2[m]] = u2;
    }
  }
  return map;
}
...
function _rankIndexY_(periode, ulpFilter) {
  ...
  try {
    ...
    for (var i = 0; i < list.length; i++) {
      var it = list[i], rb = Number(it.rankBulanan);
      idx[it.petugas] = {
...
function _rankTahunIndexY_(tahun, ulpFilter) {
  var idx = {}, out = {};
  try {
    var sh = _ensureSheetRankY_(), R = COL_RANK_Y, d = _allY_(sh);
    ...
    for (var i = 1; i < d.length; i++) {
      ...
      if (!nm) continue;
      if (!idx[nm]) idx[nm] = { petugas: nm, ulp: u, bulan: 0, sumNilai: 0, pointP0: 0, jumlahP0: 0, jumlahShift: 0, menitEfektif: 0 };
      var a = idx[nm];
      a.bulan        += 1;
    ...
    for (var nm2 in idx) {
      var it = idx[nm2];
      it.nilaiRata = it.bulan ? _bulatY_(it.sumNilai / it.bulan, DESIMAL_NILAI_Y) : 0;
      arr.push(it);
    }
    _peringkatY_(arr, function (x) { return x.nilaiRata; }, false, function (x, rk) { x.rankTahunan = rk; },
      function (x) { return [x.menitEfektif, x.jumlahShift, x.pointP0, x.jumlahP0]; });
...
function getTabelPetugasYandal(params) {
  try {
    ...
    if (!/^\d{4}-\d{2}$/.test(String(periode))) {
      ...
      periode = _fmtY_(isNaN(acuanMs) ? _nowY_() : new Date(acuanMs), 'yyyy-MM');
    }
    var th = Number(periode.substring(0, 4)), bl = Number(periode.substring(5, 7)) - 1;

    // Baca db_Yandal_Rank untuk periode tsb (sumber tunggal angka bulanan).
    var r = getRankYandal({ periode: periode, ulp: ulpFilter, sinkron: (params.sinkron === true) });
    if (!r.ok) return { ok: false, error: r.error || 'Gagal membaca db_Yandal_Rank.', list: [] };
    var src = r.list || [];
    if (!src.length && params.sinkron !== true) {   // periode belum pernah disinkron → sinkron sekali
    ...
    for (var i = 0; i < src.length; i++) {
      ...
      var t = idxTahun[it.petugas], rb = Number(it.rankBulanan);
      var poinP0 = _toNumY_(it.pointP0) || 0, jmlP0 = _toNumY_(it.jumlahP0) || 0;
      out.push({
        no: 0,
        nama: it.petugas,
    ...
    out.sort(function (a, b2) {
      var ba = a.rankBulanan || 9999, bb = b2.rankBulanan || 9999;
      if (ba !== bb) return ba - bb;
      if ((b2.nilaiTotal || 0) !== (a.nilaiTotal || 0)) return (b2.nilaiTotal || 0) - (a.nilaiTotal || 0);
      return (a.rankTahunan || 9999) - (b2.rankTahunan || 9999);
    });
    for (var n = 0; n < out.length; n++) out[n].no = n + 1;

    var jmlTahun = 0; for (var kk in idxTahun) jmlTahun++;
    return { ok: true, list: out, periode: periode, ulp: ulpFilter,
             bulanLabel: BULAN_ID[bl] + ' ' + th, tahun: th,
             totalPetugasBulan: src.length, totalPetugasTahun: jmlTahun,
...
function _unionMenitY_(iv) {
  ...
  var a = iv.slice().sort(function (x, y) { return x[0] - y[0]; });
  var total = 0, s = a[0][0], e = a[0][1], minMs = a[0][0], maxMs = a[0][1];
  for (var i = 1; i < a.length; i++) {
    if (a[i][1] > maxMs) maxMs = a[i][1];
    if (a[i][0] <= e) { if (a[i][1] > e) e = a[i][1]; }   // tumpang tindih / bersambung → gabung
    else { total += (e - s); s = a[i][0]; e = a[i][1]; }  // ada jeda → tutup blok, mulai blok baru
  }
  total += (e - s);
  return { menit: Math.max(0, Math.round(total / 60000)), minMs: minMs, maxMs: maxMs };
}
...
function _ivSehatY_(sMs, eMs, menitDurasi) {
  ...
  var ds = new Date(sMs), de = new Date(eMs);
  var e2 = new Date(ds.getFullYear(), ds.getMonth(), ds.getDate(), de.getHours(), de.getMinutes(), de.getSeconds(), 0).getTime();
...
function _shiftWindowMsY_(tanggalMs, shiftNo) {
  ...
  var start = new Date(d.getFullYear(), d.getMonth(), d.getDate(), jamMulai, 0, 0, 0).getTime();
...
function _clampIvY_(iv, win) {
  ...
  for (var i = 0; i < (iv || []).length; i++) {
    var s = Math.max(iv[i][0], win.start), e = Math.min(iv[i][1], win.end);
    if (e > s) out.push([s, e]);
  }
  return out;
}
...
var SHIFT_JAM_Y = { 1: '08:00 - 16:00', 2: '16:00 - 00:00', 3: '00:00 - 08:00' };
function _shiftNoDariKodeY_(kodeShift) { var m = String(kodeShift || '').match(/SHF(\d)/i); return m ? parseInt(m[1], 10) : 0; }
function _menitToJamStrY_(menit) {
  if (menit == null || isNaN(menit)) return '-';
  var t = Math.round(menit), j = Math.floor(t / 60), m = t % 60, p = [];
  if (j > 0) p.push(j + ' jam');
  if (m > 0) p.push(m + ' menit');
  return p.length ? p.join(' ') : '0 menit';
}
function _bulatY_(n, d) { var f = Math.pow(10, d || 0); return Math.round(n * f) / f; }
...
function getDetailPerformaPetugasY(params) {
  try {
    ...
    var th = acuan.getFullYear(), bl = acuan.getMonth();
    ...
    var akhirBulan = th + '-' + _padNY_(bl + 1) + '-' + _padNY_(new Date(th, bl + 1, 0).getDate());
    ...
    for (var i = 0; i < d.length; i++) {
      ...
      var ada = false;
      for (var k = 0; k < orang.length; k++) if (orang[k] === nama) { ada = true; break; }
      if (!ada) continue;
      ...
      var poin = Number(row[C.point]); if (isNaN(poin)) poin = 0;
      var menit = _durasiToMenitY_(row[C.durasi]); if (isNaN(menit)) menit = 0;
      var kodeShift = String(row[C.kodeShift] || '').trim();

      p0Tahun += 1; pointTahun += poin;

      // Komposisi jenis pekerjaan: khusus BULAN berjalan (bahan pie chart)
      if (tMs >= blAwalMs && tMs <= blAkhirMs) {
        var namaKerja = String(row[C.namaPekerjaan] || '').trim() || String(row[C.pekerjaanLainnya] || '').trim() || '(Tanpa nama)';
        if (!jenis[namaKerja]) jenis[namaKerja] = { nama: namaKerja, jumlah: 0, point: 0 };
        jenis[namaKerja].jumlah += 1; jenis[namaKerja].point += poin;
      }
      ...
      if (!grpTahun[key]) grpTahun[key] = {
        kodeShift: kodeShift, tanggalMs: tMs, tanggal: _normTgl(row[C.tanggal]),
        hari: String(row[C.hari] || ''), shiftNo: _shiftNoDariKodeY_(kodeShift),
        jumlahP0: 0, menitKotor: 0, point: 0, iv: [], p0s: []
      };
      var g = grpTahun[key];
      g.jumlahP0 += 1; g.point += poin; g.menitKotor += menit;
      ...
      g.p0s.push({
        ...
        pekerjaan:  String(row[C.namaPekerjaan] || '').trim() || String(row[C.pekerjaanLainnya] || '').trim() || '(Tanpa nama)',
        ...
        durasiMenit: menit, durasiStr: _menitToJamStrY_(menit),
    ...
    if (!p0Tahun) return { ok: true, petugas: nama, unit: unit || ulpFilter || '-', tim: tim || '-',
      bulanLabel: BULAN_ID[bl] + ' ' + th, tahun: th, kosong: true, ringkasan: {}, shift: [], pekerjaan: [] };

    // Gabung interval per shift → jam efektif BERSIH (P0 yang tumpang tindih tidak dihitung dobel)
    var shiftList = [], shiftTahun = {};
    var p0Bulan = 0, pointBulan = 0, menitBulan = 0, menitKotorBulan = 0, shiftOverlapBulan = 0, menitTahun = 0;
    for (var kk in grpTahun) {
      ...
      p0Bulan += it.jumlahP0; pointBulan += it.point;
      menitBulan += u.menit; menitKotorBulan += it.menitKotor;
      var overlap = (it.menitKotor - u.menit) >= 1;
      if (overlap) shiftOverlapBulan++;
      var rentang = (!isNaN(u.minMs) && !isNaN(u.maxMs)) ? Math.max(0, Math.round((u.maxMs - u.minMs) / 60000)) : null;

      shiftList.push({
        tanggal: it.tanggal, hari: it.hari,
        ...
        rentangMenit: rentang, rentangStr: (rentang == null ? '-' : _menitToJamStrY_(rentang)),
        menitEfektif: u.menit, jamEfektifStr: _menitToJamStrY_(u.menit), terpotong: !!u.terpotong,
        menitKotor: it.menitKotor, jamKotorStr: _menitToJamStrY_(it.menitKotor), overlap: overlap,
        persen: _bulatY_(u.menit / 480 * 100, 1),
        point: _bulatY_(it.point, 2),
        p0List: (it.p0s || []).sort(function (x, y) { return x._sortMs - y._sortMs; }),
        _sort: it.tanggalMs * 10 + (it.shiftNo || 0)
      });
    }
    shiftList.sort(function (a, b) { return b._sort - a._sort; });
    for (var s = 0; s < shiftList.length; s++) delete shiftList[s]._sort;

    var pekerjaan = [];
    for (var jn in jenis) pekerjaan.push(jenis[jn]);
    pekerjaan.sort(function (a, b) { return b.jumlah - a.jumlah; });
    for (var pp = 0; pp < pekerjaan.length; pp++) pekerjaan[pp].point = _bulatY_(pekerjaan[pp].point, 2);

    var rB = getRekapPointPetugasY({ tglAwal: awalBulan, tglAkhir: akhirBulan, ulp: ulpFilter });
    var rT = getRekapPointPetugasY({ tglAwal: th + '-01-01', tglAkhir: th + '-12-31', ulp: ulpFilter });
    var iB = _rekapIndexY_(rB)[nama], iT = _rekapIndexY_(rT)[nama];
    var rkD = _rankIndexY_(th + '-' + _padNY_(bl + 1), ulpFilter)[nama] || null;   // nilai rank baru (db_Yandal_Rank)

    var jmlShiftBulan = shiftList.length, jmlShiftTahun = 0;
    for (var st in shiftTahun) jmlShiftTahun++;
    var jamKerjaBulanMenit = jmlShiftBulan * 480, jamKerjaTahunMenit = jmlShiftTahun * 480;

    return {
      ok: true, petugas: nama, unit: unit || ulpFilter || '-', tim: tim || '-',
      bulanLabel: BULAN_ID[bl] + ' ' + th, tahun: th,
      rankBulanan: (rkD && rkD.rankBulanan) ? rkD.rankBulanan : (iB ? iB.rank : null),
      nilai: rkD, totalPetugasBulan: (rB.list || []).length,
      rankTahunan: iT ? iT.rank : null, totalPetugasTahun: (rT.list || []).length,
      ringkasan: {
        jumlahShiftBulan: jmlShiftBulan, jumlahP0Bulan: p0Bulan, pointBulan: _bulatY_(pointBulan, 2),
        menitEfektifBulan: menitBulan, jamEfektifBulanStr: _menitToJamStrY_(menitBulan),
        menitKotorBulan: menitKotorBulan, jamKotorBulanStr: _menitToJamStrY_(menitKotorBulan),
        shiftOverlapBulan: shiftOverlapBulan,
        jamKerjaBulanStr: _menitToJamStrY_(jamKerjaBulanMenit),
        persenBulan: jamKerjaBulanMenit ? _bulatY_(menitBulan / jamKerjaBulanMenit * 100, 1) : 0,
        rataPerP0BulanStr: p0Bulan ? _menitToJamStrY_(menitBulan / p0Bulan) : '-',
        jumlahShiftTahun: jmlShiftTahun, jumlahP0Tahun: p0Tahun, pointTahun: _bulatY_(pointTahun, 2),
        menitEfektifTahun: menitTahun, jamEfektifTahunStr: _menitToJamStrY_(menitTahun),
        persenTahun: jamKerjaTahunMenit ? _bulatY_(menitTahun / jamKerjaTahunMenit * 100, 1) : 0
      },
      shift: shiftList, pekerjaan: pekerjaan
    };
  } catch (e) { return { ok: false, error: e.message }; }
}
...
var COL_RANK_Y = {
  no: 0, kodeRank: 1, periode: 2, ulp: 3, tim: 4, petugas: 5, jumlahShift: 6, nilaiShift: 7,
  jumlahP0: 8, pointP0: 9, nilaiP0: 10,
  menitEfektif: 11, jamEfektif: 12, targetJam: 13, persenEfektif: 14, nilaiJam: 15,
  persenVCC: 16, tglVCC: 17, shiftVCC: 18, hariTempuh: 19, skorTempuh: 20, rankTercepat: 21, nilaiVCC: 22,
  nilaiTotal: 23, rankBulanan: 24, timestamp: 25
};
var HEADER_RANK_Y = ['No', 'Kode Rank', 'Periode', 'ULP', 'Tim', 'Petugas', 'Jumlah Shift', '% Nilai Shift',
  'Jumlah P0', 'Point P0', '% Nilai P0',
  'Menit Efektif', 'Jam Efektif', 'Target Jam', '% Efektif', '% Nilai Jam Efektif',
  'Persentase VCC', 'Tanggal Capai 100% VCC', 'Shift Capai 100% VCC', 'Hari Tempuh', 'Skor Tempuh',
  'Rank Tercepat VCC', '% Nilai VCC', 'Nilai Total (%)', 'Rank Bulanan', 'Time Stamp Recalc'];
...
function _shiftPetugasTglMapY_(ulpFilter) {
  var map = {}, sh = _shY_(SHEET_YANDAL.SHIFT);
  if (!sh || sh.getLastRow() <= 1) return map;
  var d = _allY_(sh), S = COL_YANDAL_SHIFT;
  for (var i = 1; i < d.length; i++) {
    if (ulpFilter && String(d[i][S.ulp] || '').trim() !== ulpFilter) continue;
    var ms = _toMillisY_(d[i][S.tanggal]); if (isNaN(ms)) continue;
    var no = _shiftNoY_(d[i][S.shift]);                                  // label "Shift 1 (08:00 - 16:00)" → 1
    if (!no) { var mm = String(d[i][S.shift] || '').match(/(\d)/); no = mm ? parseInt(mm[1], 10) : 0; }
...
function _shiftPetugasPadaTglY_(map, nama, ms) {
  ...
  return map[_fmtY_(new Date(ms), 'yyyy-MM-dd') + '|' + String(nama || '').trim()] || 0;
}
...
function _periodeStrY_(v) {
  if (v == null || v === '') return '';
  if (Object.prototype.toString.call(v) === '[object Date]') return _fmtY_(v, 'yyyy-MM');
  if (typeof v === 'number') { var msN = _toMillisY_(v); return isNaN(msN) ? '' : _fmtY_(new Date(msN), 'yyyy-MM'); }
  var s = String(v).trim();
...
function _inisialNamaY_(nama) {
  var p = String(nama || '').trim().split(/\s+/), s = '';
  for (var i = 0; i < p.length; i++) if (p[i]) s += p[i].charAt(0).toUpperCase();
  return s || 'X';
}
...
function _kodeUlpMapY_() {
  var map = {};
  try {
    var sh = _ssY_().getSheetByName('db_Users'); if (!sh) return map;
    var d = _allY_(sh);
    for (var i = 1; i < d.length; i++) {
      var u = String(d[i][COL_USERS.ulp] || '').trim(), k = String(d[i][COL_USERS.kodeUlp] || '').trim();
...
function genKodeRankY_(kodeUlp, periode, nama, dipakai) {
  ...
  var kode = dasar, n = 2;
  while (dipakai && dipakai[kode]) { kode = dasar + n; n++; }
  return kode;
}
...
function _peringkatY_(arr, getVal, asc, setFn, tiebreak) {
  ...
  var _bandingKunci = function (a, b) {
    var ka = _kunci(a), kb = _kunci(b), n = Math.max(ka.length, kb.length);
    for (var i = 0; i < n; i++) {
      var xa = Number(ka[i]) || 0, xb = Number(kb[i]) || 0;
  ...
  idxs.sort(function (a, b) {
    var x = getVal(arr[a]), y = getVal(arr[b]);
    if (x !== y) return asc ? (x - y) : (y - x);
    return _bandingKunci(arr[a], arr[b]);
  });

  var rank = 0, cnt = 0, prevIdx = -1;
  for (var j = 0; j < idxs.length; j++) {
    cnt++;
    var it = arr[idxs[j]];
    var sama = (prevIdx >= 0) && (getVal(it) === getVal(arr[prevIdx])) && (_bandingKunci(it, arr[prevIdx]) === 0);
    if (!sama) rank = cnt;
    setFn(it, rank);
    prevIdx = idxs[j];
  }
...
function _ensureSheetRankY_() {
  ...
  if (!perluHeader) {
    if (sh.getMaxColumns() < HEADER_RANK_Y.length) sh.insertColumnsAfter(sh.getMaxColumns(), HEADER_RANK_Y.length - sh.getMaxColumns());
    var hd = sh.getRange(1, 1, 1, HEADER_RANK_Y.length).getValues()[0];
    for (var hi = 0; hi < HEADER_RANK_Y.length; hi++) {
      if (String(hd[hi] || '').trim() !== HEADER_RANK_Y[hi]) { perluHeader = true; break; }
    }
  }
  if (perluHeader) {
    if (sh.getMaxColumns() < HEADER_RANK_Y.length) sh.insertColumnsAfter(sh.getMaxColumns(), HEADER_RANK_Y.length - sh.getMaxColumns());
  ...
  try { sh.getRange(2, COL_RANK_Y.periode + 1, Math.max(1, sh.getMaxRows() - 1), 1).setNumberFormat('@'); } catch (eFmt) {}
  return sh;
}
...
function _semuaPetugasY_(ulpFilter) {
  ...
  try {
    ...
    for (var i = 0; i < arr.length; i++) {
      var it = arr[i];
      var nm = (typeof it === 'string') ? it : String((it && (it.nama || it.petugas)) || '');
      nm = nm.trim(); if (!nm) continue;
      out[nm] = (it && typeof it === 'object') ? String(it.unit || it.ulp || '') : '';
    }
  } catch (e) { }
  return out;
}
...
function sinkronRankYandal(opts) {
  try {
    opts = opts || {};
    var periode = String(opts.periode || '').trim() || _fmtY_(_nowY_(), 'yyyy-MM');
    var ulpFilter = String(opts.ulp || '').trim();
    var th = Number(periode.substring(0, 4)), bl = Number(periode.substring(5, 7));
    if (!th || !bl) return { ok: false, message: 'Periode harus format YYYY-MM' };
    ...
    var C = COL_P0, d = _readSheetDual_(SHEET_YANDAL.P0, COL_P0.kodeP0, COL_P0.folderPath + 1), agg = {};
    for (var i = 0; i < d.length; i++) {
      var row = d[i];
      var ulpRow = String(row[C.ulp] || '').trim();
      if (ulpFilter && ulpRow !== ulpFilter) continue;
      ...
      var poin = Number(row[C.point]); if (isNaN(poin)) poin = 0;
      var menit = _durasiToMenitY_(row[C.durasi]); if (isNaN(menit)) menit = 0;
      var kodeShift = String(row[C.kodeShift] || '').trim() || ('tgl-' + _normTgl(row[C.tanggal]));
      var sMs = _toMillisY_(row[C.timestampPembuatan]);
      var eMs = _toMillisY_(row[C.timestampSesudah]);
      if (isNaN(eMs)) eMs = _toMillisY_(row[C.timestampPekerjaan]);
      ...
      var orang = _splitPetugasY_(row[C.petugas]);
      for (var k = 0; k < orang.length; k++) {
        var nm = orang[k]; if (!nm) continue;
        if (!agg[nm]) agg[nm] = { ulp: ulpRow, tim: String(row[C.tim] || '').trim(), jumlahP0: 0, point: 0, shift: {} };
        var a = agg[nm];
        if (!a.ulp && ulpRow) a.ulp = ulpRow;
        if (!a.tim) a.tim = String(row[C.tim] || '').trim();
        a.jumlahP0 += 1; a.point += poin;
        if (!a.shift[kodeShift]) a.shift[kodeShift] = { iv: [], tglMs: tMs, shiftNo: _shiftNoDariKodeY_(kodeShift) };   // simpan tanggal & nomor shift utk pemotongan jendela
        if (!isNaN(sMs)) {
    ...
    var sh = _ensureSheetRankY_(), R = COL_RANK_Y, idx = {}, kodeDipakai = {};
    for (var putaran = 0; putaran < 2; putaran++) {
      idx = {}; kodeDipakai = {};
      var ex = _allY_(sh), dupRows = [];
      for (var r = 1; r < ex.length; r++) {
        var kd = String(ex[r][R.kodeRank] || '').trim();
        if (_periodeStrY_(ex[r][R.periode]) !== periode) { if (kd) kodeDipakai[kd] = true; continue; }
        var kunci = String(ex[r][R.ulp] || '').trim() + '|' + String(ex[r][R.petugas] || '').trim();
        if (idx[kunci]) {
          var vccLama = Number(idx[kunci].row[R.persenVCC]) || 0, vccDup = Number(ex[r][R.persenVCC]) || 0;
          if (!vccLama && vccDup) {   // isian operator ada di baris kembar → selamatkan ke baris utama
      ...
      dupRows.sort(function (a, b) { return b - a; });   // hapus dari bawah agar nomor baris tidak bergeser
      for (var dr = 0; dr < dupRows.length; dr++) sh.deleteRow(dupRows[dr]);
      Logger.log('sinkronRankYandal: ' + dupRows.length + ' baris duplikat periode ' + periode + ' dihapus');
      // putaran ke-2: bangun ulang indeks dari sheet yang sudah bersih
    }

    // 4) Hitung nilai tiap petugas
    var ulpKode = _kodeUlpMapY_(), shiftMap = _shiftPetugasTglMapY_(ulpFilter), list = [];
    for (var nm2 in daftar) {
      var a2 = agg[nm2] || { ulp: daftar[nm2] || ulpFilter || '', tim: '', jumlahP0: 0, point: 0, shift: {} };
      var menitEf = 0, jmlShift = 0;
      // Jam efektif per shift DIBATASI jendela shift (8 jam) -> tidak mungkin lagi > jam kerja.
      for (var ks in a2.shift) { jmlShift++; var g2 = a2.shift[ks]; menitEf += _unionMenitShiftY_(g2.iv, g2.tglMs, g2.shiftNo).menit; }
      ...
      var shiftVCC = _shiftPetugasPadaTglY_(shiftMap, nm2, _toMillisY_(tglVCCraw));   // server: dicari di db_Yandal_Shift (tanggal Q + nama petugas)
      var kode = (lama && String(lama.row[R.kodeRank] || '').trim()) || genKodeRankY_(ulpKode[ulpNm], periode, nm2, kodeDipakai);
      kodeDipakai[kode] = true;

      var targetMenit = Math.round(jmlShift * MENIT_PER_SHIFT_Y * TARGET_PERSEN_EFEKTIF_Y);

      // VCC: hari tempuh & skor tempuh (hanya bila sudah 100% dan tanggalnya terbaca)
      var hariTempuh = null, skor = null, tMsV = _toMillisY_(tglVCCraw);
      if (persenVCC >= 100 && !isNaN(tMsV)) {
        var dv = new Date(tMsV);
        hariTempuh = Math.round((new Date(dv.getFullYear(), dv.getMonth(), dv.getDate()).getTime() - awalMs) / 86400000) + 1;
      ...
      var nilaiShift = Math.min(SKALA_NILAI_Y, _bulatY_(jmlShift / TARGET_SHIFT_Y * SKALA_NILAI_Y, DESIMAL_NILAI_Y));   // 24 shift = 100%
      var nilaiP0 = Math.min(SKALA_NILAI_Y, _bulatY_(a2.point / TARGET_POINT_P0_Y * SKALA_NILAI_Y, DESIMAL_NILAI_Y));
      var nilaiJam = targetMenit > 0
        ? Math.min(SKALA_NILAI_Y, _bulatY_(menitEf / targetMenit * SKALA_NILAI_Y, DESIMAL_NILAI_Y)) : 0;
      // Dasar VCC = persentase capaian × 90%. Reward kecepatan ditambahkan di langkah 5, setelah Rank Tercepat diketahui.
      var dasarVCC = Math.min(BASIS_VCC_Y,
        _bulatY_(Math.max(0, persenVCC) / 100 * BASIS_VCC_Y, DESIMAL_NILAI_Y));

      list.push({
        rowNum: lama ? lama.rowNum : 0, kode: kode, ulp: ulpNm, tim: a2.tim, petugas: nm2,
        jumlahShift: jmlShift, nilaiShift: nilaiShift, jumlahP0: a2.jumlahP0, pointP0: _bulatY_(a2.point, 2),
        menitEfektif: menitEf, jamEfektif: _menitToJamStrY_(menitEf), targetJam: _menitToJamStrY_(targetMenit),
        persenEfektif: jmlShift > 0 ? _bulatY_(menitEf / (jmlShift * MENIT_PER_SHIFT_Y) * 100, 1) : 0,
        persenVCC: persenVCC, tglVCC: tglVCCraw, shiftVCC: shiftVCC || '',
        hariTempuh: hariTempuh, skorTempuh: skor, rankTercepat: null,
        nilaiP0: nilaiP0, nilaiJam: nilaiJam, nilaiVCC: dasarVCC, rewardVCC: 0, nilaiTotal: 0,
        rankBulanan: null
      });
    }
    ...
    for (var uu in byUlp) {
      _peringkatY_(byUlp[uu], function (x) { return x.skorTempuh; }, true, function (x, rk) { x.rankTercepat = rk; });

      // Reward kecepatan + Nilai VCC final + Nilai Total (WAJIB setelah Rank Tercepat diketahui)
      for (var gg = 0; gg < byUlp[uu].length; gg++) {
        var itR = byUlp[uu][gg];
        itR.rewardVCC = (itR.persenVCC >= 100) ? _rewardVccY_(itR.rankTercepat) : 0;
        itR.nilaiVCC = Math.min(SKALA_NILAI_Y, _bulatY_(itR.nilaiVCC + itR.rewardVCC, DESIMAL_NILAI_Y));
        itR.nilaiTotal = _bulatY_(
          (itR.nilaiJam * BOBOT_JAM_Y) + (itR.nilaiShift * BOBOT_SHIFT_Y) +
          (itR.nilaiP0 * BOBOT_P0_Y) + (itR.nilaiVCC * BOBOT_VCC_Y),
          DESIMAL_NILAI_Y);   // porsi: jam efektif 55%, jumlah shift 15%, P0 18%, VCC 12%
      }

      _peringkatY_(byUlp[uu], function (x) { return x.nilaiTotal; }, false, function (x, rk) { x.rankBulanan = rk; },
        function (x) {
          return [x.menitEfektif, x.jumlahShift, x.pointP0,
            (x.skorTempuh == null ? 0 : (SKOR_TEMPUH_MAKS_Y - x.skorTempuh)), x.jumlahP0];
        });
    }

    // 6) Tulis ke sheet
    var now = _nowY_(), tambah = [];
    for (var w = 0; w < list.length; w++) {
      var it = list[w], arr = [];
      for (var z = 0; z < HEADER_RANK_Y.length; z++) arr.push('');
      arr[R.no] = w + 1;
      arr[R.kodeRank] = it.kode; arr[R.periode] = periode; arr[R.ulp] = it.ulp; arr[R.tim] = it.tim;
      arr[R.petugas] = it.petugas; arr[R.jumlahShift] = it.jumlahShift; arr[R.nilaiShift] = it.nilaiShift;
      arr[R.jumlahP0] = it.jumlahP0;
      arr[R.pointP0] = it.pointP0; arr[R.menitEfektif] = it.menitEfektif; arr[R.jamEfektif] = it.jamEfektif;
      arr[R.targetJam] = it.targetJam; arr[R.persenEfektif] = it.persenEfektif;
      arr[R.persenVCC] = it.persenVCC; arr[R.tglVCC] = it.tglVCC; arr[R.shiftVCC] = it.shiftVCC;
      ...
      arr[R.nilaiP0] = it.nilaiP0; arr[R.nilaiJam] = it.nilaiJam; arr[R.nilaiVCC] = it.nilaiVCC;
      arr[R.nilaiTotal] = it.nilaiTotal; arr[R.rankBulanan] = (it.rankBulanan == null ? '' : it.rankBulanan);
      arr[R.timestamp] = _fmtY_(now, 'dd/MM/yyyy HH:mm:ss');
      if (it.rowNum) sh.getRange(it.rowNum, 1, 1, HEADER_RANK_Y.length).setValues([arr]);
      else tambah.push(arr);
    }
    if (tambah.length) sh.getRange(sh.getLastRow() + 1, 1, tambah.length, HEADER_RANK_Y.length).setValues(tambah);

    Logger.log('sinkronRankYandal: periode=' + periode + ' petugas=' + list.length + ' barisBaru=' + tambah.length);
    return { ok: true, periode: periode, jumlahPetugas: list.length, ditambah: tambah.length };
  } catch (e) {
...
function getRankYandal(args) {
  try {
    args = args || {};
    var periode = String(args.periode || '').trim() || _fmtY_(_nowY_(), 'yyyy-MM');
    var ulpFilter = String(args.ulp || '').trim();
    if (args.sinkron) sinkronRankYandal({ periode: periode, ulp: ulpFilter });

    var sh = _ensureSheetRankY_(), R = COL_RANK_Y, d = _allY_(sh), out = [];
    for (var i = 1; i < d.length; i++) {
      if (_periodeStrY_(d[i][R.periode]) !== periode) continue;
      var u = String(d[i][R.ulp] || '').trim();
      if (ulpFilter && u !== ulpFilter) continue;
      out.push({
        kodeRank: String(d[i][R.kodeRank] || ''), ulp: u, tim: String(d[i][R.tim] || ''),
        petugas: String(d[i][R.petugas] || ''), jumlahShift: _toNumY_(d[i][R.jumlahShift]),
        nilaiShift: _toNumY_(d[i][R.nilaiShift]),
        jumlahP0: _toNumY_(d[i][R.jumlahP0]), pointP0: _toNumY_(d[i][R.pointP0]),
        menitEfektif: _toNumY_(d[i][R.menitEfektif]), jamEfektif: String(d[i][R.jamEfektif] || ''),
        targetJam: String(d[i][R.targetJam] || ''), persenEfektif: _toNumY_(d[i][R.persenEfektif]),
        persenVCC: _toNumY_(d[i][R.persenVCC]), hariTempuh: d[i][R.hariTempuh],
        rankTercepat: d[i][R.rankTercepat], nilaiP0: _toNumY_(d[i][R.nilaiP0]),
        nilaiJam: _toNumY_(d[i][R.nilaiJam]), nilaiVCC: _toNumY_(d[i][R.nilaiVCC]),
        nilaiTotal: _toNumY_(d[i][R.nilaiTotal]), rankBulanan: d[i][R.rankBulanan]
      });
    }
    out.sort(function (a, b) { return (b.nilaiTotal || 0) - (a.nilaiTotal || 0); });
...
function getVccYandalList(args) {
  try {
    ...
    var periode = String(args.periode || '').trim() || _fmtY_(_nowY_(), 'yyyy-MM');
    var ulpFilter = String(args.ulp || '').trim();
    var sh = _ensureSheetRankY_(), R = COL_RANK_Y;

    var adaPeriode = false, d0 = _allY_(sh);
    for (var c = 1; c < d0.length; c++) {
      if (_periodeStrY_(d0[c][R.periode]) !== periode) continue;
      if (ulpFilter && String(d0[c][R.ulp] || '').trim() !== ulpFilter) continue;
      adaPeriode = true; break;
    }
    if (args.sinkron || !adaPeriode) sinkronRankYandal({ periode: periode, ulp: ulpFilter });
    ...
    out.sort(function (a, b) {
      return String(a.ulp).localeCompare(String(b.ulp)) || String(a.petugas).localeCompare(String(b.petugas));
    });
    for (var n = 0; n < out.length; n++) out[n].no = n + 1;
    return { ok: true, periode: periode, ulp: ulpFilter, list: out, jumlahPetugas: out.length };
  } catch (e) { return { ok: false, error: e.message, list: [] }; }
}
...
function setVccYandal(args) {
  try {
    ...
    var persen = (args.persenVCC === '' || args.persenVCC == null) ? 0 : _toNumY_(args.persenVCC);
    if (isNaN(persen)) return { ok: false, error: 'Persentase VCC harus berupa angka.' };
    if (persen < 0 || persen > 100) return { ok: false, error: 'Persentase VCC harus antara 0 dan 100.' };

    var tglStr = String(args.tanggal || '').trim();
    var tglMs = tglStr ? _toMillisY_(tglStr) : NaN;
    if (tglStr && isNaN(tglMs)) return { ok: false, error: 'Tanggal Capai 100% VCC tidak terbaca.' };

    var sh = _ensureSheetRankY_(), R = COL_RANK_Y;
    var f = _findRowY_(sh, R.kodeRank, kodeRank);
    if (!f) return { ok: false, error: 'Baris rank tidak ditemukan: ' + kodeRank };
    var periode = _periodeStrY_(f.row[R.periode]);

    // Tanggal wajib bila sudah 100% (dipakai menghitung Hari Tempuh & Rank Tercepat).
    if (persen >= 100 && !tglStr) return { ok: false, error: 'Persentase 100% wajib disertai Tanggal Capai 100% VCC.' };
    // Tanggal harus berada dalam periode baris tsb.
    if (tglStr && periode) {
      if (_fmtY_(new Date(tglMs), 'yyyy-MM') !== periode) {
        return { ok: false, error: 'Tanggal harus berada di periode ' + periode + '.' };
      }
    }
    ...
    var sync = null;
    try { sync = sinkronRankYandal({ periode: periode, ulp: String(f.row[R.ulp] || '').trim() }); }
    catch (eS) { Logger.log('setVccYandal: sinkron gagal — ' + eS); }

    Logger.log('setVccYandal: ' + kodeRank + ' persen=' + persen + ' tgl=' + (tglStr || '-') +
               ' oleh=' + String(args.username || '-'));
    return { ok: true, kodeRank: kodeRank, periode: periode, persenVCC: persen, tanggal: tglStr,
...
function createRankTriggerY() {
  var fn = 'sinkronRankYandal', tg = ScriptApp.getProjectTriggers();
  for (var i = 0; i < tg.length; i++) if (tg[i].getHandlerFunction() === fn) ScriptApp.deleteTrigger(tg[i]);
  ScriptApp.newTrigger(fn).timeBased().everyDays(1).atHour(0).nearMinute(30).create();
...
function sweepDurasiJarakYandalP0(opts) {
  ...
  var sh = _shY_(SHEET_YANDAL.P0);
  if (!sh || sh.getLastRow() <= 1) return { ok:true, baris:0, durasi:0, jarak:0, jarakAntarP0:0, diproses:0, terpotong:false };
  var d = _allY_(sh), C = COL_P0;
  var nDur = 0, nJarak = 0, nAntar = 0, nProses = 0, terpotong = false;

  for (var i = 1; i < d.length; i++) {
    if (max && nProses >= max) { terpotong = true; break; }
    var rowNum = i + 1, row = d[i];
    ...
    if (adaFotoSesudah && (durKosong || force)) {
      try {
        var durLama = String(sh.getRange(rowNum, C.durasi + 1).getValue() || '');
        if (force && durLama) sh.getRange(rowNum, C.durasi + 1).clearContent();
        _recalcDurasiRowY_(sh, rowNum);
        var durBaru = String(sh.getRange(rowNum, C.durasi + 1).getValue() || '').trim();
        if (durBaru) { nDur++; did = true; }
        else if (force && durLama) { _setTextY_(sh, rowNum, C.durasi, durLama); }   // gagal hitung -> pulihkan
      } catch (eD) { Logger.log('sweepDurasiJarakYandalP0: durasi gagal baris ' + rowNum + ' — ' + eD); }
    }

    // 2) JARAK Closing->Pekerjaan (S) — perlu Lat/Long (N/O) & Lat/Long Closing (Q/R). _recalcJarakRowY_ tak menulis bila koordinat kurang.
    var koordLengkap = String(row[C.lat] || '') !== '' && String(row[C.long] || '') !== '' &&
                       String(row[C.latClosing] || '') !== '' && String(row[C.longClosing] || '') !== '';
    var jarakKosong = String(row[C.jarak] || '').trim() === '';
    if (koordLengkap && (jarakKosong || force)) {
      try {
        var sJarak = _recalcJarakRowY_(sh, rowNum);
        if (sJarak) { nJarak++; did = true; }
      } catch (eJ) { Logger.log('sweepDurasiJarakYandalP0: jarak gagal baris ' + rowNum + ' — ' + eJ); }
    }
    ...
    if (adaShift && (antarKosong || force)) {
      try {
        ...
        if (sAntar) { nAntar++; did = true; }
      } catch (eT) { Logger.log('sweepDurasiJarakYandalP0: jarak antar P0 gagal baris ' + rowNum + ' — ' + eT); }
    }

    if (did) nProses++;
  }

  Logger.log('sweepDurasiJarakYandalP0' + (force ? ' [FORCE]' : '') + ' -> durasi=' + nDur +
             ' jarak=' + nJarak + ' jarakAntarP0=' + nAntar + ' barisDiproses=' + nProses +
             (terpotong ? ' (TERPOTONG oleh max)' : ''));
  return { ok:true, baris:d.length - 1, durasi:nDur, jarak:nJarak, jarakAntarP0:nAntar,
           diproses:nProses, terpotong:terpotong };
}

// Preview manual (recalc hanya yang KOSONG; aman). Cek hasil di Log (View > Logs).
function previewRecalcDurasiJarakYandalP0(opts) { return sweepDurasiJarakYandalP0(opts || {}); }
// Recalc PAKSA semua baris (abaikan nilai lama; menimpa Durasi/Jarak yg sudah ada). Gunakan hati-hati.
function recalcPaksaDurasiJarakYandalP0(opts) { opts = opts || {}; opts.force = true; return sweepDurasiJarakYandalP0(opts); }
...
function createRecalcDurasiJarakTriggerY(minutes) {
  var m = Number(minutes || 5); if ([1,5,10,15,30].indexOf(m) < 0) m = 5;   // interval valid Apps Script
  var trs = ScriptApp.getProjectTriggers();
  for (var i = 0; i < trs.length; i++) if (trs[i].getHandlerFunction() === 'sweepDurasiJarakYandalP0') ScriptApp.deleteTrigger(trs[i]);
  ScriptApp.newTrigger('sweepDurasiJarakYandalP0').timeBased().everyMinutes(m).create();
...
function hapusRecalcDurasiJarakTriggerY() {
  var trs = ScriptApp.getProjectTriggers(), n = 0;
  for (var i = 0; i < trs.length; i++) if (trs[i].getHandlerFunction() === 'sweepDurasiJarakYandalP0') { ScriptApp.deleteTrigger(trs[i]); n++; }
